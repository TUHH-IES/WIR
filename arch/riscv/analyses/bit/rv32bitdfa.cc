                                   ...
                                   std::map<WIR_id_t, WIR_UpDownValue> &results )
{
  ...
  */
  ...
  if ( o.getOpCode() == RV32I::OpCode::ADDI ) {
    if ( o.getOperationFormat() == RV32I::OperationFormat::RRC12_1 ) {
      ...
      // The last operand is a label.
      auto it = o.getExplicitParameters().rbegin();
      const auto &label = dynamic_cast<const WIR_LabelParameter &>( it->get() );
                                    ...
                                    std::map<WIR_id_t, WIR_UpDownValue> &results )
{
  ...
  */
  ...
  if ( ( o.getOpCode() == RV32I::OpCode::SLL ) ||
    ...
    for ( unsigned int i = shiftVal; i < aOut.getBitWidth(); ++i )
      if ( aOut.at( i ) == WIR_L4::bX )
          aIn.setBit( i - shiftVal, WIR_L4::bX );
