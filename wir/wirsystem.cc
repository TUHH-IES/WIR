...
/*
  ...
  - If n already is a path (either absolute or relative), i.e., n contains at
    least one '/' somewhere, this input path n is directly used. Note that it
    lies in the sole responsibility of the caller of this constructor that n is
    a valid path in this case!
  - Otherwise, the constructor probes the WIR-internal directory structure
    (archdir and sysconfdir) and checks whether a file named n exists below
    archdir or sysconfdir. If so, the full path to the found file is used.
...
WIR_System::WIR_System( WIR_System &&__o ) :
  ...
  // cppcheck-suppress accessMoved
  WIR_Name_API { std::move( __o ) },
  // cppcheck-suppress accessMoved
  WIR_SymbolTable_API { std::move( __o ) },
  mCompilationUnits { std::move( __o.mCompilationUnits ) },
  mCompilationUnitReferences { std::move( __o.mCompilationUnitReferences ) },
  mSystemName { std::move( __o.mSystemName ) },
  mComponentPointers { std::move( __o.mComponentPointers ) },
  mComponentReferences { std::move( __o.mComponentReferences ) },
  mComponentHierarchyLevels { std::move( __o.mComponentHierarchyLevels ) },
  mFlowFactPointers { std::move( __o.mFlowFactPointers ) },
  mFlowFactReferences { std::move( __o.mFlowFactReferences ) },
  mDontOptimize { __o.mDontOptimize },
  mWarnedRegions { std::move( __o.mWarnedRegions ) }
{
  ...
  mTaskManager = std::move( __o.mTaskManager );
  (void) __o.mTaskManager.release();
  (*mTaskManager).onInsert( this );
};
...
WIR_System::~WIR_System( void )
{
  ...
  DOUT(
    "System contains " << mFlowFactReferences.size() << "/" <<
    mFlowFactPointers.size() << " flow facts to be cleared." << endl; );

  // ufAssert( checkFlowFacts() );
...
WIR_System & WIR_System::operator = ( WIR_System &&__o )
{
  ...
  // cppcheck-suppress accessMoved
  WIR_Name_API::operator = ( std::move( __o ) );
  // cppcheck-suppress accessMoved
  WIR_SymbolTable_API::operator = ( std::move( __o ) );
  mDontOptimize = __o.mDontOptimize;
...
WIR_CompilationUnit & WIR_System::pushBackCompilationUnit( WIR_CompilationUnit &&o )
{
  ...
  mCompilationUnits.emplace_back( std::move( o ) );
  mCompilationUnits.back().onInsert( this );
  mCompilationUnitReferences.push_back( mCompilationUnits.back() );
...
WIR_CompilationUnit & WIR_System::pushFrontCompilationUnit( WIR_CompilationUnit &&o )
{
  ...
  mCompilationUnits.emplace_front( std::move( o ) );
  mCompilationUnits.front().onInsert( this );
  mCompilationUnitReferences.push_front( mCompilationUnits.front() );
                                                                                                    ...
                                                                                                    WIR_CompilationUnit &&o )
{
  ...
  for ( auto itr = mCompilationUnitReferences.begin(); itr != pos;
        ++itr, ++it ) ;
  auto it1 = mCompilationUnits.insert( it, std::move( o ) );
  (*it1).onInsert( this );
                                                                                                     ...
                                                                                                     WIR_CompilationUnit &&o )
{
  ...
  for ( auto itr = mCompilationUnitReferences.begin(); itr != pos;
        ++itr, ++it1 ) ;
  auto it2 = mCompilationUnits.insert( it1, std::move( o ) );
  (*it2).onInsert( this );
...
bool WIR_System::containsCompilationUnit( WIR_id_t id ) const
{
  ...
  return(
    any_of(
      mCompilationUnits.begin(), mCompilationUnits.end(),
      [&]( const WIR_CompilationUnit &c ) { return( c.getID() == id ); } ) );
};
...
bool WIR_System::containsCompilationUnit( const WIR_CompilationUnit &o ) const
{
  ...
  return(
    any_of(
      mCompilationUnits.begin(), mCompilationUnits.end(),
      [&]( const WIR_CompilationUnit &c ) { return( c == o ); } ) );
};
...
std::list<std::reference_wrapper<WIR_CompilationUnit>>::const_iterator WIR_System::findCompilationUnit( WIR_id_t id ) const
{
  ...
  return(
    find_if(
      mCompilationUnitReferences.begin(), mCompilationUnitReferences.end(),
      [&]( reference_wrapper<WIR_CompilationUnit> ref ) {
...
cout << BOOST_CURRENT_FUNCTION << endl;
  ...
  return(
    any_of(
      mComponentReferences.begin(), mComponentReferences.end(),
      [&]( const reference_wrapper<WIR_SystemComponent> ref ) {
        return( ref.get().getID() == id ); } ) );
};
...
{
  DSTART(
    "WIR_SystemComponentSet WIR_System::findMemoryRegions(const "
    "WIR_AddressRange&) const" );
...
/*
  getDataLayout computes a map representing a system's data layout.
*/
WIR_DataLayoutMap WIR_System::getDataLayout( void ) const
{
  DSTART( "WIR_DataLayoutMap WIR_System::getDataLayout() const" );

  WIR_DataLayoutMap res;

  for ( const WIR_CompilationUnit &c : mCompilationUnitReferences )
    for ( WIR_Data &d : c.getData() ) {
      const WIR_Section &sec = findSymbol( d ).getSection();
      res[ const_cast<WIR_Section &>( sec ) ].push_back( d );
    }
...
/*
  checkFlowFacts inspects the flow fact-related data structures of a WIR system
  and verifies that they are all in a coherent state.
*/
bool WIR_System::checkFlowFacts( void ) const
{
  DSTART( "bool WIR_System::checkFlowFacts() const" );

  DOUT(
    "Checking flow facts of WIR system " << hex << this << dec << " (ID " <<
    getID() << ")." << endl );

  DOUT(
    "mFlowFactPointers.size() = " << mFlowFactPointers.size() <<
    ", mFlowFactReferences.size() = " << mFlowFactReferences.size() << "." <<
    endl );

  // Check whether the size of both flow fact lists is coherent.
  if ( mFlowFactPointers.size() != mFlowFactReferences.size() ) {
    DOUT( "  Mismatch of flow fact list sizes found." << endl );
    return( false );
  }

  // Check all flow facts stored in both flow fact lists.
  DDECLARE( unsigned int cnt = 0; );
  auto itR = mFlowFactReferences.begin();
  for ( auto itP = mFlowFactPointers.begin(); itP != mFlowFactPointers.end();
        ++itP, ++itR ) {
    DOUT( endl << "Checking flow fact no. " << cnt << "." << endl );

    auto *ffPtr = itP->get();
    auto &ffRef = itR->get();

    // Check whether the currently inspected flow fact pointer and reference
    // refer to the very same flow fact.
    if ( &ffRef != ffPtr ) {
      DOUT(
        "  Flow fact pointer points to " << hex << ffPtr << " while " <<
        "reference refers to " << &ffRef << dec << "." << endl );
      return( false );
    }

    // Check the currently inspected flow fact in detail.
    if ( !ffPtr->checkFlowFact( *this ) )
      return( false );
...
void WIR_System::clearFlowFacts( void )
{
  ...
  DOUT(
    "Clearing a total of " << mFlowFactReferences.size() << "/" <<
    mFlowFactPointers.size() << " flow facts." << endl; );
...
void WIR_System::copySystem( const WIR_System &__o )
{
  ...
  clearComponents();
  for ( const WIR_SystemComponent &c : __o.mComponentReferences )
  ...
  for ( auto p : __o.mComponentHierarchyLevels )
    for ( const WIR_SystemComponent &m : p.second ) {
  ...
  for ( WIR_SystemComponent &c : __o.mComponentReferences )
    if ( c.getType() == WIR_SystemComponentType::memory ) {
      ...
      for ( const auto &l : o.getHierarchy() ) {
        list<reference_wrapper<WIR_SystemComponent>> hierarchy;
        transform(
          l.begin(), l.end(), back_inserter( hierarchy ),
          [&]( const WIR_SystemComponent &oc ) {
            return( ref( *(componentIDMap[ oc.getID() ] ) ) ); } );
        m.insertHierarchy( std::move( hierarchy ) );
      }

      // Adjust WIR_MemoryRegion::mSections.
      WIR_SectionSet newSections;
      for ( const WIR_Section &s : m.mSections )
        newSections.insert( *(sectionIDMap[ s.getID() ]) );
      m.mSections = std::move( newSections );
    }
  ...
  clearFlowFacts();
  for ( const WIR_FlowFact &ff : __o.mFlowFactReferences ) {
  ...
  clearCompilationUnits();
  for ( const WIR_CompilationUnit &c : __o.mCompilationUnitReferences )
    pushBackCompilationUnit(
      std::move( *(compilationUnitIDMap[ c.getID() ]) ) );
          ...
          for ( WIR_Operation &o : i )
            for ( auto it = o.getParameters().begin();
              ...
              if ( p.getType() == WIR_ParameterType::label ) {
                const auto &labP =
                  dynamic_cast<const WIR_LabelParameter &>( p );

                if ( labP.getLabelType() == WIR_SymbolType::function ) {
                  const WIR_Function &fct = labP.getFunction();
...
/*
  ...
  - If n already is a path (either absolute or relative), i.e., n contains at
    ...
    a valid path in this case!
  - Otherwise, getConfigPath probes the WIR-internal directory structure
...
// cppcheck-suppress checkLevelNormal
{
    ...
    for ( const WIR_SystemConfig::Paragraph &p : configFile.getParagraphs() ) {
      ...
      if ( ( round == 1 ) &&
        ...
        for ( auto prop : p.getProperties() ) {
          ...
          unsigned int line1 = prop.second.line;
          ...
          if ( key != "isa" )
            ufWarnMsg << ufFile( f, line1 ) << "Currently unsupported core "
                      << "attribute " << "'" + key + "'" << " found, "
                      << "ignoring." << endl;
        }
      }
      ...
      if ( ( round == 2 ) &&
        ...
        if ( p.getType() == WIR_SystemComponentType::bus ) {
          ...
          WIR_BusArbitrationType arbitration = WIR_BusArbitrationType::fp;
          ...
          switch ( arbitration ) {
              ...
              for ( auto prop : p.getProperties() ) {
                ...
                if ( key == "priorities" ) {
                  ...
                  while ( level != "" ) {
                    ...
                    const WIR_SystemComponent &comp = *it;
                    ...
                    arb.setPriority(
                      dynamic_cast<const WIR_BaseProcessor &>( comp ),
                      currPrio-- );
                  ...
                  for ( const WIR_BaseProcessor &core :
            ...
            case WIR_BusArbitrationType::pd: {
              ...
              auto it1 =
                find_if(
                  p.getProperties().begin(), p.getProperties().end(),
                  [&]( pair<string,
                            struct WIR_SystemConfig::Paragraph::value> p ) {
                    return( p.first == "arbitrationdelay" ); } );
              if ( it1 != p.getProperties().end() )
                arbitrationDelay = toLong( it1->second.value );
              ...
              for ( auto prop : p.getProperties() ) {
                ...
                if ( key == "priorities" ) {
                  ...
                  while ( level != "" ) {
                    auto it3 = findComponent( level );

                    if ( it3 == getComponents().end() )
                      throw
                        ufFatalError(
                          f, line,
                          "Unknown system component '" + level +
                    ...
                    const WIR_SystemComponent &comp = *it3;
            ...
            case WIR_BusArbitrationType::tdma: {
              ...
              auto it2 =
                find_if(
                  ...
                  [&]( pair<string,
                            struct WIR_SystemConfig::Paragraph::value> p ) {
                    return( p.first == "arbitrationdelay" ); } );
              if ( it2 != p.getProperties().end() )
                arbitrationDelay = toLong( it2->second.value );
              ...
              DACTION(
                DOUT( "  Slots:" );
                for ( const auto &slot : arb1.getSlots() )
                  DOUT(
                    " { " << slot.getIndex() << ", '"
                ...
                DOUT( endl ); );
              DACTION(
                for ( const WIR_BaseProcessor &c :
                        getComponents<WIR_BaseProcessor>() ) {
                  DOUT( "  Slots of '" << c.getName() << "':" );
                  for ( const auto &slot : arb.getSlots( c ) )
                    DOUT(
                      " { " << slot.getIndex() << ", '"
                            << slot.getOwner().getName() << "', "
                            << slot.getStart() << ", " << slot.getLength()
          ...
          for ( auto prop : p.getProperties() ) {
            ...
            auto checkParamSet = [&]( const string &k, unsigned int &v, bool &b,
                                      unsigned int &l ) {
              if ( key == k ) {
                v = toLong( value );
                l = line;
      ...
      if ( ( round == 3 ) &&
        ...
        for ( auto prop : p.getProperties() ) {
          ...
          if ( key == "hierarchy" ) {
            ...
            r.insertHierarchy( std::move( hierarchy ) );
          } else
        ...
        DACTION(
          ...
          for ( const auto &l : r.getHierarchy() ) {
            DOUT( " {" );
            for ( const WIR_SystemComponent &m : l )
              DOUT( " '" << m.getName() << "'" );
            DOUT( " }" );
          }
          DOUT( endl );
  ...
  // .bss and .rodata.
  for ( const WIR_BaseProcessor &c : getComponents<WIR_BaseProcessor>() ) {
    list<string> secNames { ".text", ".data", ".bss", ".rodata" };
    auto it =
      find_if(
        secNames.begin(), secNames.end(),
        [&]( const std::string &secName ) {
          return( !c.containsSection( secName ) ); } );
    if ( it != secNames.end() )
      throw
        ufFatalError(
          f, 0,
          "Mandatory default section '" + *it + "' not specified for " +
            "core '" + c.getName() + "'.",
          false );
  }
                                 ...
                                 const std::string &f, unsigned int l ) const
{
  ...
  for ( const WIR_Section &s : sections )
    r.insertSection( s );
};
                                       ...
                                       const std::string &f )
{
  ...
  for ( auto &p : l ) {
    ...
    for ( const WIR_BaseProcessor &c : cores ) {
      for ( const auto &s : loadedSectionNames ) {
...
void WIR_System::computeHierarchyLevels( void )
{
  ...
  for ( const WIR_SystemComponent &m : mComponentReferences ) {
    if ( m.getType() != WIR_SystemComponentType::memory ) {
  ...
  while ( !startNodes.empty() ) {
    ...
    const auto &succs = successors[ currentNode ];
    for ( auto succ : succs ) {
...
void WIR_System::insertSymbols( const WIR_CompilationUnit &u )
{
  ...
  for ( const WIR_Data &d : u.getData() )
    insertSymbol( d );
};
...
void WIR_System::computeSectionLayout( void )
{
  ...
  for ( const WIR_CompilationUnit &c : getCompilationUnits() ) {

    // Process data objects.
    for ( const WIR_Data &d : c.getData() ) {
...
void WIR_System::computePhysicalAddresses( void )
{
  ...
  // Iterate over all memory regions.
  for ( const WIR_MemoryRegion &r : getComponents<WIR_MemoryRegion>() ) {
    size_t regionOffset = r.getBaseAddress();
    const auto &sectionList = mRegion2sections[ r.getID() ];

    // Iterate over all sections assigned to a memory region.
    for ( const WIR_Section &sec : sectionList ) {
      size_t alignment;
