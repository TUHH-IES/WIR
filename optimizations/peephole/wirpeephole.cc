...
void WIR_Peephole::runOptimization( WIR_Function &f )
{
  ...
  for ( const WIR_BasicBlock &b : f )
    for ( auto it = b.begin(); it != b.end(); ++it )
      ...
      for ( const auto &p : getPeepholes( it ) )
        if ( matchPeephole( p ) )
          transformPeephole( p );
};
...
*/
WIR_Peephole::peepholeList WIR_Peephole::getPeepholes( std::list<
                                                         std::reference_wrapper<
                                                           WIR::WIR_Instruction>>::const_iterator it ) const
{
  DSTART(
    "WIR_Peephole::peepholeList WIR_Peephole::getPeepholes("
  ...
  while ( !q.empty() ) {
    ...
    if ( p.size() < maxLen ) {
      ...
      auto lastPos = p.back();
      const WIR_BasicBlock &b = lastPos->get().getBasicBlock();
      ++lastPos;
