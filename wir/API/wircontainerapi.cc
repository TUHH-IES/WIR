...
*/
void WIR_Container_API::eraseContainer( const WIR_BaseContainer &c )
{
  DSTART( "void WIR_Container_API::eraseContainer(const WIR_BaseContainer&)" );
  ...
  if ( it != mContainerPointers.end() )
    return(
      any_of(
        ...
        [&]( const unique_ptr<WIR_BaseContainer> &ptr ) {
          return( *ptr == c ); } ) );
...
WIR_Container_API::WIR_Container_API( WIR_Container_API &&__o ) :
  mContainerPointers { std::move( __o.mContainerPointers ) }
{
...
void WIR_Container_API::copyContainers( const WIR_Container_API &__o )
{
  ...
  transform(
    originalContainers.begin(), originalContainers.end(),
    back_inserter( originalIDs ),
    [&]( const WIR_BaseContainer &c ) { return( c.getID() ); } );
