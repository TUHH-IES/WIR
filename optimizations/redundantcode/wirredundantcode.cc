...
void WIR_RedundantCode::runOptimization( WIR_Function &f )
{
  ...
  WIR_DominationAnalysis dom( f, true );
  dom.analyze();
  ...
  for ( WIR_BasicBlock &b : f ) {
    ...
    for ( WIR_Instruction &i : b ) {
      ...
      auto &domContainer = i.getContainers<WIR_Domination>().begin()->get();
        ...
        for ( WIR_Parameter &p2 : o2.getExplicitParameters() ) {
          ...
          DACTION(
            ...
            if ( rp2.getRegister().isVirtual() ) {
              const auto &vreg =
                dynamic_cast<const WIR_VirtualRegister &>( rp2.getRegister() );
          ...
          for ( WIR_RegisterParameter &rp1 : availableDefinitions ) {
            ...
            // Next, ensure that the defining operation dominates the using
            // operation.
            if ( !domContainer.getDominatorInstructions().count(
                   rp1.getOperation().getInstruction() ) ||
                 ( i == rp1.getOperation().getInstruction() ) )
              continue;
            ...
            DACTION(
              if ( redundant ) {
                ...
                for ( auto it1 = i1.getBasicBlock().getInstructions().begin();
                      (*it1).get() != i1; ++it1, ++bbPos ) ;
                DOUT(
  ...
  f.eraseContainers( WIR_AvailableDefinitions::getContainerTypeID(), true );
...
{
  DSTART(
    "virtual bool WIR_RedundantCode::areRedundant(const "
    "WIR_RegisterParameter&, const WIR_UpDownValue&, const "
    "WIR_RegisterParameter&, const WIR_UpDownValue&) const" );
...
{
  DSTART(
    "bool WIR_RedundantCode::checkRegisters(const WIR_RegisterParameter&, "
    "const WIR_RegisterParameter&) const" );

  const auto &defReg = def.getRegister();
  const auto &useReg = use.getRegister();
  ...
  if ( defReg.isVirtual() && useReg.isPhysical() ) {
    const auto &defVReg = dynamic_cast<const WIR_VirtualRegister &>( defReg );
    if ( defVReg.isPrecolored() && ( defVReg.getPrecolor() == useReg ) )
      return( false );
  } else

  if ( defReg.isPhysical() && useReg.isVirtual() ) {
    const auto &useVReg = dynamic_cast<const WIR_VirtualRegister &>( useReg );
...
{
  DSTART(
    "bool WIR_RedundantCode::areRedundant(const WIR_UpDownValue&, const "
...
void WIR_RedundantCode::replaceParameters( void )
{
  ...
  for ( auto &[ use, def ] : mParameterReplacement ) {
    auto useID = use.get().getID();

    WIR_Operation &o = use.get().getOperation();
    ...
    DACTION(
      stringstream sstr1;
      sstr1 << wir << o;
      DOUT( "Replacing used parameter '" << use.get() );
      if ( use.get().getRegister().isVirtual() ) {
        const auto &vreg =
          dynamic_cast<const WIR_VirtualRegister &>( use.get().getRegister() );
        if ( vreg.isPrecolored() )
          DOUT( "/" << vreg.getPrecolor().getName() );
      }
      DOUT( "' (ID " << use.get().getID() << ") by definition from '" << *def );
    ...
    if ( use.get().containsContainers( WIR_BitValues::getContainerTypeID() ) ) {
      // Get the used parameter's bitValue container.
      auto &c = use.get().getContainers<WIR_BitValues>().begin()->get();
      ...
      for ( auto &inEdge : c.getInValues() ) {
        ...
        srcContainer.eraseOutValues( use.get() );
      }
    }

    auto it = o.replaceParameter(
      o.findParameter( use.get() ),
      WIR_RegisterParameter(
        def->getRegister(), WIR_Usage::use, use.get().isImplicit() ) );
