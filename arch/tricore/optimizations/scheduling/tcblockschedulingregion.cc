...
{
  DSTART(
    "TC_BlockSchedulingRegion::TC_BlockSchedulingRegion(WIR_BasicBlock&, bool, "
    "bool)" );
...
{
  DSTART(
    "virtual WIR_Instruction* "
    "TC_BlockSchedulingRegion::moveOperations(list<WIR_Operation*>&, const "
    "WIR_Instruction*)" );
  ...
  auto itNew =
    bb.insertInstruction(
      ( pred != nullptr ) ? next( bb.findInstruction( *pred ) ) : bb.end(),
      WIR_Instruction {} );
  auto &i = itNew->get();
  ...
  if ( o1 != nullptr ) {
    ...
    auto *newOp = &(i.moveOperation( *o1 ));

    // Replace all occurrences of o1 by newOp in mDGraph.
...
void TC_BlockSchedulingRegion::postProcessingHook( void )
{
  ...
  for ( const WIR_Operation &nop : mSiliconBugNOPs ) {
    const auto &nopInstr = nop.getInstruction();

    const WIR_Operation &o = mSiliconBugNOPPartner.at( nop.getID() );
    const auto &oInstr = o.getInstruction();
...
{
  DSTART(
    "virtual bool TC_BlockSchedulingRegion::checkUCert(const WIR_Operation&, "
    "const WIR_RegisterParameter&) const" );

  const auto &o2 = rp2.getOperation();
...
{
  DSTART(
    "virtual bool TC_BlockSchedulingRegion::checkCtrl(const WIR_Operation&, "
    "const WIR_RegisterParameter&) const" );
...
{
  DSTART(
    "virtual long long int TC_BlockSchedulingRegion::computeLatency(const "
    "WIR_Operation&, const WIR_Operation&, WIR_DGEdgeType)" );
...
{
  DSTART(
    "virtual long long int TC_BlockSchedulingRegion::getLatency(const "
    "WIR_Operation&)" );
...
{
  DSTART(
    "long long int TC_BlockSchedulingRegion::computeSiliconBugStall(const "
  ...
  if ( i1.containsContainers( WIR_Comment::getContainerTypeID() ) &&
    ...
    auto it =
      find_if(
        c1.begin(), c1.end(),
        [&]( const reference_wrapper<WIR_Comment> ref ) {
          return(
            ref.get().getText().substr( 0, 20 ) ==
              "Fix for silicon bug " ); } );
    if ( it != c1.end() )
      s = it->get().getText();

    if ( !s.empty() )
      for ( const WIR_Comment &c : c2 )
        if ( c.getText() == s ) {
...
{
  DSTART(
    "virtual long long int TC_BlockSchedulingRegion::getStartCycle(const "
    "WIR_Operation&) const" );
...
void TC_BlockSchedulingRegion::determineCallRetSituation( void )
{
  ...
  const WIR_BasicBlock &b = *(mBasicBlocks.front());
  ...
  for ( WIR_BasicBlock &pred : b.getPredecessors() ) {
    const auto *op = lastOp( pred );

    if ( op && ( op->isCall() || op->isIndirectCall() || op->isReturn() ) ) {
      mCallRetSituation = true;
      DOUT(
        "Block region '" << b.getName() << "' is in a CALL/RET scenario." <<
        endl );
      return;
    }
  }
