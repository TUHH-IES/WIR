...
/*!
  ...
  - Full support of arbitrary WIR register hierarchies, also during coalescing
...
class WIR_GraphColoring : public WIR_BitOpt
{
  ...
  protected:
    ...
    */
    ...
    /*!
      ...
      @author Heiko Falk <Heiko.Falk@tuhh.de>
    */
    bool isStackPointer( const WIR_BaseRegister & ) const;
    ...
    /*!
      ...
      Since identifying caller-saved registers is processor-specific, this
      method is purely virtual.

      @author Heiko Falk <Heiko.Falk@tuhh.de>
    */
    virtual bool isCallerSaved( const WIR_PhysicalRegister &r ) const = 0;
    ...
    */
    ...
    /*!
      ...
      @return isFunctionReturnMove returns true iff
              - the register used by the move an argument passing register
                according to a processor's ABI, and
      ...
      @author Heiko Falk <Heiko.Falk@tuhh.de>
    */
    virtual bool isFunctionReturnMove( const WIR_Operation &o ) const = 0;
    ...
    */
    ...
    /*!
      @brief avoidCoalescing checks whether the two given registers which are
             both move-related must not be coalesced due to processor-specific
             reasons.
      ...
      trouble when implementing the saving/restoring polity of caller-saved
      registers later.

      If we would allow coalescing in the second case, the non pre-colored
      register will be allocated to the caller-saved register specified in the
      ...
      Since there might be additional, processor-specific scenarios where
      coalescing should be avoided, this method is virtual so that it can be
      overloaded.
    ...
    */
    ...
    /*!
      ...
      this method is virtual so that it can be overloaded. The method of this
      class is just a dummy always returning maxInt.
    ...
    */
    ...
    /*!
      ...
      this method is virtual so that it can be overloaded. The method of this
      class is just a dummy always returning an empty list of rematerialization
      instructions.
    ...
    */
    ...
    /*!
      @brief applyCallerSavingPolicy inserts dedicated spill-stores and -loads
             of caller-saved registers that are alive across some function call.

      @param[in,out] f A reference to a %WIR function.

      Since this task might be processor-specific for some actual processor's
      ABI, this method is virtual and can be overloaded if required.

      @author Heiko Falk <Heiko.Falk@tuhh.de>
    */
    virtual void applyCallerSavingPolicy( WIR_Function & );
    ...
    */
    ...
    /*!
      @brief mCallerSavingStackSpace stores for the currently processed %WIR
             function, how many additional bytes have to be allocated in the
             function's stack frame to store caller-saved registers across
             calls.
    */
    unsigned int mCallerSavingStackSpace;
    ...
    */
    std::map<
      std::reference_wrapper<WIR_Instruction>, WIR_VirtualRegisterSet,
      WIR_Compare<WIR_Instruction>> mVregsAliveAcrossCall;

    /*!
      @brief mLocalCallerSavedRegsAliveAcrossCall stores temporary information
             about caller-saved registers alive during a call for the current
             round of the graph coloring algorithm.
    */
    std::map<
      std::reference_wrapper<WIR_Instruction>, WIR_RegisterSet,
      WIR_Compare<WIR_Instruction>> mLocalCallerSavedRegsAliveAcrossCall;

    /*!
      @brief mBestLocalCallerSavedRegsAliveAcrossCall temporarily stores map
             mLocalCallerSavedRegsAliveAcrossCall above during Bernstein's
    ...
    */
    ...
    /*!
      @brief mCallerSavedRegsAliveAcrossCall provides a set of caller-saved
             registers that are live-out at a given call instruction and thus
             require caller-saving before and after the call.
    */
    std::map<
      std::reference_wrapper<WIR_Instruction>, WIR_RegisterSet,
    ...
    /*!
      ...
      A detailed explanation of the purpose of such dummy parameters is given
      below for list mDummyParameters. Here, mLocalDummyParameters stores pairs
      of a physical register that is defined/used implicitly by a call, and its
      respective call instruction.
    */
    std::list<
      std::pair<
    ...
    */
    ...
    /*!
      @brief mDummyParameters stores all dummy parameters that need to be
             attached to call instructions.

      These dummy parameters are implicit register parameters of the calls with
      proper def/use attributes modelling reads of argument registers and writes
      of result registers by the call. This way, these dummy parameters help to
      maintain correct liveness information.
    */
    ...
    */
    ...
    /*!
      @brief mPrecolored maps all (virtual and physical) registers pre-assigned
             a color to their pre-colored physical register.
    */
    std::map<
    ...
    */
    std::map<
      WIR_id_t,
      std::list<
        std::list<std::reference_wrapper<WIR_Instruction>>>> mSpillLoads;
    ...
    */
    std::map<
      WIR_id_t,
      std::list<
        std::list<std::reference_wrapper<WIR_Instruction>>>> mSpillStores;
    ...
    */
    std::map<
      ...
      std::list<
        std::list<
          std::reference_wrapper<WIR_Instruction>>>> mRematerializedSpills;
    ...
    */
    ...
    /*!
      @brief mSpillClones stores the IDs of all virtual registers created as
             spill-clones.
    */
    std::set<WIR_id_t> mSpillClones;
  ...
  private:
    ...
    */
    ...
    /*!
      @brief buildReturnMoves counts how many move operations storing a function
             call's result immediately follow a call.

      @param[in] f A const reference to a %WIR function.

      For a %WIR instruction i being a function call, buildReturnMoves stores
      the number of return-related moves in map mFunctionReturnMoves using the
      unique ID of i.

      @author Heiko Falk <Heiko.Falk@tuhh.de>
    */
    void buildReturnMoves( const WIR_Function & );
    ...
    /*!
      ...
      @author Heiko Falk <Heiko.Falk@tuhh.de>
    */
    void makeWorkList( const WIR_Function &, const WIR_InterferenceGraph & );
    ...
    */
    ...
    /*!
      @brief computeCallerSavingStackSpace computes how many additional bytes
             have to be allocated in the currently processed function's stack
             frame to store caller-saved registers across calls.
      ...
      @author Heiko Falk <Heiko.Falk@tuhh.de>
    */
    void computeCallerSavingStackSpace( const WIR_Function & );
    ...
    */
    ...
    std::map<
      std::reference_wrapper<WIR_BasicBlock>, unsigned int,
    ...
    */
    std::map<
      WIR_id_t,
      std::reference_wrapper<WIR_PhysicalRegister>> mBestRegisterMapping;
    ...
    */
    std::map<
      WIR_id_t, std::reference_wrapper<WIR_BaseRegister>> mBestSpillAliases;
    ...
    */
    std::map<
      WIR_id_t, std::reference_wrapper<WIR_PhysicalRegister>> mRegisterMapping;
