...
/*
  checkFlowFact inspects an entry point in depth and verifies that all flow
  fact-related data structures addressing this entry point are in a coherent
  state.
*/
bool WIR_EntryPoint::checkFlowFact( const WIR_System &s ) const
{
  DSTART(
    "virtual bool WIR_EntryPoint::checkFlowFact(const WIR_System&) const" );

  DOUT(
    "Checking entry point " << hex << this << dec << " (ID " << getID() <<
    ")." << endl );

  // Do a few general checks that apply to all kinds of flow facts.
  if ( !isInserted() ) {
    DOUT( "  Entry point is not inserted into some WIR system." << endl );
    return( false );
  }
  ...
  // Do entry point-specific checks.
  DOUT(
    "  mFunctionPointer = " << hex << mFunctionPointer << dec << "." << endl );

  if ( mFunctionPointer ) {
    DOUT(
      "  mFunctionPointer is '" << mFunctionPointer->getName() << "' (ID " <<
      mFunctionPointer->getID() << ")." << endl );
  }
