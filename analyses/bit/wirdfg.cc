...
{
  DSTART(
    "long long int WIR_DFG::getDistance(const WIR_Operation&, const "
    "WIR_Operation&)" );

  if ( !mNodeDistancesComputed.count( o1.getID() ) ) {
    // Solve the single-source shortest path problem.
    vector<long long> d( num_vertices( mDGraph ) );
    dijkstra_shortest_paths(
      mDGraph, mNodeByID.at( o1.getID() ),
      distance_map( &d[ 0 ] ).
    ...
    BGL_FORALL_VERTICES( w, mDGraph, DGraph )
      mDistances[ mNodeByID.at( o1.getID() ) ][ w ] = d.at( w );
    mNodeDistancesComputed.insert( o1.getID() );
  }

  return(
    mDistances.at(
      mNodeByID.at( o1.getID() ) ).at( mNodeByID.at( o2.getID() ) ) );
};
          ...
          if ( p.getType() == WIR_ParameterType::reg ) {
            ...
            if ( rp.isUsed() || rp.isDefUsed() ) {
              ...
              if ( rpContainer.getUDChains().empty() ||
                ...
                postProcessEdge( mDGraph[ e.first ] );
              }
            }
          }
        }
        ...
        for ( WIR_Parameter &p : o ) {
          ...
          if ( p.getType() == WIR_ParameterType::reg ) {
            ...
            if ( rp.isUsed() || rp.isDefUsed() ) {
              ...
              // chains ending in p.
              const WIR_DUUDChain &rpContainer =
                rp.getContainers<WIR_DUUDChain>().begin()->get();
