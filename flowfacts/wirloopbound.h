  ...
  public:
    ...
    */
    ...
    /*!
      ...
      @param[in] _chk A Boolean defaulting to true which denotes whether a
                      sanity check is carried out during construction to ensure
                      that a loop bound definitely refers to a loop basic block.
    ...
    */
    WIR_LoopBound( int, int, const WIR_BasicBlock &,
                   LoopControlType = LoopControlType::unknown, bool = true );
    ...
    */
    ...
    /*!
      @brief checkFlowFact inspects a loop bound in depth and verifies that all
             flow fact-related data structures addressing this loop bound are in
             a coherent state.

      @param[in] s A const reference to the WIR_System to which this loop bound
                   belongs.
