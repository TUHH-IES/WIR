  ...
  mDFG { &d }
{
  ...
  const auto &srcIns = s.getOperation().getInstruction();
  const auto &tgtIns = t.getOperation().getInstruction();
  ...
  mDFG { &d }
{
  DSTART(
    ...
    "WIR_BaseImmediateParameter&, WIR_DFG&)" );
};
