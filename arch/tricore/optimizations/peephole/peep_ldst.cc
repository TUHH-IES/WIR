                                ...
                                const WIR_Operation &st ) const
{
  ...
  if ( absAddressing.count( ld.getOperationFormat() ) ) {
    const auto &ldAbsAdr = ld.getExplicitParameter( 2 );
    const auto &stAbsAdr = st.getExplicitParameter( 1 );
    ...
    if ( ldAbsAdr.getType() == WIR_ParameterType::imm )
      ...
      return(
        sameLabel(
          dynamic_cast<const WIR_LabelParameter &>( ldAbsAdr ),
          dynamic_cast<const WIR_LabelParameter &>( stAbsAdr ) ) );
  }
  ...
  return(
    any_of(
      ...
      [&]( const WIR_Parameter &p ) {
        const WIR_RegisterParameter *ptr =
          dynamic_cast<const WIR_RegisterParameter *>( &p );

        return(
          ptr && ( ptr->isUsed() || ptr->isDefUsed() ) &&
          TC13::isSP( ptr->getRegister() ) ); } ) );
};
...
bool TC_Peep_LDST::accessesVolatileData( const WIR_Operation &o ) const
{
  ...
  if ( o.containsContainers( WIR_DataAccess::getContainerTypeID() ) ) {
    ...
    if ( any_of(
           cont.getData().begin(), cont.getData().end(),
           [&]( const reference_wrapper<WIR_Data> ref ) {
             return( sys.findSymbol( ref.get() ).isVolatile() ); } ) )
      return( true );
  }
