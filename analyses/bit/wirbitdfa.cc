                                                        ...
                                                        const WIR_RegisterParameter &p2 )
{
  ...
  const auto &r1 = p1.getRegister();
  const auto &r2 = p2.getRegister();
...
/*
  updateOperand updates the up/down value associated with a register parameter
  to the bit width required by the parameter's actual register.

  In the case of hierarchical registers where parameter p refers to some child
  register of narrow bit width, it can happen that the associated up/down value
  has a large width according to a parent register. Exactly in this situation,
  updateOperand reduces the up/down value's bit width by a tailored extract
  operation.

  This method is virtual so that it can be overloaded by derived classes.
*/
void WIR_BitDFA::updateOperand( const WIR_Parameter &p, WIR_UpDownValue &v )
{
  DSTART(
    "virtual void WIR_BitDFA::updateOperand(const WIR_Parameter&, "
    "WIR_UpDownValue&)" );
  ...
  if ( p.getType() == WIR_ParameterType::reg ) {
    ...
    if ( r.getBitWidth() < v.getBitWidth() ) {
      // A wide up/down value is used for a tall register which can only happen
...
unsigned int WIR_BitDFA::getHierarchicalRegisterOffset( DGraphEdge e ) const
{
  ...
  const auto &srcP = eInfo.getSourceRegisterParameter();
  const auto &tgtP = eInfo.getTargetRegisterParameter();
...
void WIR_BitDFA::topDownAnalysis( DGraphVertex v, const WIR_Operation &o )
{
  ...
  for ( auto p = out_edges( v, mDGraph ); p.first != p.second; ++p.first ) {
    ...
    // registers.
    eInfo.setDownValue( std::move( res ) );
    ...
    if ( ( p.getType() == WIR_ParameterType::reg ) ||
      ...
      unsigned int width =
        ( p.getType() == WIR_ParameterType::reg ) ?
          ...
          dynamic_cast<const WIR_BaseImmediateParameter &>( p ).getBitWidth();

      bool isUsed =
        ( p.getType() == WIR_ParameterType::imm ) ||
        dynamic_cast<const WIR_RegisterParameter &>( p ).isUsed() ||
        dynamic_cast<const WIR_RegisterParameter &>( p ).isDefUsed();
      bool isDefined =
        ( p.getType() == WIR_ParameterType::reg ) &&
        ( dynamic_cast<const WIR_RegisterParameter &>( p ).isDefined() ||
          dynamic_cast<const WIR_RegisterParameter &>( p ).isDefUsed() );
                                ...
                                bool d ) const
{
  ...
  const auto &v = d ? eInfo.getDownValue() : eInfo.getUpValue();
  ...
  BGL_FORALL_EDGES( e, mDGraph, DGraph ) {
    ...
    if ( eInfo.getType() == WIR_DFGNodeType::op ) {
      ...
      tgtContainer.insertInValues(
        ...
        std::move( eInfo.getDownValue() ), std::move( eInfo.getUpValue() ) );
    } else
