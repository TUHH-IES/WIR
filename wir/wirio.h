...
/*!
  @file wirio.h
  @brief This file provides elementary output stream routines for the %WIR
         library.
...
//
// Type definitions for stream output functions.
//

/*!
  @brief WIR_BasicBlockDumper represents pointers to functions for
         processor-specific stream output of %WIR basic blocks.
*/
using WIR_BasicBlockDumper = void (*)( std::ostream &, const WIR_BasicBlock & );
...
*/
using WIR_BlockLabelDumper = void (*)( std::ostream &, const WIR_BasicBlock & );

/*!
  @brief WIR_CommentDumper represents pointers to functions for
         processor-specific stream output of %WIR comments.
*/
using WIR_CommentDumper = void (*)( std::ostream &, const WIR_Comment & );

/*!
  @brief WIR_CompilationUnitDumper represents pointers to functions for
         processor-specific stream output of %WIR compilation units.
*/
using WIR_CompilationUnitDumper =
  void (*)( std::ostream &, const WIR_CompilationUnit & );

/*!
  @brief WIR_DataDumper represents pointers to functions for processor-specific
         stream output of %WIR data objects.
*/
using WIR_DataDumper = void (*)( std::ostream &, const WIR_Data & );

/*!
  @brief WIR_DataDumper represents pointers to functions for processor-specific
         stream output of %WIR data sections.
*/
using WIR_DataSectionDumper =
  void (*)( std::ostream &, const WIR_Section *,
            const std::list<std::reference_wrapper<WIR_Data>> &, bool );

/*!
  @brief WIR_FileInfoDumper represents pointers to functions for
         processor-specific stream output of %WIR file information.
*/
using WIR_FileInfoDumper = void (*)( std::ostream &, const WIR_FileInfo & );

/*!
  @brief WIR_FunctionDumper represents pointers to functions for
         processor-specific stream output of %WIR functions.
*/
using WIR_FunctionDumper = void (*)( std::ostream &, const WIR_Function & );

/*!
  @brief WIR_InstructionDumper represents pointers to functions for
         processor-specific stream output of %WIR instructions.
*/
using WIR_InstructionDumper =
  void (*)( std::ostream &, const WIR_Instruction & );

/*!
  @brief WIR_OperationDumper represents pointers to functions for
         processor-specific stream output of %WIR operations.
*/
using WIR_OperationDumper = void (*)( std::ostream &, const WIR_Operation & );

/*!
  @brief WIR_AddressingModeParameterDumper represents pointers to functions for
         processor-specific stream output of %WIR addressing mode parameters.
*/
using WIR_AddressingModeParameterDumper =
  void (*)( std::ostream &, const WIR_AddressingModeParameter & );

/*!
  @brief WIR_ConditionFieldParameterDumper represents pointers to functions for
         processor-specific stream output of %WIR condition field parameters.
*/
using WIR_ConditionFieldParameterDumper =
  void (*)( std::ostream &, const WIR_ConditionFieldParameter & );

/*!
  @brief WIR_ImmediateParameterDumper represents pointers to functions for
         processor-specific stream output of %WIR immediate parameters.
*/
using WIR_ImmediateParameterDumper =
  void (*)( std::ostream &, const WIR_BaseImmediateParameter & );

/*!
  @brief WIR_LabelParameterDumper represents pointers to functions for
         processor-specific stream output of %WIR label parameters.
*/
using WIR_LabelParameterDumper =
  void (*)( std::ostream &, const WIR_LabelParameter & );

/*!
  @brief WIR_LdScriptDumper represents pointers to functions for the output of
         linker scripts.
*/
using WIR_LdScriptDumper = void (*)( std::ostream &, const WIR_System & );
...
*/
using WIR_LdScriptSectionDumper =
  bool (*)( std::ostream &, const WIR_Section & );

/*!
  @brief WIR_LdScriptBetweenSectionDumper represents pointers to functions for the
         output of ELF executable exections into linker scripts.
*/
using WIR_LdScriptBetweenSectionDumper =
  void (*)( std::ostream &, const WIR_Section & );

/*!
  @brief WIR_RegisterParameterDumper represents pointers to functions for
         processor-specific stream output of %WIR register parameters.
*/
using WIR_RegisterParameterDumper =
  void (*)( std::ostream &, const WIR_RegisterParameter & );

/*!
  @brief WIR_SystemDumper represents pointers to functions for
         processor-specific stream output of %WIR systems.
*/
using WIR_SystemDumper = void (*)( std::ostream &, const WIR_System & );


//
// Stream index handling.
//
...
/*!
  @brief wir is an output stream manipulator that provides a simple and generic
         %WIR dump.
...
/*!
  ...
  @author Heiko Falk <Heiko.Falk@tuhh.de>
*/
void dumpWIRDataSection( std::ostream &os, const WIR_Section *sec,
...
/*!
  @brief dumpLDScriptBetweenSections dumps a %WIR system's in-between section
         information as linker script to an output stream in a generic,
         processor-independent fashion.
...
*/
void dumpLDScriptBetweenSections( std::ostream &os, const WIR_Section &sec );
