...
WIR_Cache::WIR_Cache( WIR_Cache &&__o ) :
  ...
  mEnabled { std::move( __o.mEnabled ) },
  mShared { std::move( __o.mShared ) },
  mLevel { std::move( __o.mLevel ) },
  mType { std::move( __o.mType ) },
  mAssociativity { std::move( __o.mAssociativity ) },
  mNumberOfSets { std::move( __o.mNumberOfSets ) },
  mSize { std::move( __o.mSize ) },
  mSizeInPercent { __o.mSizeInPercent },
  mLineSize { std::move( __o.mLineSize ) },
  mWriteThrough { std::move( __o.mWriteThrough ) },
  mWriteAllocate { std::move( __o.mWriteAllocate ) },
  mOffsetBits { std::move( __o.mOffsetBits ) },
  mIndexBits { std::move( __o.mIndexBits ) },
  mTagBits { std::move( __o.mTagBits ) },
  mHitDelay { std::move( __o.mHitDelay ) },
  mMissDelay { std::move( __o.mMissDelay ) },
  mBusWidth { std::move( __o.mBusWidth ) },
  mIndexOffsetMask { std::move( __o.mIndexOffsetMask ) },
  mOffsetMask { std::move( __o.mOffsetMask ) },
  mIndexMask { std::move( __o.mIndexMask ) }
{
...
/*
  ...
  Typical values range from 1 (direct mapped) to 8 (set-associative). It is
...
/*
  getLineID extracts a line's ID (tag and index) from the given address.
*/
WIR_Cache::CacheLineID WIR_Cache::getLineID( const WIR_MemoryAddress &a ) const
{
cout << BOOST_CURRENT_FUNCTION << endl;
  DSTART( BOOST_CURRENT_FUNCTION );
...
/*
  getAffectedLineIDs determines the range of cache line IDs that may be affected
  by the given memory access.
*/
WIR_Interval<WIR_Cache::CacheLineID> WIR_Cache::getAffectedLineIDs( const WIR_AddressRange &a ) const
{
cout << BOOST_CURRENT_FUNCTION << endl;
  DSTART( BOOST_CURRENT_FUNCTION );

  if ( !mEnabled )
    return( WIR_Interval<CacheLineID>() );

  return(
    WIR_Interval<WIR_Cache::CacheLineID>(
      getLineID( a.getLower() ), getLineID( a.getUpper() ) ) );
};
...
/*
  ...
  Typical associativity values range from 1 (direct mapped) to 8
  (set-associative).
...
WIR_Cache::WIR_Cache( std::string &&n, unsigned int s, unsigned int a,
                      unsigned int l ) :
  WIR_SystemComponent { std::move( n ) },
  mEnabled { true },
  mShared { false },
  mLevel { CacheLevel::NOTSET },
  mAssociativity { a },
