...
Version 3.0
---------------------------------------------------------------*/
#ifndef WIR_SAFEINT_HPP
#define WIR_SAFEINT_HPP
...
// Needed for std::numeric_limits derivation
#include <limits>
#include <type_traits>

// These two may not be defined, either.
// g++ will error if they do already exist, as redefinition of a typedef is
// not allowed
...
static_assert( ( ( uint32_t ) -1 ) == ( ( uint32_t ) 0xffffffff ),
    "two's complement negative numbers assertion" );

namespace WIR {

/*!
  @brief The namespace WIR::THIRDRDPARTY groups all code that is used within
         WIR but that comes from external sources under dedicated licenses.
  @author Heiko Falk <Heiko.Falk@tuhh.de>
*/
namespace THIRDRDPARTY {
...
*
* inline bool SafeCast( const T From, U& To ) noexcept
* inline bool SafeEquals( const T t, const U u ) noexcept
* inline bool SafeNotEquals( const T t, const U u ) noexcept
* inline bool SafeGreaterThan( const T t, const U u ) noexcept
* inline bool SafeGreaterThanEquals( const T t, const U u ) noexcept
* inline bool SafeLessThan( const T t, const U u ) noexcept
* inline bool SafeLessThanEquals( const T t, const U u ) noexcept
* inline bool SafeModulus( const T& t, const U& u, T& result ) noexcept
* inline bool SafeMultiply( T t, U u, T& result ) noexcept
* inline bool SafeDivide( T t, U u, T& result ) noexcept
* inline bool SafeAdd( T t, U u, T& result ) noexcept
* inline bool SafeSubtract( T t, U u, T& result ) noexcept
*
*/
...
public:
    ...
    explicit SafeIntException( SafeIntError code )
    {
        m_code = code;
    }
    SafeIntError m_code;
...
{
public:
    static_assert( NumericType<T>::isInt, "numeric Type not int" );
    enum
    {
        ...
        bitCount  = ( sizeof(T)*8 ),
        isBool    = std::is_same<T,bool>::value
    };

    // On version 13.10 enums cannot define __int64 values
    // so we'll use const statics instead!
    const static T maxInt;
    const static T minInt;
};

template < typename T >
const T IntTraits< T >::maxInt = std::numeric_limits<T>::max();
template < typename T >
const T IntTraits< T >::minInt = std::numeric_limits<T>::min();
...
//! A template for doing a less-comparison with zero
/*!
 * We specialize this template to avoid compiler warnings.
 */
template<typename T>
...
//! A template for doing a greater-equal-comparison with zero
/*!
 * We specialize this template to avoid compiler warnings.
 */
template<typename T>
typename std::enable_if<std::is_signed<T>::value, bool>::type
isPositive( const T x ) {
  return x >= 0;
}
template<typename T>
typename std::enable_if<std::is_unsigned<T>::value, bool>::type
...
//! A template for doing an equal-comparison with -1
/*!
 * We specialize this template to avoid compiler warnings.
 */
template<typename T>
typename std::enable_if<std::is_signed<T>::value, bool>::type
isMinusOne( const T x ) {
  return x == -1;
}
template<typename T>
typename std::enable_if<std::is_unsigned<T>::value, bool>::type
isMinusOne( const T x ) {
  // Unsigned numbers can still be value-equal to -1
...
{
public:
    static unsigned __int32 Abs( T t ) noexcept
    {
        assert( isNegative( t ) );
...
{
public:
    static unsigned __int64 Abs( T t ) noexcept
    {
        assert( isNegative( t ) );
...
public:
  ...
  static bool Negative( T t, T& ret ) noexcept
  {
        // corner case
        if( t != IntTraits< T >::minInt )
        {
...
{
public:
    static bool Cast( bool b, T& t ) noexcept
    {
        t = (T)( b ? 1 : 0 );
        return true;
    }
...
{
public:
    static bool Cast( T t, bool& b ) noexcept
    {
       b = !!t;
       return true;
    }
...
public:
    static bool Cast( U u, T& t ) noexcept
    {
        if( u > static_cast<U>( IntTraits< T >::maxInt ) )
            return false;
...
public:
    static bool Cast( U u, T& t ) noexcept
    {
        ...
        if( u > static_cast<U>( IntTraits< T >::maxInt ) || isNegative( u ) )
            return false;
...
public:
    static bool Cast( U u, T& t ) noexcept
    {
        // T, U are signed
        if( u > static_cast<U>( IntTraits< T >::maxInt ) ||
            u < static_cast<U>( IntTraits< T >::minInt ) )
            return false;
...
{
public:
    static bool IsEquals( const T t, const U u ) noexcept { return ( t == u ); }
};

template < typename T, typename U > class EqualityTest< T, U, ComparisonMethod_CastInt >
{
public:
    static bool IsEquals( const T t, const U u ) noexcept { return ( (int)t == (int)u ); }
};

template < typename T, typename U > class EqualityTest< T, U, ComparisonMethod_CastInt64 >
{
public:
    static bool IsEquals( const T t, const U u ) noexcept { return ( (__int64)t == (__int64)u ); }
};

template < typename T, typename U > class EqualityTest< T, U, ComparisonMethod_UnsignedT >
{
public:
    static bool IsEquals( const T t, const U u ) noexcept
    {
...
{
public:
    static bool GreaterThan( const T t, const U u ) noexcept { return ( t > u ); }
};

template < typename T, typename U > class GreaterThanTest< T, U, ComparisonMethod_CastInt >
{
public:
    static bool GreaterThan( const T t, const U u ) noexcept { return ( (int)t > (int)u ); }
};

template < typename T, typename U > class GreaterThanTest< T, U, ComparisonMethod_CastInt64 >
{
public:
    static bool GreaterThan( const T t, const U u ) noexcept { return ( (__int64)t > (__int64)u ); }
};

template < typename T, typename U > class GreaterThanTest< T, U, ComparisonMethod_UnsignedT >
{
public:
    static bool GreaterThan( const T t, const U u ) noexcept
    {
        // one operand is 32 or 64-bit unsigned, and the other is signed and the same size or smaller
        if( isNegative( u ) )
            return true;
...
{
public:
    static SafeIntError Modulus( const T& t, const U& u, T& result ) noexcept
    {
        if(u == 0)
...
public:
    ...
    static bool Multiply( const T& t, const U& u, T& ret ) noexcept
    {
        int tmp = t * u;
...
inline bool IntrinsicMultiplyInt64( const signed __int64& a, const signed __int64& b, signed __int64* pRet )
{
  ...
  *pRet = _mul128(a , b, &llHigh);
  ...
  if( isNegative(a ^ b) )
  {
    ...
    if( isMinusOne( llHigh ) && isNegative( *pRet ) ||
      llHigh == 0 && *pRet == 0 )
    {
...
{
public:
    static bool RegMultiply( const unsigned __int64& a, const unsigned __int64& b, unsigned __int64* pRet ) noexcept
    {
#if SAFEINT_USE_INTRINSICS
...
{
public:
    static bool RegMultiply( const unsigned __int64& a, unsigned __int32 b, unsigned __int64* pRet ) noexcept
    {
#if SAFEINT_USE_INTRINSICS
...
public:
  // Intrinsic not needed
    static bool RegMultiply( const unsigned __int64& a, signed __int32 b, unsigned __int64* pRet ) noexcept
    {
        if( isNegative( b ) && a != 0 )
            return false;
...
{
public:
    static bool RegMultiply( const unsigned __int64& a, signed __int64 b, unsigned __int64* pRet ) noexcept
    {
        if( isNegative( b ) && a != 0 )
            return false;
...
public:
  // Devolves into ordinary 64-bit calculation
    static bool RegMultiply( signed __int32 a, const unsigned __int64& b, signed __int32* pRet ) noexcept
    {
...
public:
  // Becomes ordinary 64-bit multiplication, intrinsic not needed
    static bool RegMultiply( unsigned __int32 a, const unsigned __int64& b, unsigned __int32* pRet ) noexcept
    {
        // Consider that a*b can be broken up into:
        // (bHigh * 2^32 + bLow) * a
        // => (bHigh * a * 2^32) + (bLow * a)
...
{
public:
    static bool RegMultiply( unsigned __int32 a, const signed __int64& b, unsigned __int32* pRet ) noexcept
    {
        if( isNegative( b ) && a != 0 )
            return false;
...
{
public:
    static bool RegMultiply( const signed __int64& a, const signed __int64& b, signed __int64* pRet ) noexcept
    {
#if SAFEINT_USE_INTRINSICS
...
{
public:
    static bool RegMultiply( const signed __int64& a, unsigned __int32 b, signed __int64* pRet ) noexcept
    {
#if SAFEINT_USE_INTRINSICS
...
{
public:
    static bool RegMultiply( const signed __int64& a, signed __int32 b, signed __int64* pRet ) noexcept
    {
#if SAFEINT_USE_INTRINSICS
    return IntrinsicMultiplyInt64( a, (signed __int64)b, pRet );
#else
...
{
public:
    static bool RegMultiply( signed __int32 a, const signed __int64& b, signed __int32* pRet ) noexcept
    {
#if SAFEINT_USE_INTRINSICS
    __int64 tmp;
...
public:
  // Leave this one as-is - will call unsigned intrinsic internally
    static bool RegMultiply( const signed __int64& a, const unsigned __int64& b, signed __int64* pRet ) noexcept
    {
    bool aNegative = false;
...
public:
   // T, U are unsigned __int64
    static bool Multiply( const T& t, const U& u, T& ret ) noexcept
    {
        static_assert( IntTraits<T>::isUint64 && IntTraits<U>::isUint64, "not uint64" );
...
public:
    ...
    static bool Multiply( const T& t, const U& u, T& ret ) noexcept
    {
        static_assert( IntTraits<T>::isUint64, "not uint64" );
        unsigned __int64 t1 = t;
...
public:
    ...
    static bool Multiply(const T& t, const U& u, T& ret) noexcept
    {
        static_assert( IntTraits<U>::isUint64, "not unit64" );
        unsigned __int64 u1 = u;
        unsigned __int32 tmp;
    ...
    static void MultiplyThrow(const T& t, const U& u, T& ret)
    {
        static_assert( IntTraits<U>::isUint64, "not uinit64" );
        unsigned __int64 u1 = u;
        unsigned __int32 tmp;
...
public:
    ...
    static bool Multiply(const T& t, const U& u, T& ret) noexcept
    {
        static_assert( IntTraits<T>::isUint64 && IntTraits<U>::isInt64, "not int64" );
        unsigned __int64 t1 = t;
        __int64          u1 = u;
...
public:
    ...
    static bool Multiply(const T& t, const U& u, T& ret) noexcept
    {
        static_assert( IntTraits<U>::isInt64, "not int64" );
        __int64          u1 = u;
        unsigned __int32 tmp;
...
public:
    ...
    static bool Multiply( const T& t, const U& u, T& ret ) noexcept
    {
        static_assert( IntTraits<T>::isInt64, "not int64" );
        __int64          t1 = t;
...
public:
    ...
    static bool Multiply( const T& t, const U& u, T& ret ) noexcept
    {
        static_assert( IntTraits<T>::isInt64 && IntTraits<U>::isInt64, "not int64" );
        __int64          t1 = t;
        __int64          u1 = u;
...
public:
    ...
    static bool Multiply( const T& t, U u, T& ret ) noexcept
    {
        static_assert( IntTraits<T>::isInt64, "not int64" );
        __int64          t1 = t;
...
public:
    ...
    // U is unsigned __int64
    static bool Multiply(T t, const U& u, T& ret) noexcept
    {
        static_assert( IntTraits<U>::isUint64, "not uint64" );
        unsigned __int64 u1 = u;
        __int32 tmp;
...
public:
    ...
    static bool Multiply( const T& t, const U& u, T& ret ) noexcept
    {
        static_assert( IntTraits<T>::isInt64 && IntTraits<U>::isUint64, "not uint64" );
        __int64          t1 = t;
        unsigned __int64 u1 = u;
...
{
public:
    static SafeIntError Divide( const T& t, const U& u, T& result ) noexcept
    {
       if( u == 0 )
            return SafeIntDivideByZero;
...
{
public:
    static SafeIntError Divide( const T& t, const unsigned __int64& u, T& result ) noexcept
    {
        static_assert( IntTraits< U >::isUint64, "not uint64" );
...
public:
    static SafeIntError Divide( const T& t, const U& u, T& result ) noexcept
    {
        ...
        if( t == IntTraits< T >::minInt && isMinusOne( u ) )
            return SafeIntArithmeticOverflow;
...
{
public:
    static bool Addition( const T& lhs, const U& rhs, T& result ) noexcept
    {
...
public:
    static bool Addition( const T& lhs, const U& rhs, T& result ) noexcept
    {
        ...
        if( isPositive( lhs ) )
        {
            ...
            if( isPositive( rhs ) && tmp < lhs )
                return false;
        }
        else
        {
            // lhs negative
            if( isNegative( rhs ) && tmp > lhs )
                return false;
        }
...
{
public:
    static bool Addition( const __int64& lhs, const unsigned __int64& rhs, __int64& result ) noexcept
    {
        static_assert( IntTraits< T >::isInt64 && IntTraits< U >::isUint64, "not uint64" );
...
{
public:
    static bool Subtract( const T& lhs, const U& rhs, T& result ) noexcept
    {
...
{
public:
    static bool Subtract( const T& lhs, const U& rhs, U& result ) noexcept
    {
        // both are unsigned - easy case
...
{
public:
    static bool Subtract( const U& lhs, const T& rhs, T& result ) noexcept
    {
        // both values are 16-bit or less
        // rhs is signed, so could end up increasing or decreasing
        __int32 tmp = lhs - rhs;
...
public:
    static bool Subtract( const T& lhs, const U& rhs, T& result ) noexcept
    {
        ...
        if( ( isPositive( lhs ) && isNegative( rhs ) && tmp < lhs ) || // condition 2
            ( isPositive( rhs ) && tmp > lhs ) )             // condition 3
        {
             return false;
        }
...
public:
    static bool Subtract( const U& lhs, const T& rhs, T& result ) noexcept
    {
        ...
        if( ( isPositive( lhs ) && isNegative( rhs ) && tmp < lhs ) ||
            ( rhs > 0 && tmp > lhs ) )
        {
            return false;
            //else OK
...
{
public:
    static bool Subtract( const __int64& lhs, const unsigned __int64& rhs, __int64& result ) noexcept
    {
        static_assert( IntTraits< T >::isInt64 && IntTraits< U >::isUint64, "not uint64" );
...
public:
   ...
   // get smaller. If rhs > lhs, then it would also go negative, which is the other case
    static bool Subtract( const __int64& lhs, const unsigned __int64& rhs, T& result ) noexcept
    {
        static_assert( IntTraits< T >::isUint64 && IntTraits< U >::isInt64, "not uint64" );
     if( isPositive( lhs ) && (unsigned __int64)lhs >= rhs )
     {
...
public:
    SafeInt() noexcept
    {
        static_assert( NumericType< T >::isInt, "not int" );
        m_int = 0;
    }
    ...
    explicit SafeInt( const T& i ) noexcept
    {
        static_assert( NumericType< T >::isInt, "not int" );
        //always safe
        m_int = i;
    ...
    explicit SafeInt( bool b ) noexcept
    {
        static_assert( NumericType< T >::isInt, "not int" );
        m_int = (T)( b ? 1 : 0 );
    }
    ...
    template < typename U,
               class = typename std::enable_if<std::is_convertible<U,T>::value,T>::type>
    ...
    // Copy constructor.
    SafeInt( const SafeInt<T, E> &rhs ) noexcept :
      m_int { rhs.m_int }
    {};
    ...
    SafeInt< T, E >& operator =( const T& rhs ) noexcept
    {
        m_int = rhs;
        return *this;
    }
    ...
    SafeInt< T, E >& operator =( const SafeInt< T, E >& rhs ) noexcept
    {
        m_int = rhs.m_int;
        return *this;
    }
    ...
    T* Ptr() noexcept { return &m_int; }
    const T* Ptr() const noexcept { return &m_int; }
    const T& Ref() const noexcept { return m_int; }
    ...
    T* operator &() noexcept { return &m_int; }
    const T* operator &() const noexcept { return &m_int; }

    // Unary operators
    bool operator !() const noexcept { return (!m_int) ? true : false; }

    // operator + (unary)
    // note - normally, the '+' and '-' operators will upcast to a signed int
    // for T < 32 bits. This class changes behavior to preserve type
    const SafeInt< T, E >& operator +() const noexcept { return *this; };
    ...
    SafeInt< T, E > operator ~() const noexcept { return SafeInt< T, E >( (T)~m_int ); }
    ...
    bool operator <( SafeInt< T, E > rhs ) const noexcept
    {
        return m_int < (T)rhs;
    }
    ...
    bool operator ==( SafeInt< T, E > rhs ) const noexcept { return m_int == (T)rhs; }
   ...
   #define ShiftAssert(x)
    ...
    template < typename U >
    SafeInt< T, E > operator <<( U bits ) const noexcept
    {
        ShiftAssert( !IntTraits< U >::isSigned || isPositive( bits ) );
    ...
    template < typename U >
    SafeInt< T, E > operator <<( SafeInt< U, E > bits ) const noexcept
    {
        ShiftAssert( !IntTraits< U >::isSigned || isPositive( (U)bits ) );
    ...
    template < typename U >
    SafeInt< T, E >& operator <<=( U bits ) noexcept
    {
        ShiftAssert( !IntTraits< U >::isSigned || isPositive( bits ) );
        ShiftAssert( bits < (int)IntTraits< T >::bitCount );
    ...
    template < typename U >
    SafeInt< T, E >& operator <<=( SafeInt< U, E > bits ) noexcept
    {
        ShiftAssert( !IntTraits< U >::isSigned || isPositive( (U)bits ) );
        ShiftAssert( (U)bits < (int)IntTraits< T >::bitCount );
    ...
    // Bitwise &
    SafeInt< T, E > operator &( SafeInt< T, E > rhs ) const noexcept
    {
        return SafeInt< T, E >( m_int & (T)rhs );
    }

    template < typename U >
    SafeInt< T, E > operator &( U rhs ) const noexcept
    {
    ...
    // Bitwise & assignment
    SafeInt< T, E >& operator &=( SafeInt< T, E > rhs ) noexcept
    {
        m_int &= (T)rhs;
        return *this;
    }
    ...
    template < typename U >
    SafeInt< T, E >& operator &=( U rhs ) noexcept
    {
    ...
    template < typename U >
    SafeInt< T, E >& operator &=( SafeInt< U, E > rhs ) noexcept
    {
    ...
    SafeInt< T, E > Minimum( SafeInt< T, E > test, const T floor = IntTraits< T >::minInt ) const noexcept
    {
        T tmp = test < m_int ? (T)test : m_int;
        return tmp < floor ? floor : tmp;
    }

    SafeInt< T, E > Maximum( SafeInt< T, E > test, const T upper = IntTraits< T >::maxInt ) const noexcept
    {
        T tmp = test > m_int ? (T)test : m_int;
        return tmp > upper ? upper : tmp;
    }

    void Swap( SafeInt< T, E >& with ) noexcept
    {
        T temp( m_int );
        m_int = with.m_int;
        with.m_int = temp;
    ...
    const SafeInt< T, E >& Align()
    {
        ...
        ShiftAssert( ( ( IntTraits<T>::isSigned && bits < (int)IntTraits< T >::bitCount - 1 )
                    ...
                    isPositive( bits ) && ( !IntTraits<T>::isSigned || m_int > 0 ) );
...
template < typename T, typename U, typename E,
           typename = typename std::enable_if<std::is_convertible<U,T>::value,T>::type >
bool operator <( U lhs, SafeInt< T, E > rhs ) noexcept
{
    return GreaterThanTest< T, U, ValidComparison< T, U >::method >::GreaterThan( (T)rhs, lhs );
}

template < typename T, typename U, typename E >
bool operator <( SafeInt< U, E > lhs, SafeInt< T, E > rhs ) noexcept
{
...
template < typename T, typename U, typename E >
bool operator >( SafeInt< T, E > lhs, SafeInt< U, E > rhs ) noexcept
{
...
bool operator ==( bool lhs, SafeInt< T, E > rhs ) noexcept
{
...
public:
   ...
   {
        if( lhs == IntTraits< U >::minInt && isMinusOne( (T)rhs ) )
        {
...
// Division
template < typename T, typename U, typename E,
           typename = typename std::enable_if<std::is_convertible<U,T>::value,T>::type > SafeInt< T, E > operator /( U lhs, SafeInt< T, E > rhs )
{
...
template < typename T, typename U, typename E,
           typename = typename std::enable_if<std::is_integral<T>::value,T>::type >
T& operator &=( T& lhs, SafeInt< U, E > rhs ) noexcept
{
...
template < typename T, typename U, typename E,
           typename = typename std::enable_if<std::is_convertible<U,T>::value,T>::type >
SafeInt< U, E > operator <<( U lhs, SafeInt< T, E > bits ) noexcept
{
    ShiftAssert( !IntTraits< T >::isSigned || isPositive( (T)bits ) );
...
template < typename T, typename U, typename E,
           typename = typename std::enable_if<std::is_convertible<U,T>::value,T>::type >
SafeInt< T, E > operator &( U lhs, SafeInt< T, E > rhs ) noexcept
{
...
template < typename T, typename E >
std::ostream &operator<<( std::ostream &out, const SafeInt< T, E > &i )
{
  return out << i.Ref();
}
 ...
 * for SafeInt, to make it usable anywhere a built-in integer may be used. */
namespace std {

  template<typename T>
  struct numeric_limits<WIR::THIRDRDPARTY::SafeInt<T>> : public numeric_limits<T>
  {
      typedef numeric_limits<T> base_type;
      typedef WIR::THIRDRDPARTY::SafeInt<T> sat_type;
  ...
  template<typename T>
  struct is_signed<WIR::THIRDRDPARTY::SafeInt<T>> : public is_signed<T> {};
  template<typename T>
  struct is_unsigned<WIR::THIRDRDPARTY::SafeInt<T>> : public is_unsigned<T> {};

  template<typename T>
  struct make_signed<WIR::THIRDRDPARTY::SafeInt<T>> : public make_signed<T>
    { typedef WIR::THIRDRDPARTY::SafeInt<typename make_signed<T>::type> type; };
  template<typename T>
  struct make_unsigned<WIR::THIRDRDPARTY::SafeInt<T>> : public make_unsigned<T>
    { typedef WIR::THIRDRDPARTY::SafeInt<typename make_unsigned<T>::type> type; };

  template<typename T>
  typename std::enable_if<std::is_signed<T>::value,
                          WIR::THIRDRDPARTY::SafeInt<T>>::type
  abs( const WIR::THIRDRDPARTY::SafeInt<T>& i ) {
    return WIR::THIRDRDPARTY::SafeInt<T>( std::abs( i.Ref() ) );
  }
  template<typename T>
