  ...
  public:
    ...
    /*!
      ...
      -# If n already is a path (either absolute or relative), i.e., n contains
         at least one '/' somewhere, this input path n is directly used. Note
         that it lies in the sole responsibility of the caller of this
         constructor that n is a valid path in this case!
      -# Otherwise, the constructor probes the WIR-internal directory structure
         (archdir and sysconfdir) and checks whether a file named n exists
    ...
    */
    ...
    /*!
      @brief getDataLayout computes a map representing a system's data layout.

      @return A map that, for each %WIR section, groups together all data
              objects of a system which will be linked into the associated
              section.

      @author Heiko Falk <Heiko.Falk@tuhh.de>
    */
    WIR_DataLayoutMap getDataLayout( void ) const;
    ...
    */
    ...
    /*!
      @brief checkFlowFacts inspects the flow fact-related data structures of a
             %WIR system and verifies that they are all in a coherent state.
      ...
      @author Heiko Falk <Heiko.Falk@tuhh.de>
    */
    bool checkFlowFacts( void ) const;
