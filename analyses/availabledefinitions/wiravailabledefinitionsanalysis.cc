...
void WIR_AvailableDefinitionsAnalysis::runAnalysis( WIR_Function &f )
{
  ...
  do {
    ...
    for ( const WIR_BasicBlock &b : postOrderBBs )
      if ( mUpdateBlock[ b.getID() ] ) {
        ...
        const auto &blockKill = mBlockKill.at( b.getID() );
        ...
        for ( const WIR_BasicBlock &pred : mPredecessors[ b.getID() ] ) {
          ...
          if ( firstPredecessor )
            ...
            //   mBlockAvailableDefsIn[ b ] \cap mBlockAvailableDefsOut[ pred ]
...
void WIR_AvailableDefinitionsAnalysis::init( WIR_Function &f )
{
  ...
  for ( const WIR_VirtualRegister &vr : f.getVirtualRegisters() )
    for ( WIR_VirtualRegister &leaf : vr.getLeafs() )
      if ( leaf.isPrecolored() ) {
        const WIR_PhysicalRegister &pr = leaf.getPrecolor();
  ...
  for ( const WIR_PhysicalRegister &p : f.getFunctionInputs() )
  ...
  for ( const WIR_PhysicalRegister &p : inputRegs )
    ...
    if ( r.isVirtual() )
      for ( WIR_VirtualRegister &leaf :
        ...
        if ( !mVirtualRegistersOnly && leaf.isPrecolored() ) {
          ...
          WIR_BitVector diff1 = defs.at( pr.getID() );
          diff1.set_difference( instrGen );
          instrKill.set_union( diff1 );
          ...
          if ( it != precolRMap.end() )
            for ( const WIR_VirtualRegister &vr : it->second ) {
              WIR_BitVector diff2 = defs.at( vr.getID() );
              diff2.set_difference( instrGen );
              instrKill.set_union( diff2 );
            }
        }
      }
    else
      for ( WIR_PhysicalRegister &leaf :
        ...
        if ( it != precolRMap.end() )
          for ( const WIR_VirtualRegister &vr : it->second ) {
            WIR_BitVector diff1 = defs.at( vr.getID() );
            diff1.set_difference( instrGen );
            instrKill.set_union( diff1 );
          }
      }
  ...
  DACTION(
    for ( const WIR_BasicBlock &b : f ) {
      ...
      for ( const WIR_Instruction &i : b ) {
        ...
        const WIR_BitVector &instrGen = mInstrGen.at( i.getID() );
        ...
        const WIR_BitVector &instrKill = mInstrKill.at( i.getID() );
...
void WIR_AvailableDefinitionsAnalysis::propagateIns2BB( WIR_Function &f )
{
  ...
  DACTION(
    for ( const WIR_BasicBlock &b : f ) {
      ...
      const WIR_BitVector &blockKill = mBlockKill.at( b.getID() );
