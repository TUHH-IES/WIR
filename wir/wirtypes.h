...
//! WIR_ControlTreeEdge represents edges between control tree nodes.
using WIR_ControlTreeEdge =
  std::pair<std::reference_wrapper<WIR_ControlTreeNode>,
            std::reference_wrapper<WIR_ControlTreeNode>>;
...
/*!
  @brief This enum represents different types of %WIR opcodes.

  @author Heiko Falk <Heiko.Falk@tuhh.de>
*/
enum class WIR_OpCodeType : char
{
  //! An opcode that reads from memory.
  load,

  //! An opcode that writes to memory.
  store,

  /*!
    @brief An opcode that implicitly accesses memory without being an explicit
           load or store.
  */
  implicitMemoryAccess,

  //! An opcode that is a register-register move.
  move,

  //! An opcode that calls a function.
  call,

  //! An opcode that indirectly calls a function.
  indirectCall,

  //! An opcode that returns from a function call.
  ret,

  //! An opcode that conditionally branches.
  condJump,

  //! An opcode that unconditionally branches.
  uncondJump,

  //! An opcode that indirectly branches.
  indirectJump,

  //! A pseudo-opcode standing for an assembly data directive.
  asmDirective,

  //! An opcode that has side effects.
  sideEffect
};
...
/*!
  @brief WIR_Compare_Pair is a comparator class that is used to sort sets of
         pairs of %WIR classes uniquely.

  @author Heiko Falk <Heiko.Falk@tuhh.de>
*/
struct WIR_Compare_Pair
{
  bool operator()( const std::pair<std::reference_wrapper<WIR_ID_API>,
...
*/
using WIR_BasicBlockPairSet = std::set<WIR_BasicBlockPair, WIR_Compare_Pair>;
...
/*!
  @brief WIR_ControlTreeEdge represents edges between control tree nodes.
*/
using WIR_ControlTreeEdge =
  std::pair<std::reference_wrapper<WIR_ControlTreeNode>,
            std::reference_wrapper<WIR_ControlTreeNode>>;
...
/*!
  @brief WIR_ControlTreeEdgeSet represents sets of edges between control tree
         nodes.
*/
using WIR_ControlTreeEdgeSet = std::set<WIR_ControlTreeEdge, WIR_Compare_Pair>;
...
/*!
  @brief WIR_DataLayoutMap maps %WIR sections to all data objects that will be
         linked into a section.
*/
using WIR_DataLayoutMap =
  std::map<std::reference_wrapper<WIR_Section>,
           std::list<std::reference_wrapper<WIR_Data>>,
           WIR_Compare<WIR_Section>>;
