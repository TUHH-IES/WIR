...
// function.
map<unsigned int,
    WIR_CompilationUnitDumper> WIR_Registry::mCompilationUnitDumperMap;
...
map<unsigned int,
    WIR_AddressingModeParameterDumper> WIR_Registry::mAddressingModeParameterDumperMap;

// mConditionFieldParameterDumperMap maps the numerical ID of a processor
// architecture to a function pointer for a processor-specific
// WIR_ConditionFieldParameter I/O function.
map<unsigned int,
    WIR_ConditionFieldParameterDumper> WIR_Registry::mConditionFieldParameterDumperMap;

// mImmediateParameterDumperMap maps the numerical ID of a processor
// architecture to a function pointer for a processor-specific
// WIR_BaseImmediateParameter I/O function.
map<unsigned int,
    WIR_ImmediateParameterDumper> WIR_Registry::mImmediateParameterDumperMap;
...
map<unsigned int,
    WIR_LabelParameterDumper> WIR_Registry::mLabelParameterDumperMap;
...
// function.
map<unsigned int,
    WIR_LdScriptSectionDumper> WIR_Registry::mLdScriptSectionDumperMap;

// mLdScriptBetweenSectionDumperMap maps the numerical ID of a processor architecture
// to a function pointer for a processor-specific in-between section linker script I/O
// function.
map<unsigned int,
    WIR_LdScriptBetweenSectionDumper> WIR_Registry::mLdScriptBetweenSectionDumperMap;
...
// function.
map<unsigned int,
    WIR_RegisterParameterDumper> WIR_Registry::mRegisterParameterDumperMap;
                                            ...
                                            WIR_OperationFormat &&f )
{
  ...
  mOperationFormatMap[ id.mID ] = std::move( f );
};
...
/*
  getLdScriptDumper returns a pointer to a processor-specific linker script I/O
  function for the given processor architecture ID.

  If some unknown/unregistered processor ID is passed to getLdScriptDumper, the
  function pointer registered for the default ID 0 is returned.
*/
...
{
  DSTART(
    "static WIR_LdScriptSectionDumper WIR_Registry::getLdScriptSectionDumper("
    "unsigned int)" );
...
/*
  registerLdScriptBetweenSectionDumper registers a function pointer for a
  processor-specific in-between section linker script I/O function under the
  numerical ID of some processor architecture.

  registerLdScriptBetweenSectionDumper shall be called by the init methods of
  class WIR_Processor in order to setup processor-specific I/O routines.
*/
void WIR_Registry::registerLdScriptBetweenSectionDumper( unsigned int id,
                                                         WIR_LdScriptBetweenSectionDumper f )
{
  DSTART(
    "static void WIR_Registry::registerLdScriptBetweenSectionDumper("
    "unsigned int, WIR_LdScriptBetweenSectionDumper)" );

  ufAssert(
    mLdScriptBetweenSectionDumperMap.find( id ) ==
      mLdScriptBetweenSectionDumperMap.end() );

  mLdScriptBetweenSectionDumperMap[ id ] = f;
};


/*
  getLdScriptBetweenSectionDumper returns a pointer to a processor-specific
  in-between section linker script I/O function for the given processor
  architecture ID.

  If some unknown/unregistered processor ID is passed to
  getLdScriptBetweenSectionDumper, the function pointer registered for the
  default ID 0 is returned.
*/
WIR_LdScriptBetweenSectionDumper WIR_Registry::getLdScriptBetweenSectionDumper( unsigned int id )
{
  DSTART(
    "static WIR_LdScriptBetweenSectionDumper "
    "WIR_Registry::getLdScriptBetweenSectionDumper(unsigned int)" );

  auto it = mLdScriptBetweenSectionDumperMap.find( id );

  if ( it == mLdScriptBetweenSectionDumperMap.end() )
    return( mLdScriptBetweenSectionDumperMap[ 0 ] );
  else
    return( (*it).second );
};
