...
{
  DSTART( "virtual WIR_HierarchicalCFG::~WIR_HierarchicalCFG()" );
};
      ...
      for ( auto rIt = b.getInstructions().rbegin();
        ...
        for ( WIR_Operation &o : rIt->get() ) {
          if ( o.isJump() &&
            ...
            auto jumpTargets = o.getJumpTargets();
            ...
            if ( c.explicitSuccessorIsExit() ) {
              ...
              if ( any_of(
                     jumpTargets.begin(), jumpTargets.end(),
    ...
    do {
      ...
      if ( ( n->getType() == WIR_CTNodeType::bb ) &&
        ...
        transform(
          ...
          [&]( std::reference_wrapper<WIR_ControlTreeNode> ref ) {
            return( &(ref.get()) ); } );
  ...
  for ( WIR_BasicBlock &b : mFunction ) {
    for ( WIR_BasicBlock &succ : b.getSuccessors() ) {
      ...
      if ( commonParent && commonParent->isCyclic() ) {
        ...
        if ( any_of(
               commonParent->getBackEdges().begin(),
               commonParent->getBackEdges().end(),
               [&]( const WIR_ControlTreeEdge &e ) {
                 return( ( e.first.get() == *c1 ) &&
                         ( e.second.get() == *c2 ) ); } ) ) {
          DOUT( "  Identified back edge (1)." << endl );
          isBack = true;
        }
      } else {
        ...
        if ( p.second && mCGraph[ p.first ].isBackEdge ) {
          ...
          DOUT( "  Identified back edge (2)." << endl );
          isBack = true;
        }
      }

      unsigned int numSpecialProperties = 0;
      if ( isLoopExit )
      ...
      if ( numSpecialProperties ) {
        string fraction =
          ( numSpecialProperties == 1 ) ? "" :
            ( numSpecialProperties == 2 ) ? ";0.5" : ";0.33";
        unsigned int printedColors = 0;

        dotFile << ",style=bold,color=\"";

        if ( isLoopExit ) {
          dotFile << "orange" << fraction;
          ++printedColors;
        }
        ...
        if ( isBreak ) {
          if ( printedColors != 0 )
            dotFile << ":";
          dotFile << "red";
          if ( printedColors != numSpecialProperties - 1 )
            dotFile << fraction;
          ++printedColors;
        }
...
{
  DSTART(
    "void WIR_HierarchicalCFG::CFGVisitor::start_vertex("
    "WIR_HierarchicalCFG::CGraphVertex, const CGraph&) const" );
...
{
  DSTART(
    "void WIR_HierarchicalCFG::CFGVisitor::finish_vertex("
    "WIR_HierarchicalCFG::CGraphVertex, const CGraph&) const" );
                                                    ...
                                                    const CGraph &g ) const
{
  ...
  DSTART(
    "void WIR_HierarchicalCFG::CFGVisitor::examine_edge("
    "WIR_HierarchicalCFG::CGraphEdge, const CGraph&) const" );
                                                 ...
                                                 const CGraph &g ) const
{
  ...
  DSTART(
    "void WIR_HierarchicalCFG::CFGVisitor::back_edge("
    "WIR_HierarchicalCFG::CGraphEdge, const CGraph&) const" );
