...
//       b33.setSignedness( false );
  ...
  if ( o.getOpCode() == TC13::OpCode::DVADJ ) {
    ...
    WIR_L4 cmp = ( abs_remainder == abs_b );
  ...
  if ( o.getOpCode() == TC13::OpCode::DVSTEP_U ) {
    const auto &divisor = lastOp();
    const auto &d = nthOp( 1 );
    WIR_UpDownValue dividend_quotient = d.extract( 0, 32 );
    WIR_UpDownValue remainder = d.extract( 32, 32 );
  ...
  if ( o.getOpCode() == TC13::OpCode::EQANY_B ) {
    ...
    if ( ( o.getOperationFormat() == TC13::OperationFormat::DDD_1 ) &&
      ...
      cmp =
        accumulate(
          bytePos.begin(), bytePos.end(), cmp,
          [&]( WIR_L4 c, unsigned int bitPos ) {
            return(
              c | ( a.extract( bitPos, 8 ) == b.extract( bitPos, 8 ) ) ); } );
  ...
  if ( o.getOpCode() == TC13::OpCode::EQANY_H ) {
    ...
    if ( ( o.getOperationFormat() == TC13::OperationFormat::DDD_1 ) &&
      ...
      cmp =
        accumulate(
          halfwordPos.begin(), halfwordPos.end(), cmp,
          [&]( WIR_L4 c, unsigned int bitPos ) {
            return(
              c | ( a.extract( bitPos, 16 ) == b.extract( bitPos, 16 ) ) ); } );
  ...
  if ( o.getOpCode() == TC13::OpCode::LEA ) {
    if ( o.getOperationFormat() == TC13::OperationFormat::AALC16BOA ) {
      ...
      const auto &label =
        dynamic_cast<const WIR_LabelParameter &>( std::next( it )->get() );

      if ( ( ( label.getLabelType() == WIR_SymbolType::data ) ||
             ( label.getLabelType() == WIR_SymbolType::function ) ) &&
           ( lastOp().getSignedValue() == 0 ) ) {
        const auto &f = o.getInstruction().getBasicBlock().getFunction();
  ...
  if ( o.getOpCode() == TC13::OpCode::MOVH_A ) {
    ...
    if ( o.getOperationFormat() == TC13::OperationFormat::AL_1 ) {
      // Check the particular constellation
  ...
  if ( o.getOpCode() == TC13::OpCode::SAT_B ) {
    ...
    WIR_L4 cmp1 = ( a < WIR_UpDownValue( TC_Const16_Signed { -0x80 } ) );
    WIR_L4 cmp2 = ( WIR_UpDownValue( TC_Const16_Signed { 0x7F } ) < a );
  ...
  if ( o.getOpCode() == TC13::OpCode::SAT_H ) {
    ...
    WIR_L4 cmp1 = ( a < WIR_UpDownValue( TC_Const16_Signed { -0x8000 } ) );
    WIR_L4 cmp2 = ( WIR_UpDownValue( TC_Const16_Signed { 0x7FFF } ) < a );
      ...
      if ( shiftVal == 0 )
        ...
        if ( shiftedOutBits.containsBits( { WIR_L4::b1, WIR_L4::bX } ) )
          ...
          for ( auto l : LLocations )
            if ( any_of(
                   NLocations.begin(), NLocations.end(),
                   [&]( unsigned int n ) {
                     return(
                       shiftedOutBits.getLocation( l ) ==
...
// cppcheck-suppress checkLevelNormal
{
  ...
  if ( ( o.getOpCode() == TC13::OpCode::CLO_H ) ||
    ...
    for ( auto bitPos : halfwordPos ) {
      auto c1 = pc.second->extract( bitPos, 5 );
      ...
      if ( c1.isBinaryInteger() ) {
        auto leadingBits = c1.getSignedValue();
        if ( o.getOpCode() == TC13::OpCode::CLS_H )
          ++leadingBits;
  ...
  if ( ( o.getOpCode() == TC13::OpCode::MUL ) ||
    ...
    if ( ( a[ 0 ] == WIR_L4::b1 ) &&
         a.extract( 1, a.getBitWidth() - 1 ).containsOnlyBit( WIR_L4::b0 ) ) {
      for ( unsigned int j = 0; j < b.getBitWidth(); ++j )
        if ( c.at( j ) == WIR_L4::bX )
          b.setBit( j, WIR_L4::bX );
    } else
    ...
    if ( ( b[ 0 ] == WIR_L4::b1 ) &&
         b.extract( 1, b.getBitWidth() - 1 ).containsOnlyBit( WIR_L4::b0 ) )
      for ( unsigned int k = 0; k < a.getBitWidth(); ++k )
        if ( c.at( k ) == WIR_L4::bX )
          a.setBit( k, WIR_L4::bX );
                                 ...
                                 unsigned int w ) const
{
  ...
  if ( v.isUnsigned() &&
    ...
    WIR_UpDownValue max_pos1 { WIR_L4::b1, w, false };
    DOUT(
      "Saturating '" << v << "' to max_pos1 (" << max_pos1 << ")." << endl );
    return( max_pos1 );
  }
