...
/*!
  @file wirio.cc
  @brief This file implements elementary output stream routines for the %WIR
         library.
...
/*!
  ...
  code.
*/
static WIR_id_t previousCodeSection = nullid;
...
/*
  wir is an output stream manipulator that provides a simple and generic WIR
  dump.
*/
...
void dumpWIRData( std::ostream &os, const WIR_Data &d )
{
  ...
  if ( os.iword( WIR_CommentCont() ) == 1 )
    for ( const WIR_Comment &cont : d.getContainers<WIR_Comment>() )
      os << cont << endl;
  if ( os.iword( WIR_FileInfoCont() ) == 1 )
    for ( const WIR_FileInfo &cont : d.getContainers<WIR_FileInfo>() )
      os << cont << endl;
  ...
  if ( d.isInitialized() ) {
    for ( auto it = d.getInitData().begin(); it != d.getInitData().end();
      ...
      if ( it == d.getInitData().begin() )
        os << " ";
      for ( const auto &s : it->get().getValues() )
        os << " " << s;
    }
    os << endl;
  }
...
{
  DSTART(
    "void dumpWIRDataSection(ostream&, const WIR_Section*, "
  ...
  if ( sec != nullptr )
    os << "SECTION " << sec->getName() << endl << "{" << endl << endl;
...
void dumpWIRFunction( std::ostream &os, const WIR_Function &f )
{
  ...
  // Check whether the start of a new code section has to be dumped.
  if ( f.isInserted() ) {
    auto &c = f.getCompilationUnit();
    if ( c.isInserted() ) {
      auto &sys = c.getSystem();
      ...
      // Emit section end and start if necessary.
      if ( sec.getID() != previousCodeSection ) {
        // Have the previous code section end.
        if ( previousCodeSection != nullid )
          os << endl << "};" << endl;

        // Start the new code section.
        os << "SECTION " << sec.getName() << endl << "{" << endl << endl;
  ...
  // Check whether the end of a code section has to be dumped.
  if ( f.isInserted() ) {
    const auto &c = f.getCompilationUnit();
    if ( c.isInserted() ) {
      if ( f == c.getFunctions().back().get() )
        // The current function f is the very last one within its compilation
        // unit. Thus end the section now.
        os << endl << "};" << endl;
    }
  }
};
...
void dumpWIRInstruction( std::ostream &os, const WIR_Instruction &i )
{
  ...
  if ( os.iword( WIR_CommentCont() ) == 1 )
    for ( const WIR_Comment &cont : i.getContainers<WIR_Comment>() )
      os << cont << endl;
  if ( os.iword( WIR_FileInfoCont() ) == 1 )
    for ( const WIR_FileInfo &cont : i.getContainers<WIR_FileInfo>() )
      os << cont << endl;
...
void dumpWIROperation( std::ostream &os, const WIR_Operation &o )
{
  ...
  if ( os.iword( WIR_JumpTargets() ) == 1 ) {
    ...
    if ( o.jumpTargetsAdded() ) {
      os << ", targets =";
      for ( const WIR_BasicBlock &t : o.getJumpTargets() )
        os << " " << t.getID();
    }
...
void dumpLDScriptRegions( std::ostream &os, const WIR_System &sys )
{
  ...
  for ( const WIR_MemoryRegion &r : sys.getComponents<WIR_MemoryRegion>() ) {
...
void dumpLDScriptSections( std::ostream &os, const WIR_System &sys )
{
  ...
  for ( const WIR_Symbol &sym : sys.getSymbols() )
    addressMap.insert(
      make_pair(
        sym.getBaseAddress(),
  ...
  for ( auto it = regions.begin(); it != regions.end(); ++it )
    for ( const WIR_Section &sec : it->get().getSections() ) {
                          ...
                          std::set<std::string> &processedSections )
{
  ...
  if ( sectionName.find( ".", 1 ) != string::npos )
    sectionName.resize( sectionName.find( ".", 1 ) );
  ...
  // If current section is not handled by processor-specific section output
  ...
  dumpLDScriptBetweenSections( os, sec );
};
...
/*
  dumpLDScriptBetweenSections dumps a WIR system's in-between section
...
{
  DSTART( "void dumpLDScriptBetweenSections(ostream&, const WIR_Section&)" );

  WIR_Registry::getLdScriptBetweenSectionDumper(
    os.iword( WIR_ProcessorIO() ) )( os, sec );
...
{
  DSTART(
    "void dumpLDScriptSubSections(ostream&, const WIR_Section&, set<string>&, "
    "const string&)" );
  ...
  for ( const WIR_Section &ssec : subSectionList ) {
...
void dumpWIRSystem( std::ostream &os, const WIR_System &s )
{
  ...
  if ( os.iword( WIR_CommentCont() ) == 1 )
    for ( const WIR_Comment &cont : s.getContainers<WIR_Comment>() )
      os << cont << endl;
  if ( os.iword( WIR_FileInfoCont() ) == 1 )
    for ( const WIR_FileInfo &cont : s.getContainers<WIR_FileInfo>() )
      os << cont << endl;
