...
WIR_Reachability::WIR_Reachability( WIR_Reachability &&__o ) :
  WIR_Container<WIR_Reachability> { std::move( __o ) },
  mBB { __o.mBB },
  mReachableBlocks { std::move( __o.mReachableBlocks ) },
  mReachableBlockIDs { std::move( __o.mReachableBlockIDs ) },
  mReachableBlocksWOBackEdges { std::move( __o.mReachableBlocksWOBackEdges ) },
  mReachableBlockIDsWOBackEdges {
    std::move( __o.mReachableBlockIDsWOBackEdges ) }
{
cout << BOOST_CURRENT_FUNCTION << endl;
  DSTART( BOOST_CURRENT_FUNCTION );
...
cout << BOOST_CURRENT_FUNCTION << endl;
  ...
  WIR_Container<WIR_Reachability>::operator = ( std::move( __o ) );
