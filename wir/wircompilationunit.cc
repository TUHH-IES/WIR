...
WIR_CompilationUnit::WIR_CompilationUnit( WIR_CompilationUnit &&__o ) :
  ...
  mFunctions { std::move( __o.mFunctions ) },
  mFunctionReferences { std::move( __o.mFunctionReferences ) },
  mSystemPointer { nullptr },
  mData { std::move( __o.mData ) },
  mDataReferences { std::move( __o.mDataReferences ) },
...
WIR_Function & WIR_CompilationUnit::pushBackFunction( WIR_Function &&o )
{
  ...
  mFunctions.emplace_back( std::move( o ) );
  mFunctions.back().onInsert( this );
  mFunctionReferences.push_back( mFunctions.back() );
...
WIR_Function & WIR_CompilationUnit::pushFrontFunction( WIR_Function &&o )
{
  ...
  mFunctions.emplace_front( std::move( o ) );
  mFunctions.front().onInsert( this );
  mFunctionReferences.push_front( mFunctions.front() );
                                                                                               ...
                                                                                               WIR_Function &&o )
{
  ...
  auto it = mFunctions.begin();
  for ( auto itr = mFunctionReferences.begin(); itr != pos; ++itr, ++it ) ;
  auto it1 = mFunctions.insert( it, std::move( o ) );
  (*it1).onInsert( this );
                                                                                                ...
                                                                                                WIR_Function &&o )
{
  ...
  auto it1 = mFunctions.begin();
  for ( auto itr = mFunctionReferences.begin(); itr != pos; ++itr, ++it1 ) ;
  auto it2 = mFunctions.insert( it1, std::move( o ) );
  (*it2).onInsert( this );
...
bool WIR_CompilationUnit::containsFunction( WIR_id_t id ) const
{
  ...
  return(
    any_of(
      mFunctions.begin(), mFunctions.end(),
      [&]( const WIR_Function &f ) { return( f.getID() == id ); } ) );
};
...
bool WIR_CompilationUnit::containsFunction( const WIR_Function &o ) const
{
  ...
  return(
    any_of(
      mFunctions.begin(), mFunctions.end(),
      [&]( const WIR_Function &f ) { return( f == o ); } ) );
};
...
std::list<std::reference_wrapper<WIR_Function>>::const_iterator WIR_CompilationUnit::findFunction( WIR_id_t id ) const
{
  ...
  return(
    find_if(
      mFunctionReferences.begin(), mFunctionReferences.end(),
      [&]( reference_wrapper<WIR_Function> ref ) {
...
WIR_Data &WIR_CompilationUnit::pushFrontData( WIR_Data &&o )
{
  ...
  mData.emplace_front( std::move( o ) );
  mData.front().onInsert( this );
  mDataReferences.push_front( mData.front() );
                                                                                       ...
                                                                                       WIR_Data &&o )
{
  ...
  auto it = mData.begin();
  for ( auto itr = mDataReferences.begin(); itr != pos; ++itr, ++it ) ;
  auto it1 = mData.insert( it, std::move( o ) );
  (*it1).onInsert( this );
                                                                                        ...
                                                                                        WIR_Data &&o )
{
  ...
  auto it1 = mData.begin();
  for ( auto itr = mDataReferences.begin(); itr != pos; ++itr, ++it1 ) ;
  auto it2 = mData.insert( it1, std::move( o ) );
  (*it2).onInsert( this );
...
cout << BOOST_CURRENT_FUNCTION << endl;
  ...
  return(
    any_of(
      ...
      [&]( const WIR_Data &d ) { return( d.getID() == id ); } ) );
};
...
cout << BOOST_CURRENT_FUNCTION << endl;
  ...
  return(
    find_if(
      mDataReferences.begin(), mDataReferences.end(),
      [&]( reference_wrapper<WIR_Data> ref ) {
        return( ref.get().getID() == id ); } ) );
};
...
/*
  getDataLayout computes a map representing a compilation unit's data layout.
*/
WIR_DataLayoutMap WIR_CompilationUnit::getDataLayout( void ) const
{
  DSTART( "WIR_DataLayoutMap WIR_CompilationUnit::getDataLayout() const" );

  WIR_DataLayoutMap res;

  if ( isInserted() ) {
    const auto &sys = getSystem();

    for ( WIR_Data &d : mDataReferences ) {
      const WIR_Section &sec = sys.findSymbol( d ).getSection();
...
cout << BOOST_CURRENT_FUNCTION << endl;
  ...
  for ( const WIR_Function &f : getFunctions() )
...
{
  DSTART( "void WIR_CompilationUnit::addFunctions() const" );
};
...
void WIR_CompilationUnit::copyCompilationUnit( const WIR_CompilationUnit &__o )
{
  ...
  clearFunctions();
  for ( const WIR_Function &f : __o.mFunctionReferences ) {
    pushBackFunction( std::move( *(functionIDMap[ f.getID() ]) ) );
  ...
  clearData();
  for ( const WIR_Data &d : __o.mDataReferences )
    pushBackData( std::move( *(dataIDMap[ d.getID() ]) ) );
