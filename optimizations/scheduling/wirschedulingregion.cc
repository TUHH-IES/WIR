...
void WIR_SchedulingRegion::buildDG( void )
{
  ...
  for ( auto iIt = instrs.begin(); iIt != instrs.end(); ++iIt ) {
    ...
    for ( ++succIt; succIt != instrs.end(); ++succIt ) {
      for ( const WIR_Operation &o1 : *(*iIt) )
        for ( const WIR_Operation &o2 : *(*succIt) )
    ...
    for ( WIR_SchedulingConstraint &c :
      ...
      for ( auto iIt = constrainedInstrs.begin();
        ...
        for ( ++nextIt; nextIt != constrainedInstrs.end(); ++nextIt ) {
          for ( const WIR_Operation &o1 : iIt->get() )
            for ( const WIR_Operation &o2 : nextIt->get() ) {
...
{
  DSTART(
    "const list<WIR_BasicBlock*>& WIR_SchedulingRegion::getBasicBlocks() "
    "const" );
...
unsigned int WIR_SchedulingRegion::getOperationCount( void ) const
{
  ...
  for ( const WIR_BasicBlock *b : mBasicBlocks )
    accumulate(
      b->begin(), b->end(), res,
      []( unsigned int j, const WIR_Instruction &i ) {
        return( j + i.getOperations().size() ); } );
...
{
  DSTART(
    "unsigned int WIR_SchedulingRegion::getNumberOfSuccessors(const "
    "WIR_Operation&) const" );
...
{
  DSTART(
    "WIR_OperationSet WIR_SchedulingRegion::getUnscheduledPredecessors(const "
    "WIR_Operation&) const" );
...
{
  DSTART(
    "map<long long int, set<reference_wrapper<WIR_Operation>, "
    "WIR_Compare<WIR_Operation> > >& "
    "WIR_SchedulingRegion::getEarliestCycleOpsMap()" );
...
{
  DSTART(
    "map<long long unsigned int, long long int>& "
    "WIR_SchedulingRegion::getEarliestCycleMap()" );
...
std::map<long long, WIR_OperationSet> &WIR_SchedulingRegion::getLatestCycleOpsMap( void )
{
  DSTART(
    "map<long long int, set<reference_wrapper<WIR_Operation>, "
    "WIR_Compare<WIR_Operation> > >& "
    "WIR_SchedulingRegion::getLatestCycleOpsMap()" );
...
std::map<WIR_id_t, long long> &WIR_SchedulingRegion::getLatestCycleMap( void )
{
  DSTART(
    "map<long long unsigned int, long long int>& "
    "WIR_SchedulingRegion::getLatestCycleMap()" );
...
{
  DSTART(
    "map<long long unsigned int, long long unsigned int>& "
    "WIR_SchedulingRegion::getMaxDelayMap()" );
...
std::map<WIR_id_t, long long> &WIR_SchedulingRegion::getMobilityMap( void )
{
  DSTART(
    "map<long long unsigned int, long long int>& "
    "WIR_SchedulingRegion::getMobilityMap()" );
...
{
  DSTART(
    "long long int WIR_SchedulingRegion::updateCycleMaps(const "
    "list<WIR_Operation*>&, long long int)" );
...
*/
void WIR_SchedulingRegion::updateMovedOperations( const std::list<WIR_Operation *> &b )
{
  DSTART(
    "void WIR_SchedulingRegion::updateMovedOperations(const "
    "list<WIR_Operation*>&)" );
...
{
  DSTART(
    "virtual void WIR_SchedulingRegion::addDependences(const WIR_Operation&, "
    "const WIR_Operation&)" );
  ...
  for ( const WIR_Parameter &p2 : o2 )
    if ( p2.getType() == WIR_ParameterType::reg ) {
      const auto &rp2 = dynamic_cast<const WIR_RegisterParameter &>( p2 );
      o2hasRegParams = true;
...
{
  DSTART(
    "bool WIR_SchedulingRegion::checkRAW(const WIR_Operation&, const "
  ...
  return(
    any_of(
      o1.begin(), o1.end(),
      [&]( const WIR_Parameter &p1 ) {
        const auto *rp1 = dynamic_cast<const WIR_RegisterParameter *>( &p1 );
        return(
          rp1 && ( rp1->getUsage() != WIR_Usage::use ) &&
...
{
  DSTART(
    "bool WIR_SchedulingRegion::checkWAR(const WIR_Operation&, const "
    "WIR_RegisterParameter&) const" );
  ...
  return(
    any_of(
      ...
      [&]( const WIR_Parameter &p1 ) {
        const auto *rp1 = dynamic_cast<const WIR_RegisterParameter *>( &p1 );
        return(
          rp1 && ( rp1->getUsage() != WIR_Usage::def ) &&
          sameRegisters( rp1->getRegister(), rp2.getRegister() ) ); } ) );
};
...
{
  DSTART(
    "bool WIR_SchedulingRegion::checkWAW(const WIR_Operation&, const "
    "WIR_RegisterParameter&) const" );
...
void WIR_SchedulingRegion::computeLatencies( void )
{
  ...
  BGL_FORALL_EDGES( e, mDGraph, DGraph ) {
    const auto &o1 = *(mDGraph[ source( e, mDGraph ) ]);
    const auto &o2 = *(mDGraph[ target( e, mDGraph ) ]);
...
{
  DSTART(
    "void WIR_SchedulingRegion::setEarliestCycle(const WIR_Operation&, long "
    "long int, map<long long unsigned int, long long int>&)" );
  ...
  for ( auto p = out_edges( v, mDGraph ); p.first != p.second; ++p.first ) {
    ...
    const WIR_Operation &tgtOp = *(mDGraph[ tgt ]);
...
{
  DSTART(
    "void WIR_SchedulingRegion::setLatestCycle(const WIR_Operation&, long long "
    "int, map<long long unsigned int, long long int>&)" );
  ...
  for ( auto p = in_edges( v, mDGraph ); p.first != p.second; ++p.first ) {
    DGraphEdge e = *(p.first);
    auto src = source( e, mDGraph );
    const WIR_Operation &srcOp = *(mDGraph[ src ]);
...
{
  DSTART(
    "virtual long long int WIR_SchedulingRegion::getStartCycle(const "
    "WIR_Operation&) const" );
...
unsigned long long WIR_SchedulingRegion::computeMaxDelays( void )
{
  ...
  BGL_FORALL_EDGES( e, mDGraph, DGraph ) {
    ...
    if ( mBlockIDs.count( b1.getID() ) ) {
      ...
      if ( !differentBBs ) {
        for ( const WIR_Instruction &i: b2 )
...
{
  DSTART(
    "void WIR_SchedulingRegion::setMaxDelay(const WIR_Operation&, long long "
    "unsigned int)" );
...
{
  DSTART(
    "bool WIR_SchedulingRegion::sameRegisters(const WIR_BaseRegister&, const "
    "WIR_BaseRegister&) const" );

  const auto leafs1 = r1.getLeafs();
  const auto leafs2 = r2.getLeafs();

  bool res =
    any_of(
      leafs1.begin(), leafs1.end(),
      [&]( const WIR_BaseRegister &l1 ) {
        return(
          any_of(
            leafs2.begin(), leafs2.end(),
            [&]( const WIR_BaseRegister &l2 ) {
              if ( l1 == l2 )
                return( true );

              if ( l1.isVirtual() ) {
                const auto &vreg1 =
                  dynamic_cast<const WIR_VirtualRegister &>( l1 );
                if ( vreg1.isPrecolored() ) {
                  ...
                  if ( l2.isVirtual() ) {
                    const auto &vreg2 =
                      dynamic_cast<const WIR_VirtualRegister &>( l2 );
                    if ( vreg2.isPrecolored() &&
                         ( vreg1.getPrecolor() == vreg2.getPrecolor() ) )
                        return( true );
                  }
                }
              }
  ...
  DOUT(
    ...
    "' are " << string( res ? "" : "not " ) << "the same." << endl );
  return( res );
};
...
{
  DSTART(
    "void WIR_SchedulingRegion::visualizeNodes(fstream&, map<unsigned int, "
    "unsigned int>&) const" );
...
{
  DSTART(
    "void WIR_SchedulingRegion::visualizeEdges(fstream&, const map<unsigned int, "
    "unsigned int>&) const" );
