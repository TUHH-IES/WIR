...
// cppcheck-suppress checkLevelNormal
{
  ...
  auto labelParam = [&]( unsigned int i ) {
    ...
    const auto &sys = mFunction.getCompilationUnit().getSystem();
    if ( !sys.containsSymbol( label ) )
      return( (WIR_LabelParameter *) nullptr );

    const auto &sym = sys.findSymbol( label );
  ...
  auto fctLabelParam = [&]( unsigned int i ) {
    ...
    auto it =
      find_if(
        sys.getSymbols().begin(), sys.getSymbols().end(),
        [&]( const reference_wrapper<WIR_Symbol> ref ) {
          return(
            ( ref.get().getType() == WIR_SymbolType::function ) &&
            ( ref.get().getName() == label ) ); } );
    if ( it != sys.getSymbols().end() )
      return( new WIR_LabelParameter( it->get().getFunction() ) );
      ...
      if ( a[ 0 ]->isCompatible( TC_AsmArgument::Type::DREG ) &&
        ...
        if ( ( w != TC_AsmOperationWidth::CODE16 ) &&
             ( ( o == TC13::OpCode::ABS ) || ( o == TC13::OpCode::ABS_B ) ||
               ...
               ( o == TC13::OpCode::FTOU ) || ( o == TC13::OpCode::ITOF ) ||
  ...
  if ( res ) {
    const WIR_Operation &op = b.rbegin()->get().begin()->get();
    if ( op.isCall() || op.isIndirectCall() || op.isReturn() ||
        op.isConditionalJump() || op.isUnconditionalJump() ||
        op.isIndirectJump() ) {
      auto &b1 = mFunction.insertBasicBlock( mIt, {} )->get();
      mBasicBlocks.push_back( b1 );
    }
  }
