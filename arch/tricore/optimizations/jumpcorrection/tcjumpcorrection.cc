...
*/
TC_JumpCorrection::TC_JumpCorrection( WIR_System &s ) :
  WIR_JumpCorrection { s },
  mFreeSpillAddress { 0 }
{
  DSTART( "TC_JumpCorrection::TC_JumpCorrection(WIR_System&)" );
};
...
void TC_JumpCorrection::runOptimization( WIR_Function &f )
{
  ...
  WIR_JumpCorrection::runOptimization( f );
};
...
unsigned int TC_JumpCorrection::adjustConditionalJump( WIR_Operation &o )
{
  ...
  ufAssertT(
    succs.size() == 2,
    "Block requires two successor blocks, got " << succs.size() << "." );
  WIR_BasicBlock &impTgtBB =
    ( succs.begin()->get() == expTgtBB ) ?
    succs.rbegin()->get() : succs.begin()->get();

  if ( !isPhysicalSuccessor( src, impTgtBB ) ) {
    const auto &impTgtSym = mSystem.findSymbol( impTgtBB );
    WIR_disp_t disp1 = computeDisplacement( src, impTgtSym, op );
    InvalidJump j { op, srcSym, impTgtSym, disp1, true };

    DOUT(
      "Displacement of implicit successor = 0x" << hex <<
      getAddress( impTgtBB ) << " (tgt) - ( 0x" << getAddress( src ) <<
      " (src) + " << dec << src.getSize() << " (srcSize) ) = " << disp1 <<
      endl );
    ...
    const auto &r = srcSym.getSection().getRegion();
  ...
  WIR_disp_t disp = computeDisplacement( src, expTgtSym, op );
    ...
    if ( fixInvalidDisplacement( j ) ) {
      ...
      while ( bbIt != src.getFunction().getBasicBlocks().end() ) {
        ...
        if ( bbSym.getSection() == srcSym.getSection() ) {
          ...
          const WIR_Operation *o1 = nullptr;
          for ( auto iit = bbIt->get().rbegin(); iit != bbIt->get().rend();
                ++iit )
            if ( !iit->get().getOperations().empty() ) {
              o1 = &(iit->get().getOperations().back().get());
              break;
            }

          if ( o1 && ( o1->isUnconditionalJump() || isTCReturn( *o1 ) ) )
            break;
      ...
      if ( trampDisp < TC_Const16_Signed::getMaxValue( 16 ) ) {
        ...
        // Invert the jump condition to jump to the successor block instead.
        if ( fixInvalidCondition( j ) ) {
          DOUT( "Inverted jump condition to jump to successor block." << endl );
          ++count;
        } else {
          DOUT( "Failed to invert jump condition to successor basic block." );
        }
      ...
      // The trampoline is not the direct successor of a conditional jump.
      if ( bbIt != srcIt ) {
...
unsigned int TC_JumpCorrection::adjustImplicitJump( WIR_Operation &o )
{
  ...
  const WIR_BasicBlock &tgt = src.getSuccessors().begin()->get();

  if ( !isPhysicalSuccessor( src, tgt ) ) {
    const auto &srcSym = mSystem.findSymbol( src );
    const auto &tgtSym = mSystem.findSymbol( tgt );
    WIR_disp_t disp = computeDisplacement( src, tgtSym, o );
...
unsigned int TC_JumpCorrection::adjustUnconditionalJump( WIR_Operation &o )
{
  ...
  if ( !( o.isCall() || o.isIndirectCall() ) &&
    ...
    if ( o.isIndirectJump() ) {
      ...
      const WIR_DUUDChain &c3 =
        rp2.getContainers<WIR_DUUDChain>().begin()->get();
...
{
  DSTART(
    "void TC_JumpCorrection::generateLongJump(const "
    "WIR_JumpCorrection::InvalidJump&, WIR_BasicBlock&)" );
  ...
  if ( !( jmp.op.isCall() || jmp.op.isIndirectCall() ) || jmp.isImplicit ) {
    ...
    WIR_Instruction &i1 = srcBB.rbegin()->get();
    WIR_Operation &o = i1.begin()->get();
    for ( WIR_Parameter &p1 : o )
      if ( p1.isImplicit() ) {
        if ( p1.getType() == WIR_ParameterType::addr )
          call.pushBackParameter(
            dynamic_cast<WIR_AddressingModeParameter &>( p1 ) );
        else
        if ( p1.getType() == WIR_ParameterType::cond )
          call.pushBackParameter(
            dynamic_cast<WIR_ConditionFieldParameter &>( p1 ) );
        else
        if ( p1.getType() == WIR_ParameterType::imm )
          call.pushBackParameter(
            dynamic_cast<WIR_BaseImmediateParameter &>( p1 ) );
        else
        if ( p1.getType() == WIR_ParameterType::label )
          call.pushBackParameter( dynamic_cast<WIR_LabelParameter &>( p1 ) );
        else
        if ( p1.getType() == WIR_ParameterType::reg )
          call.pushBackParameter( dynamic_cast<WIR_RegisterParameter &>( p1 ) );
        else
          call.pushBackParameter( dynamic_cast<WIR_StringParameter &>( p1 ) );
      }
    ...
    if ( spill ) {
      ...
      // Assign the new basic block to the same memory section as the
      // source basic block.
      const auto &srcSym = mSystem.findSymbol( srcBB );
      mSystem.findSymbol( newBB ).setSection( srcSym.getSection() );
...
bool TC_JumpCorrection::eraseTrampoline( WIR_BasicBlock &b )
{
  ...
  const WIR_Instruction &i = b.begin()->get();
  const WIR_Operation &o = i.begin()->get();
  ...
  const WIR_BasicBlock &succ = succs.begin()->get();
  ...
  for ( const WIR_BasicBlock &pred : b.getPredecessors() ) {
    ...
    bool t = ( jmp->isConditionalJump() && ( getJumpTarget( *jmp ) != b ) );
    if ( jmp->isCall() || jmp->isIndirectCall() || !jmp->isJump() || t ) {
      // Skip preceding calls or indirect jumps, nothing needs to be done here.
      DOUT(
        "No need to redirect, isCall: " << boolalpha << jmp->isCall() <<
        " isIndirectCall: " << jmp->isIndirectCall() << " !isJump: " <<
        !jmp->isJump() << " isConditionalJump && jumpTarget != b: " << t <<
        noboolalpha << endl );
      removeTrampoline = false;
      continue;
    } else {
      DOUT( "Redirecting jump in eraseTrampoline." << endl );
  ...
  if ( removeTrampoline ) {
    DOUT( "Removing trampoline '" + b.getName() + "'." << endl );

    // Move flow facts to the next and only successor.
    WIR_FlowFactUpdater::replaceBasicBlock( b, succ );
...
std::pair<
  ...
  bool> TC_JumpCorrection::determineAddressRegister( WIR_Function &f,
                                                     const WIR_Instruction &i ) const
{
  DSTART(
    "pair<reference_wrapper<WIR_BaseRegister>, bool> "
    "TC_JumpCorrection::determineAddressRegister(WIR_Function&, const "
    "WIR_Instruction&) const" );

  if ( mPhysicalWIR ) {
    // Look for a free physical register. Apply liveness analysis first.
...
{
  DSTART(
    "WIR_Symbol& TC_JumpCorrection::getJumpTarget(const WIR_Operation&) "
    "const" );
  ...
  if ( o.isIndirectCall() || o.isIndirectJump() ) {
    ...
    const WIR_LabelParameter &p =
      dynamic_cast<const WIR_LabelParameter &>( lea.getExplicitParameter( 3 ) );
...
{
  DSTART(
    "unsigned int TC_JumpCorrection::getDisplacementWidth(const "
    "WIR_Operation&) const" );
...
{
  DSTART( "bool TC_JumpCorrection::fixInvalidCondition(const InvalidJump&)" );
  ...
  ufAssertT(
    succs.size() == 2,
    "Failed to fix invalid condition: Expected 2 successor blocks, got " <<
    succs.size() );
  WIR_BasicBlock &impTgtBB =
    ( succs.begin()->get() == expTgtBB ) ?
    succs.rbegin()->get() : succs.begin()->get();
...
{
  DSTART(
    "bool TC_JumpCorrection::fixInvalidDisplacement(const "
    "TC_JumpCorrection::InvalidJump&)" );
...
{
  DSTART(
    "void TC_JumpCorrection::redirectJump(WIR_Operation&, const "
