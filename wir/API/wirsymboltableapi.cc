...
bool WIR_SymbolTable_API::containsSymbol( const std::string &s ) const
{
  ...
  return(
    any_of(
      mSymbolReferences.begin(), mSymbolReferences.end(),
      [&]( const reference_wrapper<WIR_Symbol> ref ) {
        return( ref.get().getName() == s ); } ) );
};
...
WIR_Symbol &WIR_SymbolTable_API::findSymbol( const std::string &s ) const
{
  ...
  auto it =
    find_if(
      mSymbolReferences.begin(), mSymbolReferences.end(),
      [&]( reference_wrapper<WIR_Symbol> ref ) {
        return( ref.get().getName() == s ); } );

  if ( it != mSymbolReferences.end() )
    return( it->get() );
...
WIR_SymbolTable_API::WIR_SymbolTable_API( WIR_SymbolTable_API &&__o ) :
  ...
  mSymbols { std::move( __o.mSymbols ) },
  mSymbolReferences { std::move( __o.mSymbolReferences ) },
...
WIR_SymbolTable_API & WIR_SymbolTable_API::operator = ( WIR_SymbolTable_API &&__o )
{
  ...
  mID2Symbol = std::move( __o.mID2Symbol );
  __o.mID2Symbol.clear();
