...
const std::vector<std::reference_wrapper<WIR_BaseRegister>> WIR_BaseRegister::getLeafs( void ) const
{
  ...
  while ( !q.empty() ) {
    ...
    if ( !( r->hasChilds() ) )
      ...
      if ( r->isVirtual() )
        transform(
          dynamic_cast<WIR_VirtualRegister *>( r )->getChilds().begin(),
          dynamic_cast<WIR_VirtualRegister *>( r )->getChilds().end(),
          back_inserter( q ), [&]( WIR_BaseRegister &c ) { return( &c ); } );
      else
        transform(
          dynamic_cast<WIR_PhysicalRegister *>( r )->getChilds().begin(),
          dynamic_cast<WIR_PhysicalRegister *>( r )->getChilds().end(),
          back_inserter( q ), [&]( WIR_BaseRegister &c ) { return( &c ); } );
    }
  }
