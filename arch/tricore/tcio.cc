...
/*
  ...
  specific fashion.
*/
void dumpTCDataSection( std::ostream &os, const WIR_Section *sec,
                        const std::list<std::reference_wrapper<WIR_Data>> &l,
                        bool firstSec )
{
  DSTART(
    "void dumpTCDataSection(ostream&, const WIR_Section*, const "
...
void dumpTCLdScript( std::ostream &os, const WIR_System &sys )
{
  ...
  if ( it != sys.getComponents().end() ) {
    const auto &r = dynamic_cast<const WIR_MemoryRegion &>( it->get() );
...
/*
  dumpTCLDScriptBetweenSections dumps a WIR system's in-between section
  information as linker script to an output stream in a TriCore-specific
  fashion.
*/
void dumpTCLDScriptBetweenSections( [[maybe_unused]] std::ostream &os,
                                    [[maybe_unused]] const WIR_Section &sec )
{
  DSTART( "void dumpTCLDScriptBetweenSections(ostream&, const WIR_Section&)" );

  // Expand this function if there are any TriCore-specific in-between section
  // information that needs to be added to the linker script.
};
  ...
  for ( auto it = params.begin(); it != params.end(); ++it ) {
    ...
    if ( emitParam ) {
      ...
      if ( ( ( expParamCount == 1 ) &&
        ...
        const auto &offset =
          dynamic_cast<const TC_Const16_Signed &>( (*it).get() );
      ...
      if ( ( ( expParamCount == 3 ) &&
        ...
        switch ( p.getType() ) {
          ...
          case WIR_ParameterType::addr: {
            const auto &addP =
              dynamic_cast<const WIR_AddressingModeParameter &>( p );
