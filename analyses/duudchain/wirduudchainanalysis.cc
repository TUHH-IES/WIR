          ...
          if ( p.getType() == WIR_ParameterType::reg ) {
            ...
            if ( rp.isUsed() || rp.isDefUsed() ) {
              ...
              for ( WIR_RegisterParameter &def : reachingDefinitions ) {
                WIR_BaseRegister &defReg = def.getRegister();
                    ...
                    if ( it != precolRMap.end() )
                      if ( any_of(
                             ...
                             [&]( const WIR_VirtualRegister &vr ) {
                               return( usedLeafs.count( vr.getID() ) ); } ) ) {
                        duudChainFound = true;
                        break;
                      }
                  }
              ...
              const auto &reachingInputs =
                i.getContainers<WIR_ReachingDefinitions>().begin()->get()
                 .getReachingInputs();

              // Iterate over all external inputs reaching the current use.
              for ( const WIR_PhysicalRegister &pReg : reachingInputs ) {
                DOUT(
                ...
                for ( const WIR_PhysicalRegister &leaf : pReg.getLeafs() ) {
                  if ( usedLeafs.count( leaf.getID() ) ) {
                    udChainFound = true;
                    break;
                  }
