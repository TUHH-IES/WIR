...
@/tmp/WIR@> ls -F
AUTHORS       |DEBUGMACRO_CONF/|  Makefile.am         README.md  |arch/|         |doc/|        |optimizations/|
CITATION.cff  |LIBUSEFUL/|        Makefile.bootstrap  Releases   configure.ac  |flowfacts/|  |tests/|
ChangeLog     LICENSE           NEWS                |analyses/|  |containers/|   |m4/|         |wir/|
\end{lstlisting}
...
@/tmp/WIR@> ls -F
AUTHORS           LICENSE             README.md   |autom4te.cache/|  §configure§*    |flowfacts/|   |optimizations/|
CITATION.cff      Makefile.am         Releases    §compile§*         configure.ac  §install-sh§*  §test-driver§*
ChangeLog         Makefile.bootstrap  aclocal.m4  §config.guess§*    |containers/|   §ltmain.sh§*   |tests/|
|DEBUGMACRO_CONF/|  Makefile.in         |analyses/|   §config.sub§*      §depcomp§*      |m4/|          |wir/|
|LIBUSEFUL/|        NEWS                |arch/|       config_wir.h.in  |doc/|          §missing§*
\end{lstlisting}
...
@/tmp/WIR/BUILD@> ls -F
|LIBUSEFUL/|  |analyses/|  config.log      config_wir.h  |doc/|        §libtool§*        stamp-h1  |wir/|
Makefile    |arch/|      §config.status§*  |containers/|   |flowfacts/|  |optimizations/|  |tests/|
\end{lstlisting}
...
\begin{itemize}
  ...
  \item to activate or deactivate certain features of the \gls{WIR} software package like, e.g., the browsable HTML reference documentation (\texttt{-{}-with-doxygen-doc}), the present \LaTeX\ documentation (\texttt{-{}-with-latex-doc}), the debug macro based infrastructure (\texttt{-{}-enable-wir-debugmacros}), or the full memory debugging features based on AddressSanitizer and memory protection (\texttt{-{}-enable-memory-debugging}).
\end{itemize}
...
\begin{table}[!b]
  \begin{center}
    ...
    \begin{tabular}{|p{69mm}|p{96mm}|}
      ...
      \hline
      \raggedright \texttt{\small-{}-with-latex-doc} or \texttt{\small-{}-without-latex-doc} & Build comprehensive manuals and documentation using \LaTeX\ (default: no) \\
      \hline
