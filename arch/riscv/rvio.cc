...
{
  DSTART(
    "void dumpRVDataSection(ostream&, const WIR_Section*, "
    "const list<reference_wrapper<WIR_Data> >&, bool)" );
  ...
  if ( sec->getName() != ".rodata" ) {
    ...
    if ( !( r.getAttributes() &
         (unsigned long) WIR_MemoryRegionAttributes::read ) )
      // TODO: Check and remove this TriCore-specific stuff.
      throw ufFatalError(
...
{
  DSTART( "void dumpRVFunction(ostream&, const WIR_Function&)" );
...
/*
  dumpRVLdScript dumps a WIR system's memory layout as linker script to an
  output stream in a RISC-V- and GNU-ld specific fashion.
*/
void dumpRVLdScript( std::ostream &os, const WIR_System &sys )
{
  DSTART( "void dumpRVLdScript(ostream&, const WIR_System&)" );
  ...
  os << "OUTPUT_ARCH(riscv)" << endl;
  os << "ENTRY(_start)" << endl << endl;

  // Write list of available memory regions.
  dumpLDScriptRegions( os, sys );

  // Write some global definitions.
  os << "/* Begin of RISC-V definitions */" << endl << hex;
  os << "/* End of RISC-V definitions */" << endl << endl;
...
/*
  dumpRVLdScriptSection dumps a WIR section as linker script entry to an output
  stream in a RSIC-V- and GNU-ld specific fashion.
*/
bool dumpRVLdScriptSection( std::ostream &os, const WIR_Section &sec )
{
  DSTART( "bool dumpRVLdScriptSection(ostream&, const WIR_Section&)" );
  ...
  if( sec.getName() == ".sdata" ) {
    os << "    __SDATA_BEGIN__ = .;" << endl
       << "    *(.srodata.cst16) *(.srodata.cst8)"
       << " *(.srodata.cst4) *(.srodata.cst2) *(.srodata .srodata.*)" << endl
       << "    *(.sdata .sdata.* .gnu.linkonce.s.*)" << endl;
    return( true );
  }

  if( sec.getName() == ".data" ) {
    os << "    __DATA_BEGIN__ = .;" << endl
       << "    *(.data .data.* .gnu.linkonce.d.*)" << endl
  ...
  /*if( sec.getName() == ".text" ) {
    os << "    *(.text.unlikely .text.*_unlikely .text.unlikely.*)" << endl
       << "    *(.text.exit .text.exit.*)" << endl
       << "    *(.text.startup .text.startup.*)" << endl
       << "    *(.text.hot .text.hot.*)" << endl
       << "    *(SORT(.text.sorted.*))" << endl
       << "    *(.text .stub .text.* .gnu.linkonce.t.*)" << endl
       << "    *(.gnu.warning)" << endl;
    return( true );
  }*/
...
/*
  dumpRVLDScriptBetweenSections dumps a WIR system's in-between section
  information as linker script to an output stream in a RISC-V- and GNU-ld
  specific fashion.
*/
void dumpRVLDScriptBetweenSections( std::ostream &os, const WIR_Section &sec )
{
  DSTART( "void dumpRVLDScriptBetweenSections(ostream&, const WIR_Section&)" );
  ...
  if ( sec.getName() == ".bss" ) {
    os << "    . = ALIGN(32/8);" << endl
       << "    . = SEGMENT_START(\"ldata-segment\", .);" << endl
       << "    . = ALIGN(32 / 8);" << endl
       << "    __BSS_END__ = .;" << endl
       << "    __global_pointer$ = MIN(__SDATA_BEGIN__ + 0x800, " << endl
  ...
  if ( sec.getName() == ".text" ) {
    os << "    . = DATA_SEGMENT_ALIGN (CONSTANT (MAXPAGESIZE), "
       << "CONSTANT (COMMONPAGESIZE));" << endl;
  }
};
...
void dumpRVOperation( std::ostream &os, const WIR_Operation &o )
{
  ...
  // Space between opcode and parameters.
  auto whitespace = string( 16 - o.getOpCode().getName().size(), ' ' );
    ...
    if ( emitParam ) {
      ...
      if ( ( expParamCount != 3 ) ||
           ( ( f != RV32I::OperationFormat::RC12R_1 ) &&
             ...
             ( f != RV32IC::OperationFormat::SRC7R_1 ) &&
             ( f != RV32IC::OperationFormat::SRC7R_2 ) &&
             ( f != RV32IC::OperationFormat::SRC8R_1 ) &&
             ( f != RV32IC::OperationFormat::SRC8R_2 ) &&
             ( f != RV32IF::OperationFormat::FC12R_1 ) &&
             ( f != RV32IF::OperationFormat::FC12R_2 ) ) )
        os << ", ";
      ...
      if ( ( ( f == RV32I::OperationFormat::RLR_1 ) ||
             ( f == RV32I::OperationFormat::RLR_2 ) ||
             ( f == RV32IF::OperationFormat::FLR_1 ) ||
             ( f == RV32IF::OperationFormat::FLR_2 ) ) &&
           ( expParamCount == 2 ) ) {
        // LO-part of a label.
        os << "%lo(" << dynamic_cast<WIR_LabelParameter &>( it->get() ) << ")";
      ...
      if ( ( o.getOpCode() == RV32I::OpCode::JR ) && ( expParamCount == 1 ) )
        os << "x0, " << p << ", 0";
      else

      if ( ( ( o.getOpCode() == RV32IF::OpCode::FMV_S ) ||
             ( o.getOpCode() == RV32IFD::OpCode::FMV_D ) ) &&
           ( expParamCount == 2 ) )
        os << p << ", " << p;
      else
  ...
  if ( ( o.getOpCode() == RV32I::OpCode::RET ) )
    // The return operation is a pseudo-operation de facto realized by a JALR.
    // For a proper realization, the JALR needs registers x0, x1 and a 0 as
    // immediate value, which are added here.
    os << whitespace << "x0, x1, 0";
};
...
void dumpRVRegisterParameter( std::ostream &os, const WIR_RegisterParameter &p )
{
  ...
  if ( r.isVirtual() ) {
    ...
    if ( vreg.hasChilds() )
      ufAssert( !(vreg.begin()->get().isPrecolored()) );
  }
};
