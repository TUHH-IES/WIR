...
{
  DSTART(
    "WIR_LocalScheduling::WIR_LocalScheduling(WIR_Function&, const "
    "WIR_SchedulingPriority&, bool, bool)" );
};
...
void WIR_LocalScheduling::init( WIR_Function &f )
{
  ...
  transform(
    regions.begin(), regions.end(), back_inserter( mRegions ),
    [&]( WIR_BlockSchedulingRegion *r ) {
      return( unique_ptr<WIR_BlockSchedulingRegion>( r ) ); } );
};
...
void WIR_LocalScheduling::schedule( WIR_BlockSchedulingRegion &r )
{
  ...
  DACTION(
    ...
    for ( const auto *b : r.getBasicBlocks() ) {
      DOUT( *b );
  ...
  while ( !mReadyList->empty() ) {
    ...
    if ( bestBundle.empty() )
      ...
      const WIR_Operation *lastOp = nullptr;
  ...
  DACTION(
    ...
    if ( newInstrIDs.size() != instrIDs.size() )
      ...
      for ( ; iIt != instrIDs.end(); ++iIt, ++iIt1 ) {
        const auto &opIDs = *iIt;
        const auto &opIDs1 = *iIt1;
...
*/
void WIR_LocalScheduling::removeEmptyInstructions( const WIR_BlockSchedulingRegion &r )
{
  DSTART(
    "void WIR_LocalScheduling::removeEmptyInstructions(const "
    "WIR_BlockSchedulingRegion&)" );
