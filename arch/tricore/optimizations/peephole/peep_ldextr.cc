...
void TC_Peep_LDEXTR::transformPeephole( const WIR_Peephole::peephole &p )
{
  ...
  if ( mPatchLd ) {
    ...
    for ( auto it2 = o0.begin(); it2 != o0.end(); ++it2 )
      if ( it2->get().containsContainers(
             WIR_BitValues::getContainerTypeID() ) ) {
        auto &cOrig = it2->get().getContainers<WIR_BitValues>().begin()->get();

        for ( auto &inEdge : cOrig.getInValues() ) {
          auto &srcContainer =
            inEdge.rp->getContainers<WIR_BitValues>().begin()->get();
          srcContainer.eraseOutValues( it2->get() );
        }

        for ( auto &outEdge : cOrig.getOutValues() ) {
          auto &tgtContainer =
            outEdge.rp->getContainers<WIR_BitValues>().begin()->get();
          tgtContainer.eraseInValues( it2->get() );
        }
      }
  }
  ...
  for ( WIR_Operation &o1 : mFoundEXTRs ) {
    ...
    mNewLocation.insert( { pExtr2.getID(), ref( *r2 ) } );
  ...
  for ( const WIR_Operation &o1 : mFoundEXTRs )
