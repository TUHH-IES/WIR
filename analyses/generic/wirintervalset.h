...
std::cout << BOOST_CURRENT_FUNCTION << std::endl;
    ...
    */
    ...
    explicit WIR_IntervalSet( const WIR_IntervalSet<intType, otherComp> &__o ) :
      mChunks { __o.mChunks }
    {
std::cout << BOOST_CURRENT_FUNCTION << std::endl;
      DSTART( BOOST_CURRENT_FUNCTION );
    ...
    */
    template<WIR_IntervalCompression otherComp>
    explicit WIR_IntervalSet( WIR_IntervalSet<intType, otherComp> &&__o ) :
      mChunks { std::move( __o.mChunks ) }
    {
      DSTART(
...
std::cout << BOOST_CURRENT_FUNCTION << std::endl;
      ...
      return(
        accumulate(
          mChunks.begin(), mChunks.end(), res,
          []( intType acc, const _intervalType &i ) {
            return( acc + i.getWidth() ); } ) );
    };
...
std::cout << BOOST_CURRENT_FUNCTION << std::endl;
      ...
      transform(
        mChunks.begin(), mChunks.end(), mChunks.begin(),
        [&]( const _intervalType &chunk ) { return( chunk + i ); } );
      validate<WIR_IntervalCompression::permanent>();
      return( *this );
    };
