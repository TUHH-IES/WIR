...
int main( void )
{
  ...
  // This must assert since the operation contains virtual registers owned by
  // f1.
  WIR_BasicBlock &b2 =
  ...
  WIR_VirtualRegisterSet vregs = b2.getVREGs();

  for ( WIR_VirtualRegister &r : vregs )
    ufAssertT(
      !r.isInserted() || r.getFunction() == f,
      "Illegal attempt to insert basic block '" << b2.getName() <<
      "' into function '" << f.getName() << "' (ID " << f.getID() <<
