...
WIR_Stats::WIR_Stats( WIR_Stats &&__o ) :
  mSystems1 { std::move( __o.mSystems1 ) },
  mCompilationUnits1 { std::move( __o.mCompilationUnits1 ) },
  mFunctions1 { std::move( __o.mFunctions1 ) },
  mBasicBlocks1 { std::move( __o.mBasicBlocks1 ) },
  mInstructions1 { std::move( __o.mInstructions1 ) },
  mOperations1 { std::move( __o.mOperations1 ) },
  mParameters1 { std::move( __o.mParameters1 ) },
  mRegisters1 { std::move( __o.mRegisters1 ) },
  mDataObjects1 { std::move( __o.mDataObjects1 ) },
  mSystems2 { std::move( __o.mSystems2 ) },
  mCompilationUnits2 { std::move( __o.mCompilationUnits2 ) },
  mFunctions2 { std::move( __o.mFunctions2 ) },
  mBasicBlocks2 { std::move( __o.mBasicBlocks2 ) },
  mInstructions2 { std::move( __o.mInstructions2 ) },
  mOperations2 { std::move( __o.mOperations2 ) },
  mParameters2 { std::move( __o.mParameters2 ) },
  mRegisters2 { std::move( __o.mRegisters2 ) },
  mDataObjects2 { std::move( __o.mDataObjects2 ) }
{
cout << BOOST_CURRENT_FUNCTION << endl;
  DSTART( BOOST_CURRENT_FUNCTION );
...
void WIR_Stats::rotate( void )
{
  ...
  mSystems1 = std::move( mSystems2 );
  mCompilationUnits1 = std::move( mCompilationUnits2 );
  mFunctions1 = std::move( mFunctions2 );
  mBasicBlocks1 = std::move( mBasicBlocks2 );
  mInstructions1 = std::move( mInstructions2 );
  mOperations1 = std::move( mOperations2 );
  mParameters1 = std::move( mParameters2 );
  mRegisters1 = std::move( mRegisters2 );
  mDataObjects1 = std::move( mDataObjects2 );
};
                       ...
                       std::set<WIR_id_t> &rid )
{
  ...
  fid.insert( f.getID() );
  for ( const WIR_BaseRegister &r : f.getVirtualRegisters() )
    rid.insert( r.getID() );
                              ...
                              const std::set<WIR_id_t> &s2 ) const
{
  ...
  return(
    count_if(
      s1.begin(), s1.end(),
      [&]( WIR_id_t id ) { return( !s2.count( id ) ); } ) );
};
