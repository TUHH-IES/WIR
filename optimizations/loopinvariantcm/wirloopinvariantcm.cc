...
{
  DSTART(
    "virtual bool WIR_LoopInvariantCM::isLoopInvariant(const "
    "WIR_StringParameter&) const" );
...
void WIR_LoopInvariantCM::identifyLoops( const WIR_Function &f )
{
  ...
  while ( !workSet.empty() )
    for ( auto it = workSet.begin(); it != workSet.end(); ) {
      ...
      if ( all_of(
             n.getChilds().begin(), n.getChilds().end(),
             [&]( const WIR_ControlTreeNode &ch ) {
               return( visitedNodes.count( ch.getID() ) ); } ) ) {
...
void WIR_LoopInvariantCM::identifyLIC( const WIR_ControlTreeNode &l )
{
  ...
  for ( const WIR_BasicBlock &b : mBSet ) {
        ...
        for ( const WIR_Parameter &p : o )
          if ( p.getType() == WIR_ParameterType::reg ) {
            ...
            if ( rp.isDefined() || rp.isDefUsed() ) {
              ...
              for ( const WIR_BaseRegister &leaf : r.getLeafs() ) {
                ++(mRegisterDefinitions[ leaf.getID() ]);

                if ( leaf.isVirtual() ) {
                  const auto &vReg =
                    dynamic_cast<const WIR_VirtualRegister &>( leaf );
  ...
  function<void( WIR_BasicBlock & )> bfs = [&]( const WIR_BasicBlock &b ) {
    ...
    for ( reference_wrapper<WIR_BasicBlock> ref : b.getSuccessors() )
      if ( mBSet.count( ref.get() ) )
        if ( !visited.count( ref.get().getID() ) ) {
          bfsOrder.push_back( &(ref.get()) );
          visited.insert( ref.get().getID() );
          T.push_back( &(ref.get()) );
        }
  ...
  do {
    change = false;
    accumulate(
      bfsOrder.begin(), bfsOrder.end(), change,
      [&]( bool res, const WIR_BasicBlock *b ) {
        return( res | identifyLIC( *b ) ); } );
  } while ( change );
...
bool WIR_LoopInvariantCM::identifyLIC( const WIR_BasicBlock &b )
{
  ...
  // register defined by i within the loop (this implements criterion Dom_Uses
  // from S. S. Muchnick, page 403).
  auto domUses = [&]( const WIR_Instruction &i ) -> bool {
    // A basic block without any successor can of course not dominate any other
    // block of some loop.
    if ( i.getBasicBlock().getSuccessors().size() == 0 )
      return( false );
  ...
  auto domForLoop = [&]( const WIR_Instruction &i ) -> bool {
    ...
    for ( const WIR_Operation &o : i )
      for ( const WIR_Parameter &p : o )
        if ( p.getType() == WIR_ParameterType::reg ) {
          ...
          if ( rp.isDefined() || rp.isDefUsed() ) {
            // Get the current definition's DU chain.
            const auto &du = rp.getContainers<WIR_DUUDChain>().begin()->get();
            const auto &duChain = du.getDUChains();

            for ( const WIR_RegisterParameter &use : duChain ) {
              const auto &b =
                use.getOperation().getInstruction().getBasicBlock();

              if ( !mBSet.count( const_cast<WIR_BasicBlock &>( b ) ) )
                allUsesInLoop = false;
            }
          }
        }
    ...
    // Criterion 4b) above.
    auto loopBounds =
      WIR_FlowFactRef::get( *mEntry ).getFlowFacts<WIR_LoopBound>();
    bool loopExecutedOnce =
      any_of(
        loopBounds.begin(), loopBounds.end(),
        [&]( const WIR_LoopBound &lb ) { return( lb.getMin() > 0 ); } );
  ...
  for ( WIR_Instruction &i : b ) {
    ...
    if ( all_of(
           ...
           [&]( const WIR_Operation &o ) {
             return( mInvariantOpIDs.count( o.getID() ) ); } ) ) {
...
void WIR_LoopInvariantCM::checkLoopInvariant( const WIR_Operation &o )
{
  ...
  if ( o.isImplicitMemoryAccess() || o.isMemoryLoad() || o.isMemoryStore() ||
  ...
  for ( const WIR_Parameter &p : o )
    if ( p.getType() == WIR_ParameterType::reg ) {
      ...
      if ( rp.isDefined() || rp.isDefUsed() ) {
        ...
        for ( const WIR_BaseRegister &l : r.getLeafs() ) {
          if ( mRegisterDefinitions[ l.getID() ] != 1 )
            return;

          if ( l.isVirtual() ) {
            const auto &vReg = dynamic_cast<const WIR_VirtualRegister &>( l );
            if ( vReg.isPrecolored() &&
  ...
  if ( all_of(
         ...
         [&]( const WIR_Parameter &p ) {
           return( mInvariantParamIDs.count( p.getID() ) ); } ) ) {
...
void WIR_LoopInvariantCM::checkLoopInvariant( const WIR_Parameter &p )
{
  ...
  if ( all_of(
         udChain.begin(), udChain.end(),
         [&]( const WIR_RegisterParameter &def ) {
           return(
             !mBSet.count(
               def.getOperation().getInstruction().getBasicBlock() ) ); } ) ) {
    // Identified a loop-invariant parameter.
    mInvariantParamIDs.insert( p.getID() );
                                            ...
                                            const WIR_Operation &o2 ) const
{
  ...
  const auto &i2 = o2.getInstruction();

  auto &b1 = i1.getBasicBlock();
  const auto &b2 = i2.getBasicBlock();
  ...
  if ( b1 == b2 ) {
    ...
    for ( const WIR_Instruction &i: b1 ) {
      if ( i == i1 )
        return ( true );
      else if ( i == i2 )
        return ( false );
    ...
    for ( WIR_Instruction &i : pred ) {
      for ( WIR_Operation &o : i ) {
        ...
        if ( o.jumpTargetsAdded() ) {
          ...
          for ( const WIR_BasicBlock &tgt : targets )
            if ( tgt == en ) {
