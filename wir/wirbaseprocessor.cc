...
WIR_BaseProcessor::WIR_BaseProcessor( WIR_BaseProcessor &&__o ) :
  WIR_SystemComponent { std::move( __o ) },
  mPhRegPointers { std::move( __o.mPhRegPointers ) },
  mPhRegReferences { std::move( __o.mPhRegReferences ) },
  mProcessorName { std::move( __o.mProcessorName ) },
  mISAName { std::move( __o.mISAName ) },
  mClockFrequency { std::move( __o.mClockFrequency ) },
  mVoltage { std::move( __o.mVoltage ) },
  mSections { std::move( __o.mSections ) },
  mSectionReferences { std::move( __o.mSectionReferences ) }
{
...
cout << BOOST_CURRENT_FUNCTION << endl;
  ...
  WIR_SystemComponent::operator = ( std::move( __o ) );
...
/*
  isImplicitMemoryAccess returns whether an opcode implicitly accesses memory
  without being an explicit load or store operation.

  By overloading this method in derived classes, specific tests for individual
  processor architectures can be realized.
*/
bool WIR_BaseProcessor::OpCode::isImplicitMemoryAccess( const WIR_Operation &o ) const
{
  DSTART(
    "virtual bool WIR_BaseProcessor::OpCode::isImplicitMemoryAccess(const "
    "WIR_Operation&) const" );

  (void) o;

  return( mIsImplicitMemoryAccess );
};
...
/*
  Default constructor for opcodes without any specific properties.
*/
WIR_BaseProcessor::OpCode::OpCode( const std::string &s ) :
  WIR_InheritableEnum {},
  mName { s },
  mIsImplicitMemoryAccess{ false },
  mIsMemoryStore { false },
  mIsMemoryLoad { false },
  mIsMove { false },
  mIsCall { false },
  mIsIndirectCall { false },
  mIsReturn { false },
  mIsConditionalJump { false },
  mIsUnconditionalJump { false },
  mIsIndirectJump { false },
  mIsAsmDataDirective { false },
  mHasSideEffects { false }
{
  DSTART( "WIR_BaseProcessor::OpCode::OpCode(const string&)" );
};


/*
  addType adds a concrete opcode type by setting the class' corresponding
  Boolean flag.
*/
void WIR_BaseProcessor::OpCode::addType( WIR_OpCodeType t )
{
  DSTART( "void WIR_BaseProcessor::OpCode::addType(WIR_OpCodeType)" );

  switch ( t ) {
    case WIR_OpCodeType::load: {
      mIsMemoryLoad = true;
      break;
    }

    case WIR_OpCodeType::store: {
      mIsMemoryStore = true;
      break;
    }

    case WIR_OpCodeType::implicitMemoryAccess: {
      mIsImplicitMemoryAccess = true;
      break;
    }

    case WIR_OpCodeType::move: {
      mIsMove = true;
      break;
    }

    case WIR_OpCodeType::call: {
      mIsCall = true;
      break;
    }

    case WIR_OpCodeType::indirectCall: {
      mIsIndirectCall = true;
      break;
    }

    case WIR_OpCodeType::ret: {
      mIsReturn = true;
      break;
    }

    case WIR_OpCodeType::condJump: {
      mIsConditionalJump = true;
      break;
    }

    case WIR_OpCodeType::uncondJump: {
      mIsUnconditionalJump = true;
      break;
    }

    case WIR_OpCodeType::indirectJump: {
      mIsIndirectJump = true;
      break;
    }

    case WIR_OpCodeType::asmDirective: {
      mIsAsmDataDirective = true;
      break;
    }

    case WIR_OpCodeType::sideEffect: {
...
/*
  Dummy function for adding opcode type information which does nothing.

  It only serves to terminate the recursion of the variadic method addTypes.
*/
void WIR_BaseProcessor::OpCode::addTypes( void ) const
{
  DSTART( "void WIR_BaseProcessor::OpCode::addTypes() const" );
};
...
/*
  setBitWidth sets a register type's bit width.
*/
void WIR_BaseProcessor::RegisterType::setBitWidth( unsigned int w )
{
  DSTART( "void WIR_BaseProcessor::RegisterType::setBitWidth(unsigned int)" );
...
const std::vector<std::reference_wrapper<const WIR_PhysicalRegister>> WIR_BaseProcessor::getPhRegs( void ) const
{
  ...
  copy(
    mPhRegReferences.begin(), mPhRegReferences.end(), back_inserter( res ) );
...
const std::vector<std::reference_wrapper<const WIR_PhysicalRegister>> WIR_BaseProcessor::getPhRegs( const RegisterType &t ) const
{
  ...
  copy_if(
    mPhRegReferences.begin(), mPhRegReferences.end(), back_inserter( res ),
    [&]( reference_wrapper<WIR_PhysicalRegister> ref ) {
      return( ref.get().getType() == t ); } );
...
cout << BOOST_CURRENT_FUNCTION << endl;
  ...
  return(
    any_of(
      mSectionReferences.begin(), mSectionReferences.end(),
      [&]( const reference_wrapper<WIR_Section> ref ) {
        return( ref.get().getID() == id ); } ) );
};
...
/*
  registerProcessor is a wrapper around the corresponding method of class
  WIR_Registry.
*/
void WIR_BaseProcessor::registerProcessor( WIR_BaseProcessor &&p )
{
  WIR_Registry::registerProcessor( std::move( p ) );
};


/*
  registerOperationFormat is a wrapper around the corresponding method of class
  WIR_Registry.
*/
void WIR_BaseProcessor::registerOperationFormat( const OperationFormat &id,
                                                 WIR_OperationFormat &&f )
{
  WIR_Registry::registerOperationFormat( id, std::move( f ) );
};


/*
  registerOpCode is a wrapper around the corresponding method of class
  WIR_Registry.
*/
void WIR_BaseProcessor::registerOpCode( const OpCode &o,
                                        const OperationFormat &f )
{
  WIR_Registry::registerOpCode( o, f );
};


/*
  registerBasicBlockDumper is a wrapper around the corresponding method of class
  WIR_Registry.
*/
void WIR_BaseProcessor::registerBasicBlockDumper( unsigned int id,
                                                  WIR_BasicBlockDumper f )
{
  WIR_Registry::registerBasicBlockDumper( id, f );
};


/*
  registerBlockLabelDumper is a wrapper around the corresponding method of class
  WIR_Registry.
*/
void WIR_BaseProcessor::registerBlockLabelDumper( unsigned int id,
                                                  WIR_BlockLabelDumper f )
{
  WIR_Registry::registerBlockLabelDumper( id, f );
};


/*
  registerCommentDumper is a wrapper around the corresponding method of class
  WIR_Registry.
*/
void WIR_BaseProcessor::registerCommentDumper( unsigned int id,
                                               WIR_CommentDumper f )
{
  WIR_Registry::registerCommentDumper( id, f );
};


/*
  registerCompilationUnitDumper is a wrapper around the corresponding method of
  class WIR_Registry.
*/
void WIR_BaseProcessor::registerCompilationUnitDumper( unsigned int id,
                                                       WIR_CompilationUnitDumper f )
{
  WIR_Registry::registerCompilationUnitDumper( id, f );
};


/*
  registerDataDumper is a wrapper around the corresponding method of class
  WIR_Registry.
*/
void WIR_BaseProcessor::registerDataDumper( unsigned int id, WIR_DataDumper f )
{
  WIR_Registry::registerDataDumper( id, f );
};


/*
  registerDataSectionDumper is a wrapper around the corresponding method of
  class WIR_Registry.
*/
void WIR_BaseProcessor::registerDataSectionDumper( unsigned int id,
                                                   WIR_DataSectionDumper f )
{
  WIR_Registry::registerDataSectionDumper( id, f );
};


/*
  registerFileInfoDumper is a wrapper around the corresponding method of class
  WIR_Registry.
*/
void WIR_BaseProcessor::registerFileInfoDumper( unsigned int id,
                                                WIR_FileInfoDumper f )
{
  WIR_Registry::registerFileInfoDumper( id, f );
};


/*
  registerFunctionDumper is a wrapper around the corresponding method of class
  WIR_Registry.
*/
void WIR_BaseProcessor::registerFunctionDumper( unsigned int id,
                                                WIR_FunctionDumper f )
{
  WIR_Registry::registerFunctionDumper( id, f );
};


/*
  registerInstructionDumper is a wrapper around the corresponding method of
  class WIR_Registry.
*/
void WIR_BaseProcessor::registerInstructionDumper( unsigned int id,
                                                   WIR_InstructionDumper f )
{
  WIR_Registry::registerInstructionDumper( id, f );
};


/*
  registerOperationDumper is a wrapper around the corresponding method of class
  WIR_Registry.
*/
void WIR_BaseProcessor::registerOperationDumper( unsigned int id,
                                                 WIR_OperationDumper f )
{
  WIR_Registry::registerOperationDumper( id, f );
};


/*
  registerAddressingModeParameterDumper is a wrapper around the corresponding
  method of class WIR_Registry.
*/
void WIR_BaseProcessor::registerAddressingModeParameterDumper( unsigned int id,
                                                               WIR_AddressingModeParameterDumper f )
{
  WIR_Registry::registerAddressingModeParameterDumper( id, f );
};


/*
  registerConditionFieldParameterDumper is a wrapper around the corresponding
  method of class WIR_Registry.
*/
void WIR_BaseProcessor::registerConditionFieldParameterDumper( unsigned int id,
                                                               WIR_ConditionFieldParameterDumper f )
{
  WIR_Registry::registerConditionFieldParameterDumper( id, f );
};


/*
  registerImmediateParameterDumper is a wrapper around the corresponding method
  of class WIR_Registry.
*/
void WIR_BaseProcessor::registerImmediateParameterDumper( unsigned int id,
                                                          WIR_ImmediateParameterDumper f )
{
  WIR_Registry::registerImmediateParameterDumper( id, f );
};


/*
  registerLabelParameterDumper is a wrapper around the corresponding method of
  class WIR_Registry.
*/
void WIR_BaseProcessor::registerLabelParameterDumper( unsigned int id,
                                                      WIR_LabelParameterDumper f )
{
  WIR_Registry::registerLabelParameterDumper( id, f );
};


/*
  registerLdScriptDumper is a wrapper around the corresponding method of class
  WIR_Registry.
*/
void WIR_BaseProcessor::registerLdScriptDumper( unsigned int id,
                                                WIR_LdScriptDumper f )
{
  WIR_Registry::registerLdScriptDumper( id, f );
};


/*
  registerLdScriptSectionDumper is a wrapper around the corresponding method of
  class WIR_Registry.
*/
void WIR_BaseProcessor::registerLdScriptSectionDumper( unsigned int id,
                                                       WIR_LdScriptSectionDumper f )
{
  WIR_Registry::registerLdScriptSectionDumper( id, f );
};


/*
  registerLdScriptBetweenSectionDumper is a wrapper around the corresponding
  method of class WIR_Registry.
*/
void WIR_BaseProcessor::registerLdScriptBetweenSectionDumper( unsigned int id,
                                                              WIR_LdScriptBetweenSectionDumper f )
{
  WIR_Registry::registerLdScriptBetweenSectionDumper( id, f );
};


/*
  registerRegisterParameterDumper is a wrapper around the corresponding method
  of class WIR_Registry.
*/
void WIR_BaseProcessor::registerRegisterParameterDumper( unsigned int id,
                                                         WIR_RegisterParameterDumper f )
{
  WIR_Registry::registerRegisterParameterDumper( id, f );
};


/*
  registerSystemDumper is a wrapper around the corresponding method of class
  WIR_Registry.
*/
void WIR_BaseProcessor::registerSystemDumper( unsigned int id,
                                              WIR_SystemDumper f )
{
  WIR_Registry::registerSystemDumper( id, f );
};
...
void WIR_BaseProcessor::copyProcessor( const WIR_BaseProcessor &__o )
{
  ...
  for ( const WIR_PhysicalRegister &p : __o.mPhRegReferences ) {
