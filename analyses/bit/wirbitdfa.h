...
/*!
  ...
  Analysis results are stored in WIR_BitValues containers that are attached to
  %WIR register and immediate parameters.
                   ...
                   public WIR_DFG
{
  ...
  protected:
    ...
    */
    ...
    /*!
      @brief updateOperand updates the up/down value associated with a register
             parameter to the bit width required by the parameter's actual
             register.

      @param[in] p A const reference to some parameter. updateOperand only
                   becomes active if p refers to a register parameter.
      @param[in,out] v A reference to an up/down value associated with the given
                       register parameter to be updated.

      In the case of hierarchical registers where parameter p refers to some
      child register of narrow bit width, it can happen that the associated
      up/down value has a large width according to a parent register. Exactly in
      this situation, updateOperand reduces the up/down value's bit width by a
      tailored extract operation.

      This method is virtual so that it can be overloaded by derived classes.

      @author Heiko Falk <Heiko.Falk@tuhh.de>
    */
    virtual void updateOperand( const WIR_Parameter &, WIR_UpDownValue & );
