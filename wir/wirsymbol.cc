...
WIR_Symbol::WIR_Symbol( WIR_Symbol &&__o ) :
  mSymbolType { std::move( __o.mSymbolType ) },
  mReferedID { __o.mReferedID },
  mConst { std::move( __o.mConst ) },
  mExtern { std::move( __o.mExtern ) },
  mGlobal { std::move( __o.mGlobal ) },
  mVolatile { std::move( __o.mVolatile ) },
...
const WIR_Section &WIR_Symbol::getSection( void ) const
{
  ...
  if ( mSymbolType == WIR_SymbolType::block ) {
    const auto &f = getBasicBlock().getFunction();
    const auto &sym = f.getCompilationUnit().getSystem().findSymbol( f );
  ...
  if ( mSymbolType == WIR_SymbolType::function ) {
    ...
    bool blockLevelAssignment =
      any_of(
        f.begin(), f.end(),
        [&]( const WIR_BasicBlock &b ) {
          return( s.findSymbol( b ).mSection != nullptr ); } );

    if ( blockLevelAssignment ) {
      // Yes, so return the section assigned to the very first basic block.
      const WIR_BasicBlock &b = f.getBasicBlocks().front().get();
      const WIR_Symbol &sym = s.findSymbol( b );
