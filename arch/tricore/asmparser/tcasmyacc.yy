...
%define api.parser.class {TC_AsmYacc}
  ...
  | label
    {
      ...
      /*
        We have to use free here, because string tokens are created in
        tcasmlex.ll line 737, using strdup from <string.h>. strdup internally
        uses malloc to allocate storage.
      */
      free( $1 );
    }
  ...
  | HI STRING
    {
      ...
      /*
        We have to use free here, because string tokens are created using strdup
        from <string.h>. strdup internally uses malloc to allocate storage.
      */
      free( $2 );
    }
  | iareg LO STRING
