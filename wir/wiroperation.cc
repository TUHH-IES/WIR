...
WIR_Operation::WIR_Operation( WIR_Operation &&__o ) :
  ...
  mInstructionPointer { nullptr },
  mExplicitParameterReferences {
    std::move( __o.mExplicitParameterReferences ) },
  mOpCode { __o.mOpCode },
  mOperationFormat { __o.mOperationFormat },
  mCheckParameters { true },
  mDontOptimize { __o.mDontOptimize },
  mJumpTargets { std::move( __o.mJumpTargets ) }
{
...
WIR_Operation & WIR_Operation::operator = ( WIR_Operation &&__o )
{
  ...
  mOpCode = std::move( __o.mOpCode );
  __o.mOpCode = nullptr;
  mOperationFormat = std::move( __o.mOperationFormat );
  __o.mOperationFormat = nullptr;
  mParameterPointers = std::move( __o.mParameterPointers );
  __o.mParameterPointers.clear();
  mParameterReferences = std::move( __o.mParameterReferences );
  __o.mParameterReferences.clear();
  mExplicitParameterReferences = std::move( __o.mExplicitParameterReferences );
  __o.mExplicitParameterReferences.clear();
  mInstructionPointer = nullptr;
  mDontOptimize = __o.mDontOptimize;
  mJumpTargets = std::move( __o.mJumpTargets );
...
bool WIR_Operation::containsParameter( WIR_id_t id ) const
{
  ...
  return(
    any_of(
      mParameterReferences.begin(), mParameterReferences.end(),
      [&]( const reference_wrapper<WIR_Parameter> ref ) {
...
/*
  isImplicitMemoryAccess returns whether an operation implicitly accesses memory
  without being an explicit load or store operation.
*/
bool WIR_Operation::isImplicitMemoryAccess( void ) const
{
  DSTART( "bool WIR_Operation::isImplicitMemoryAccess() const" );

  return( getOpCode().isImplicitMemoryAccess( *this ) );
};
...
WIR_BasicBlockSet WIR_Operation::getJumpTargets( void ) const
{
  ...
  for ( const WIR_Parameter &p : getParameters() )
    if ( p.getType() == WIR_ParameterType::label ) {
      const WIR_LabelParameter &lp =
...
void WIR_Operation::checkParameters( void ) const
{
  ...
  for ( const WIR_Parameter &p : mExplicitParameterReferences ) {
    bool ok = true;
    ...
    if ( p.getType() != (*formatIt).get().getType() )
      ...
      switch ( p.getType() ) {
        ...
        case WIR_ParameterType::cond: {
          ...
          const auto &regP =
            dynamic_cast<const WIR_ConditionFieldParameter &>( p );
          const auto &regF =
        ...
        case WIR_ParameterType::imm: {
          ...
          const auto &regP =
            dynamic_cast<const WIR_BaseImmediateParameter &>( p );
          const auto &regF =
        ...
        case WIR_ParameterType::reg: {
          ...
          const auto &regF =
            dynamic_cast<const WIR_RegisterParameter &>( (*formatIt).get() );
...
void WIR_Operation::copyOperation( const WIR_Operation &__o )
{
  ...
  transform(
    __o.mParameterReferences.begin(), __o.mParameterReferences.end(),
    back_inserter( originalIDs ),
    [&]( const WIR_Parameter &p ) { return( p.getID() ); } );
