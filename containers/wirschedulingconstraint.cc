...
WIR_SchedulingConstraint::WIR_SchedulingConstraint( WIR_SchedulingConstraint &&__o ) :
  WIR_Container<WIR_SchedulingConstraint> { std::move( __o ) },
  mType { __o.mType },
  mInstrSequence { std::move( __o.mInstrSequence ) },
  mInstrs { std::move( __o.mInstrs ) }
{
...
cout << BOOST_CURRENT_FUNCTION << endl;
  ...
  WIR_Container<WIR_SchedulingConstraint>::operator = ( std::move( __o ) );
...
bool WIR_SchedulingConstraint::check( void ) const
{
  ...
  for ( const WIR_Instruction &i : mInstrSequence )
    for ( const WIR_Instruction &j : i.getBasicBlock() )
  ...
  for ( const WIR_Instruction &i : mInstrSequence ) {
    if ( cnt != -1 ) {
      ...
      if ( ( mType == WIR_SchedulingConstraintType::ordered ) &&
