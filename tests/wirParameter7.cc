...
int main( void )
{
  ...
  WIR_CompilationUnit &c1 = s.pushBackCompilationUnit( {} );
  WIR_Function &f = c1.pushBackFunction( WIR_Function( "main" ) );
  ...
  WIR_Parameter &p =
    c1.getFunctions().front().get().getBasicBlocks().front().get().getInstructions().front().get().getOperations().front().get().getParameters().front().get();
  p.setImplicit();
