...
void WIR_Webs::identifyWebs( const WIR_VirtualRegister &r )
{
        ...
        for ( WIR_Parameter &p : o )
          if ( p.getType() == WIR_ParameterType::reg ) {
            ...
            if ( ( rp.getRegister() == r ) &&
              ...
              const WIR_DUUDChain &chainContainer =
              ...
              while ( it != mWebs.end() ) {
                ...
                bool usesIntersect =
                  any_of(
                    newWeb.uses.begin(), newWeb.uses.end(),
                    [&]( const WIR_RegisterParameter &newp ) {
                      return(
                        w.uses.count(
                          const_cast<WIR_RegisterParameter &>( newp ) ) ); } );
                DOUT(
                  "usesIntersect = " << boolalpha << usesIntersect <<
                  noboolalpha << endl );
                ...
                if ( rp.isDefUsed() )
                  ...
                  defUsesIntersect =
                    any_of(
                      newWeb.defs.begin(), newWeb.defs.end(),
                      [&]( const WIR_RegisterParameter &newp ) {
                        return(
                          w.uses.count(
                            const_cast<WIR_RegisterParameter &>(
                              newp ) ) ); } );
                DOUT(
                  "defUsesIntersect = " << boolalpha << defUsesIntersect <<
                  noboolalpha << endl );
              ...
              mWebs.push_back( std::move( newWeb ) );
            }
          }
  ...
  DACTION(
    ...
    bool first = true;
    for ( const Web &w : mWebs ) {
      if ( !first )
        DOUT( endl );
      first = false;
      DOUT( "  DEFs:" );
      for ( const WIR_RegisterParameter &rp : w.defs )
        DOUT( " " << rp.getID() );
      DOUT( endl << "  USEs:" );
      for ( const WIR_RegisterParameter &rp : w.uses )
        DOUT( " " << rp.getID() );
      DOUT( endl );
    } );
};
...
void WIR_Webs::createWebs( const WIR_VirtualRegister &r )
{
  ...
  if ( mWebs.size() > 1 ) {
    for ( Web &w : mWebs ) {
      ...
      mNewWebs.push_back( std::move( newWeb ) );
    }
  }
...
void WIR_Webs::cloneRegisters( const WIR_VirtualRegister &r )
{
  ...
  while ( !worklist.empty() ) {
    ...
    auto &clone1 = clonedWorklist.front().get();
    worklist.pop_front();
    clonedWorklist.pop_front();

    mClonedReg[ reg.getID() ] = &(clone1);
    DOUT(
      "Adding new clone " << clone1.getName() << " for register " <<
      reg.getName() << endl );

    copy( reg.begin(), reg.end(), back_inserter( worklist ) );
    copy( clone1.begin(), clone1.end(), back_inserter( clonedWorklist ) );
  }
};
...
void WIR_Webs::cloneParameter( WIR_RegisterParameter &p, struct Web &newWeb )
{
  ...
  auto newIt = o.replaceParameter( it, std::move( newParam ) );
