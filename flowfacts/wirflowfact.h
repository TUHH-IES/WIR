...
class WIR_FlowFact : public WIR_ID_API
{
  ...
  public:
    ...
    */
    ...
    /*!
      @brief checkFlowFact inspects a flow fact in depth and verifies that all
             flow fact-related data structures addressing this flow fact are in
             a coherent state.

      @param[in] s A const reference to the WIR_System to which this flow fact
                   belongs.
      @return true if the flow fact-related data structures are in a coherent
              state, false otherwise.

      Since the implementation details depend on some actual flow fact's
      characteristics, checkFlowFact is purely virtual here.

      @author Heiko Falk <Heiko.Falk@tuhh.de>
    */
    virtual bool checkFlowFact( const WIR_System &s ) const = 0;
