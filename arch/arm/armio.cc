...
std::ostream &arm( std::ostream &os )
{
  ...
  os.iword( WIR_ProcessorIO() ) = ARMv4::getProcessorTypeID();
  return( os );
};
...
void dumpARMBasicBlock( std::ostream &os, const WIR_BasicBlock &b )
{
  ...
  if ( b.isInserted() ) {
    const WIR_Function &f = b.getFunction();
    if ( f.isInserted() ) {
      const WIR_CompilationUnit &c = f.getCompilationUnit();
      if ( c.isInserted() ) {
        const WIR_System &sys = c.getSystem();
        const WIR_Symbol &sym = sys.findSymbol( b );
        ...
        if ( sec.getID() != previousCodeSection ) {
          ...
          if ( sec.getName() != ".text" ) {
            const WIR_MemoryRegion &r = sec.getRegion();
  ...
  if ( os.iword( WIR_CommentCont() ) == 1 )
    for ( const WIR_Comment &cont : b.getContainers<WIR_Comment>() )
      os << cont << endl;
  if ( os.iword( WIR_FileInfoCont() ) == 1 )
    for ( const WIR_FileInfo &cont : b.getContainers<WIR_FileInfo>() )
      os << cont << endl;
  ...
  // Output instructions.
  for ( const WIR_Instruction &i : b.getInstructions() )
    os << i;
};
...
void dumpARMCompilationUnit( std::ostream &os, const WIR_CompilationUnit &c )
{
  ...
  if ( os.iword( WIR_CommentCont() ) == 1 )
    for ( const WIR_Comment &cont : c.getContainers<WIR_Comment>() )
      os << cont << endl;
  if ( os.iword( WIR_FileInfoCont() ) == 1 )
    for ( const WIR_FileInfo &cont : c.getContainers<WIR_FileInfo>() )
      os << cont << endl;

  // Output the data objects section-wise.
  auto dataLayout = c.getDataLayout();
...
void dumpARMData( std::ostream &os, const WIR_Data &d )
{
  ...
  if ( d.isInserted() && d.getCompilationUnit().isInserted() ) {
    const WIR_System &sys = d.getCompilationUnit().getSystem();
    const WIR_Symbol &sym = sys.findSymbol( d );
    ...
    if ( d.isInitialized() || ( sym.getSection().getName() != ".bss" ) ) {
      ...
      if ( !d.isInitialized() )
        ...
        for ( const WIR_DataInit &i : d.getInitData() ) {
          switch ( i.getType() ) {
...
/*
  ...
  fashion.
*/
void dumpARMDataSection( std::ostream &os, const WIR_Section *sec,
                         const std::list<std::reference_wrapper<WIR_Data>> &l,
                         bool firstSec )
{
  DSTART(
    "void dumpARMDataSection(ostream&, const WIR_Section*, const "
    "list<reference_wrapper<WIR_Data> >&, bool)" );

  const WIR_MemoryRegion &r = sec->getRegion();
  const WIR_System &sys = r.getSystem();

  // Determine whether there are data objects to be dumped.
  bool dumpData = false;
  for ( const WIR_Data &d : l ) {
    const WIR_Symbol &sym = sys.findSymbol( d );
    if ( !sym.isExtern() ) {
      dumpData = true;
      break;
...
void dumpARMFunction( std::ostream &os, const WIR_Function &f )
{
  ...
  const WIR_Symbol *symPtr = nullptr;
  ...
  if ( os.iword( WIR_CommentCont() ) == 1 )
    for ( const WIR_Comment &cont : f.getContainers<WIR_Comment>() )
      os << cont << endl;
  if ( os.iword( WIR_FileInfoCont() ) == 1 )
    for ( const WIR_FileInfo &cont : f.getContainers<WIR_FileInfo>() )
      os << cont << endl;
  ...
  // Output basic blocks.
  for ( const WIR_BasicBlock &b : f.getBasicBlocks() )
    os << b;
...
void dumpARMOperation( std::ostream &os, const WIR_Operation &o )
{
  ...
  if ( os.iword( WIR_CommentCont() ) == 1 )
    for ( const WIR_Comment &cont : o.getContainers<WIR_Comment>() )
      os << cont << endl;
  if ( os.iword( WIR_FileInfoCont() ) == 1 )
    for ( const WIR_FileInfo &cont : o.getContainers<WIR_FileInfo>() )
      os << cont << endl;
  ...
  if ( ( oc == ARMv4::OpCode::LDRB ) ||
       ( oc == ARMv4::OpCode::LDRBT ) ||
       ( oc == ARMv4::OpCode::LDRH ) ||
       ( oc == ARMv4::OpCode::LDRSB ) ||
       ( oc == ARMv4::OpCode::LDRSH ) ||
       ( oc == ARMv4::OpCode::LDRT ) ||
       ( oc == ARMv4::OpCode::STRB ) ||
       ( oc == ARMv4::OpCode::STRBT ) ||
       ( oc == ARMv4::OpCode::STRH ) ||
       ( oc == ARMv4::OpCode::STRT ) ||
       ( oc == ARMv4::OpCode::SWPB ) ||
  ...
  if ( o.getParameters().front().get().getType() ==
    ...
    const WIR_ConditionFieldParameter &c =
      dynamic_cast<WIR_ConditionFieldParameter &>(
        o.getParameters().front().get() );

    if ( c.getCondition() != ARMv4::Condition::al )
      mnemonic << c;
  }
  ...
  if ( ( oc == ARMv4::OpCode::LDM ) || ( oc == ARMv4::OpCode::STM ) ) {
    // Special handling of LDM/STM opcodes.
    const WIR_AddressingModeParameter &m =
  ...
  if ( ( f == ARMv4::OperationFormat::CASRAC10_2 ) ||
       ( f == ARMv4::OperationFormat::CASARAC10_2 ) ||
       ( f == ARMv4::OperationFormat::CASRC8_2 ) ||
       ( f == ARMv5T::OperationFormat::ASRAC8_2 ) ||
       ( f == ARMv5T::OperationFormat::ASARAC8_2 ) ||
       ( f == ARMv5T::OperationFormat::ASRC8_2 ) )
    mnemonic << "l";

  if ( ( f == ARMv4::OperationFormat::CRRC8RA_2 ) ||
       ( f == ARMv4::OperationFormat::CRRR_2 ) ||
       ( f == ARMv4::OperationFormat::CRRR_4 ) ||
       ( f == ARMv4::OperationFormat::CRRRAR_2 ) ||
       ( f == ARMv4::OperationFormat::CRRRC5_2 ) ||
       ( f == ARMv4::OperationFormat::CRRRAC60_2 ) ||
       ( f == ARMv4::OperationFormat::CRRRC50_2 ) ||
       ( f == ARMv4::OperationFormat::CRRRR_2 ) ||
       ( f == ARMv4::OperationFormat::CRRRR_4 ) ||
       ( f == ARMv4::OperationFormat::CRC8RA_2 ) ||
       ( f == ARMv4::OperationFormat::CRR_2 ) ||
       ( f == ARMv4::OperationFormat::CRR_4 ) ||
       ( f == ARMv4::OperationFormat::CRRAR_2 ) ||
       ( f == ARMv4::OperationFormat::CRRC5_2 ) ||
       ( f == ARMv4::OperationFormat::CRRAC60_2 ) ||
       ( f == ARMv4::OperationFormat::CRRC50_2 ) )
    // Dump S-Bit.
    mnemonic << "s";
  ...
  for ( auto it = params.begin(); it != params.end(); ++it ) {
    ...
    if ( emitParam ) {
      ...
      bool addressOffset =
        ( ( expParamCount > 1 ) &&
          ( ( f == ARMv4::OperationFormat::CASRAC10_1 ) ||
            ( f == ARMv4::OperationFormat::CASRAC10_2 ) ||
            ( f == ARMv4::OperationFormat::CASARAC10_1 ) ||
            ( f == ARMv4::OperationFormat::CASARAC10_2 ) ||
            ( f == ARMv5T::OperationFormat::ASRAC8_1 ) ||
            ( f == ARMv5T::OperationFormat::ASRAC8_2 ) ||
            ( f == ARMv5T::OperationFormat::ASARAC8_1 ) ||
            ( f == ARMv5T::OperationFormat::ASARAC8_2 ) ) ) ||
        ( f == ARMv4::OperationFormat::CRRAC8_1 ) ||
        ( f == ARMv4::OperationFormat::CRRAC12_1 ) ||
        ( f == ARMv4::OperationFormat::CRARAC8_1 ) ||
        ( f == ARMv4::OperationFormat::CRARAC12_1 ) ||
        ( f == ARMv4::OperationFormat::CRRAC8_2 ) ||
        ( f == ARMv4::OperationFormat::CRRAC12_2 ) ||
        ( f == ARMv4::OperationFormat::CRARAC8_2 ) ||
        ( f == ARMv4::OperationFormat::CRARAC12_2 ) ||
      ...
      if ( firstParam ) {
        ...
        if ( ( f == ARMv4::OperationFormat::CAAAAC8RA_1 ) ||
             ( f == ARMv4::OperationFormat::CAAAAR_1 ) )
          os << "CPSR_";
        if ( ( f == ARMv4::OperationFormat::CAAAAC8RA_2 ) ||
             ( f == ARMv4::OperationFormat::CAAAAR_2 ) )
          os << "SPSR_";
      ...
      if ( ( ( expParamCount == 2 ) &&
             ( ( f == ARMv4::OperationFormat::CRRAC8_1 ) ||
               ...
               ( f == ARMv4::OperationFormat::CRARAC8_1 ) ||
               ( f == ARMv4::OperationFormat::CRARAC12_1 ) ||
               ( f == ARMv4::OperationFormat::CRRAR_3 ) ||
               ( f == ARMv4::OperationFormat::CRARAR_1 ) ||
               ( f == ARMv4::OperationFormat::CRRARC5_1 ) ||
               ( f == ARMv4::OperationFormat::CRRARAC60_1 ) ||
               ( f == ARMv4::OperationFormat::CRRARC50_1 ) ||
               ( f == ARMv4::OperationFormat::CRRAR_4 ) ||
               ( f == ARMv4::OperationFormat::CRARARC5_1 ) ||
               ( f == ARMv4::OperationFormat::CRARARAC60_1 ) ||
               ( f == ARMv4::OperationFormat::CRARARC50_1 ) ||
               ( f == ARMv4::OperationFormat::CRARAR_2 ) ||
               ( f == ARMv4::OperationFormat::CRRAC8_2 ) ||
               ( f == ARMv4::OperationFormat::CRRAC12_2 ) ||
               ( f == ARMv4::OperationFormat::CRARAC8_2 ) ||
               ( f == ARMv4::OperationFormat::CRARAC12_2 ) ||
               ( f == ARMv4::OperationFormat::CRRAR_6 ) ||
               ( f == ARMv4::OperationFormat::CRARAR_3 ) ||
               ( f == ARMv4::OperationFormat::CRRARC5_2 ) ||
               ( f == ARMv4::OperationFormat::CRRARAC60_2 ) ||
               ( f == ARMv4::OperationFormat::CRRARC50_2 ) ||
               ( f == ARMv4::OperationFormat::CRRAR_7 ) ||
               ( f == ARMv4::OperationFormat::CRARARC5_2 ) ||
               ( f == ARMv4::OperationFormat::CRARARAC60_2 ) ||
               ( f == ARMv4::OperationFormat::CRARARC50_2 ) ||
               ( f == ARMv4::OperationFormat::CRARAR_4 ) ||
           ...
           ( ( expParamCount == 3 ) &&
             ( ( f == ARMv4::OperationFormat::CASRAC10_1 ) ||
               ...
               ( f == ARMv4::OperationFormat::CASARAC10_1 ) ||
               ( f == ARMv4::OperationFormat::CASARAC10_2 ) ||
               ( f == ARMv4::OperationFormat::CASRC8_1 ) ||
               ( f == ARMv4::OperationFormat::CASRC8_2 ) ||
               ( f == ARMv5T::OperationFormat::ASRAC8_1 ) ||
               ( f == ARMv5T::OperationFormat::ASRAC8_2 ) ||
               ( f == ARMv5T::OperationFormat::ASARAC8_1 ) ||
      ...
      switch ( p.getType() ) {
        ...
        case WIR_ParameterType::addr: {
          const auto &addP = dynamic_cast<WIR_AddressingModeParameter &>( p );

          bool coprocessor =
            ( addP.getAddressingMode() == ARMv4::AddressingMode::p0 ) ||
            ( addP.getAddressingMode() == ARMv4::AddressingMode::p1 ) ||
            ( addP.getAddressingMode() == ARMv4::AddressingMode::p2 ) ||
            ( addP.getAddressingMode() == ARMv4::AddressingMode::p3 ) ||
            ( addP.getAddressingMode() == ARMv4::AddressingMode::p4 ) ||
            ( addP.getAddressingMode() == ARMv4::AddressingMode::p5 ) ||
            ( addP.getAddressingMode() == ARMv4::AddressingMode::p6 ) ||
            ( addP.getAddressingMode() == ARMv4::AddressingMode::p7 ) ||
            ( addP.getAddressingMode() == ARMv4::AddressingMode::p8 ) ||
            ( addP.getAddressingMode() == ARMv4::AddressingMode::p9 ) ||
            ( addP.getAddressingMode() == ARMv4::AddressingMode::p10 ) ||
            ( addP.getAddressingMode() == ARMv4::AddressingMode::p11 ) ||
            ( addP.getAddressingMode() == ARMv4::AddressingMode::p12 ) ||
            ( addP.getAddressingMode() == ARMv4::AddressingMode::p13 ) ||
            ( addP.getAddressingMode() == ARMv4::AddressingMode::p14 ) ||
            ( addP.getAddressingMode() == ARMv4::AddressingMode::p15 );

          // After an addressing mode, the comma must be omitted (except MSR and
          // coprocessor operations).
          omitComma = true;
          if ( ( ( expParamCount == 4 ) && ( oc == ARMv4::OpCode::MSR ) ) ||
               coprocessor ||
               ( o.getOperationFormat() == ARMv6::OperationFormat::AC5_1 ) )
            omitComma = false;

          // Check special cxsf flags of MSR operation.
          if ( addP.getAddressingMode() == ARMv4::AddressingMode::c ) {
            if ( !cEmitted )
              cEmitted = true;
            else
              break;
          }
          if ( addP.getAddressingMode() == ARMv4::AddressingMode::x ) {
            if ( !xEmitted )
              xEmitted = true;
            else
              break;
          }
          if ( addP.getAddressingMode() == ARMv4::AddressingMode::s ) {
            if ( !sEmitted )
              sEmitted = true;
            else
              break;
          }
          if ( addP.getAddressingMode() == ARMv4::AddressingMode::f ) {
            if ( !fEmitted )
              fEmitted = true;
            else
              break;
          ...
          postIndex =
            ( addP.getAddressingMode() == ARMv4::AddressingMode::post );

          if ( !postIndex &&
               ( addP.getAddressingMode() != ARMv4::AddressingMode::pre ) ) {
            if ( addressOffset )
              os << "#" << p;
            else
            ...
            if ( ( ( expParamCount == 2 ) &&
                 ...
                 ( f == ARMv4::OperationFormat::CRARARC50_2 ) ||
                 ( f == ARMv4::OperationFormat::CRARAR_4 ) ||
                 ( oc == ARMv4::OpCode::MSR ) || coprocessor ||
          ...
          if ( addP.getAddressingMode() == ARMv4::AddressingMode::pre )
            emitExcl = true;
        ...
        case WIR_ParameterType::imm: {
          if ( ( expParamCount != 5 ) &&
               ( ( f == ARMv4::OperationFormat::CAAAAC8RA_1 ) ||
                 ( f == ARMv4::OperationFormat::CAAAAC8RA_2 ) ) )
            break;

          if ( ( f == ARMv4::OperationFormat::CRRRC5_1 ) ||
               ( f == ARMv4::OperationFormat::CRRC5_1 ) ||
               ( f == ARMv4::OperationFormat::CRRC5_3 ) ||
               ( f == ARMv4::OperationFormat::CRRARC5_1 ) ||
               ( f == ARMv4::OperationFormat::CRARARC5_1 ) ||
               ( f == ARMv4::OperationFormat::CRRARC5_2 ) ||
               ( f == ARMv4::OperationFormat::CRARARC5_2 ) ||
          ...
          if ( ( f == ARMv4::OperationFormat::CRRRC50_1 ) ||
               ( f == ARMv4::OperationFormat::CRRC50_1 ) ||
               ( f == ARMv4::OperationFormat::CRRC50_3 ) ||
               ( f == ARMv4::OperationFormat::CRRARC50_1 ) ||
               ( f == ARMv4::OperationFormat::CRARARC50_1 ) ||
               ( f == ARMv4::OperationFormat::CRRARC50_2 ) ||
               ( f == ARMv4::OperationFormat::CRARARC50_2 ) ||
          ...
          if ( ( f == ARMv4::OperationFormat::CASRC8_1 ) ||
               ( f == ARMv4::OperationFormat::CASRC8_2 ) ||
               ( f == ARMv5T::OperationFormat::ASRC8_1 ) ||
               ( f == ARMv5T::OperationFormat::ASRC8_2 ) )
            os << "{ ";
          ...
          // are prefixed with a '#'.
          if ( !( ( ( expParamCount == 4 ) &&
                    ( ( f == ARMv4::OperationFormat::CRRC8RA_1 ) ||
                      ( f == ARMv4::OperationFormat::CRRC8RA_2 ) ) ) ||
                  ( ( expParamCount == 3 ) &&
                    ( ( f == ARMv4::OperationFormat::CRC8RA_1 ) ||
                      ( f == ARMv4::OperationFormat::CRC8RA_2 ) ||
                      ( f == ARMv4::OperationFormat::CRC8RA_3 ) ) ) ) &&
               !addressOffset )
            os << "#";
        ...
        case WIR_ParameterType::reg: {
          ...
          if ( ( expParamCount == 1 ) &&
               ( ( f == ARMv4::OperationFormat::CARR1_2 ) ||
                 ( f == ARMv4::OperationFormat::CARR2_2 ) ||
                 ( f == ARMv4::OperationFormat::CARR3_2 ) ||
                 ( f == ARMv4::OperationFormat::CARR4_2 ) ||
                 ( f == ARMv4::OperationFormat::CARR5_2 ) ||
                 ( f == ARMv4::OperationFormat::CARR6_2 ) ||
                 ( f == ARMv4::OperationFormat::CARR7_2 ) ||
                 ( f == ARMv4::OperationFormat::CARR8_2 ) ||
                 ( f == ARMv4::OperationFormat::CARR9_2 ) ||
                 ( f == ARMv4::OperationFormat::CARR10_2 ) ||
                 ( f == ARMv4::OperationFormat::CARR11_2 ) ||
                 ( f == ARMv4::OperationFormat::CARR12_2 ) ||
                 ( f == ARMv4::OperationFormat::CARR13_2 ) ||
                 ( f == ARMv4::OperationFormat::CARR14_2 ) ||
                 ( f == ARMv4::OperationFormat::CARR15_2 ) ||
                 ( f == ARMv4::OperationFormat::CARR16_2 ) ||
                 ( f == ARMv4::OperationFormat::CARR1_7 ) ||
                 ( f == ARMv4::OperationFormat::CARR2_7 ) ||
                 ( f == ARMv4::OperationFormat::CARR3_7 ) ||
                 ( f == ARMv4::OperationFormat::CARR4_7 ) ||
                 ( f == ARMv4::OperationFormat::CARR5_7 ) ||
                 ( f == ARMv4::OperationFormat::CARR6_7 ) ||
                 ( f == ARMv4::OperationFormat::CARR7_7 ) ||
                 ( f == ARMv4::OperationFormat::CARR8_7 ) ||
                 ( f == ARMv4::OperationFormat::CARR9_7 ) ||
                 ( f == ARMv4::OperationFormat::CARR10_7 ) ||
                 ( f == ARMv4::OperationFormat::CARR11_7 ) ||
                 ( f == ARMv4::OperationFormat::CARR12_7 ) ||
                 ( f == ARMv4::OperationFormat::CARR13_7 ) ||
                 ( f == ARMv4::OperationFormat::CARR14_7 ) ||
                 ( f == ARMv4::OperationFormat::CARR15_7 ) ||
                 ( f == ARMv4::OperationFormat::CARR16_7 ) ||
  ...
  if ( ( f == ARMv4::OperationFormat::CRRR_3 ) ||
       ( f == ARMv4::OperationFormat::CRR_3 ) ||
       ( f == ARMv4::OperationFormat::CRR_6 ) ||
       ( f == ARMv4::OperationFormat::CRRAR_4 ) ||
       ( f == ARMv4::OperationFormat::CRARAR_2 ) ||
       ( f == ARMv4::OperationFormat::CRRAR_7 ) ||
       ( f == ARMv4::OperationFormat::CRARAR_4 ) ||
  ...
  if ( ( f == ARMv4::OperationFormat::CASRAC10_1 ) ||
       ( f == ARMv4::OperationFormat::CASRAC10_2 ) ||
       ( f == ARMv4::OperationFormat::CRRAC8_1 ) ||
       ( f == ARMv4::OperationFormat::CRRAC12_1 ) ||
       ( f == ARMv4::OperationFormat::CRRAR_3 ) ||
  ...
  if ( f == ARMv4::OperationFormat::CR_1 )
    os << ", CPSR";
  if ( f == ARMv4::OperationFormat::CR_2 )
    os << ", SPSR";
  if ( f == ARMv6::OperationFormat::AC5_3 )
    os << "!";

  if ( ( oc == ARMv4::OpCode::LDM ) || ( oc == ARMv4T::OpCode::LDMIA ) ||
       ( oc == ARMv4T::OpCode::POP ) || ( oc == ARMv4T::OpCode::PUSH ) ||
       ( oc == ARMv4::OpCode::STM ) || ( oc == ARMv4T::OpCode::STMIA ) )
    // Output closing curly braces for the register lists required by LDM,
    // LDMIA, POP, PUSH, STM and STMIA.
    os << " }";

  if ( ( f == ARMv4::OperationFormat::CARR1_3 ) ||
       ( f == ARMv4::OperationFormat::CARR1_4 ) ||
       ( f == ARMv4::OperationFormat::CARR1_5 ) ||
       ( f == ARMv4::OperationFormat::CARR1_8 ) ||
       ( f == ARMv4::OperationFormat::CARR2_3 ) ||
       ( f == ARMv4::OperationFormat::CARR2_4 ) ||
       ( f == ARMv4::OperationFormat::CARR2_5 ) ||
       ( f == ARMv4::OperationFormat::CARR2_8 ) ||
       ( f == ARMv4::OperationFormat::CARR3_3 ) ||
       ( f == ARMv4::OperationFormat::CARR3_4 ) ||
       ( f == ARMv4::OperationFormat::CARR3_5 ) ||
       ( f == ARMv4::OperationFormat::CARR3_8 ) ||
       ( f == ARMv4::OperationFormat::CARR4_3 ) ||
       ( f == ARMv4::OperationFormat::CARR4_4 ) ||
       ( f == ARMv4::OperationFormat::CARR4_5 ) ||
       ( f == ARMv4::OperationFormat::CARR4_8 ) ||
       ( f == ARMv4::OperationFormat::CARR5_3 ) ||
       ( f == ARMv4::OperationFormat::CARR5_4 ) ||
       ( f == ARMv4::OperationFormat::CARR5_5 ) ||
       ( f == ARMv4::OperationFormat::CARR5_8 ) ||
       ( f == ARMv4::OperationFormat::CARR6_3 ) ||
       ( f == ARMv4::OperationFormat::CARR6_4 ) ||
       ( f == ARMv4::OperationFormat::CARR6_5 ) ||
       ( f == ARMv4::OperationFormat::CARR6_8 ) ||
       ( f == ARMv4::OperationFormat::CARR7_3 ) ||
       ( f == ARMv4::OperationFormat::CARR7_4 ) ||
       ( f == ARMv4::OperationFormat::CARR7_5 ) ||
       ( f == ARMv4::OperationFormat::CARR7_8 ) ||
       ( f == ARMv4::OperationFormat::CARR8_3 ) ||
       ( f == ARMv4::OperationFormat::CARR8_4 ) ||
       ( f == ARMv4::OperationFormat::CARR8_5 ) ||
       ( f == ARMv4::OperationFormat::CARR8_8 ) ||
       ( f == ARMv4::OperationFormat::CARR9_3 ) ||
       ( f == ARMv4::OperationFormat::CARR9_4 ) ||
       ( f == ARMv4::OperationFormat::CARR9_5 ) ||
       ( f == ARMv4::OperationFormat::CARR9_8 ) ||
       ( f == ARMv4::OperationFormat::CARR10_3 ) ||
       ( f == ARMv4::OperationFormat::CARR10_4 ) ||
       ( f == ARMv4::OperationFormat::CARR10_5 ) ||
       ( f == ARMv4::OperationFormat::CARR10_8 ) ||
       ( f == ARMv4::OperationFormat::CARR11_3 ) ||
       ( f == ARMv4::OperationFormat::CARR11_4 ) ||
       ( f == ARMv4::OperationFormat::CARR11_5 ) ||
       ( f == ARMv4::OperationFormat::CARR11_8 ) ||
       ( f == ARMv4::OperationFormat::CARR12_3 ) ||
       ( f == ARMv4::OperationFormat::CARR12_4 ) ||
       ( f == ARMv4::OperationFormat::CARR12_5 ) ||
       ( f == ARMv4::OperationFormat::CARR12_8 ) ||
       ( f == ARMv4::OperationFormat::CARR13_3 ) ||
       ( f == ARMv4::OperationFormat::CARR13_4 ) ||
       ( f == ARMv4::OperationFormat::CARR13_5 ) ||
       ( f == ARMv4::OperationFormat::CARR13_8 ) ||
       ( f == ARMv4::OperationFormat::CARR14_3 ) ||
       ( f == ARMv4::OperationFormat::CARR14_4 ) ||
       ( f == ARMv4::OperationFormat::CARR14_5 ) ||
       ( f == ARMv4::OperationFormat::CARR14_8 ) ||
       ( f == ARMv4::OperationFormat::CARR15_3 ) ||
       ( f == ARMv4::OperationFormat::CARR15_4 ) ||
       ( f == ARMv4::OperationFormat::CARR15_5 ) ||
       ( f == ARMv4::OperationFormat::CARR15_8 ) ||
       ( f == ARMv4::OperationFormat::CARR16_3 ) ||
       ( f == ARMv4::OperationFormat::CARR16_4 ) ||
       ( f == ARMv4::OperationFormat::CARR16_8 ) )
    // Emit trailing caret '^'.
    os << "^";
