...
WIR_UpDownValue::WIR_UpDownValue( WIR_UpDownValue &&__o ) :
  mBitValues { std::move( __o.mBitValues ) },
  mBitWidth { __o.mBitWidth },
  mIsSigned { __o.mIsSigned },
...
/*
  ...
  This operator implements the Booth algorithm for efficient signed
  multiplication, extended to the partial order L4. It asserts if up/down values
  of different bit widths are combined.
*/
WIR_UpDownValue WIR_UpDownValue::operator * ( const WIR_UpDownValue &__o ) const
{
  DSTART(
  ...
  const unsigned int n = __o.mBitWidth;

  #ifdef FAILSAFEMODE
  ufAssert( mBitWidth == n );
  #endif

  // Handle special cases:
  // 1 * a = a
  if ( isBinaryInteger() && ( getSignedValue() == 1 ) )
    return( __o.extend( 2 * n ) );

  // a * 1 = a
  if ( __o.isBinaryInteger() && ( __o.getSignedValue() == 1 ) )
    return( extend( 2 * n ) );
  ...
  // 0 * a = 0
  if ( isBinaryInteger() && ( getSignedValue() == 0 ) )
    return( extend( 2 * n ) );

  // a * 0 = 0
  if ( __o.isBinaryInteger() && ( __o.getSignedValue() == 0 ) )
    return( __o.extend( 2 * n ) );

  // If both up/down values have the same bit width, the result has 2n + 1 bits.
  WIR_UpDownValue res { WIR_L4::b0, 2 * n + 1, true };

  // Initialize the result:
  // - Least-significant bit = 0 (initialized above)
  // - Bits n+1 ... 2*n + 1 = 0 (initialized above)
  // - Bits 1 ... n = __o
  insert( res, __o, 1 );

  // rUpper helper variable = most-significant n bits of res.
  WIR_UpDownValue rUpper { WIR_L4::b0, n, mIsSigned };

  for ( unsigned int i = 0; i < n; ++i ) {
    // Get current most-significant n bits of res.
    rUpper = res.extract( n + 1, n );

    // Case 1: res[0-1] = 00 or 11 -> do nothing.
    if ( ( ( res[ 1 ] == WIR_L4::b0 ) && ( res[ 0 ] == WIR_L4::b0 ) ) ||
         ( ( res[ 1 ] == WIR_L4::b1 ) && ( res[ 0 ] == WIR_L4::b1 ) ) ) {
    } else

    // Case 2: res[0-1] = 10 -> subtract *this from rUpper.
    if ( ( res[ 1 ] == WIR_L4::b1 ) && ( res[ 0 ] == WIR_L4::b0 ) ) {
      rUpper = rUpper - (*this);
      insert( res, rUpper, n + 1 );
    } else

    // Case 3: res[0-1] = 01 -> add *this to rUpper.
    if ( ( res[ 1 ] == WIR_L4::b0 ) && ( res[ 0 ] == WIR_L4::b1 ) ) {
      rUpper = rUpper + (*this);
      insert( res, rUpper, n + 1 );
    } else

    // Case 4: res[0-1] = XX or 1X or X1 -> set both bits to 1.
    if ( ( ( res[ 1 ] == WIR_L4::bX ) && ( res[ 0 ] == WIR_L4::bX ) ) ||
         ( ( res[ 1 ] == WIR_L4::b1 ) && ( res[ 0 ] == WIR_L4::bX ) ) ||
         ( ( res[ 1 ] == WIR_L4::bX ) && ( res[ 0 ] == WIR_L4::b1 ) ) ) {
      res[ 0 ] = WIR_L4::b1;
      res[ 1 ] = WIR_L4::b1;
    } else

    // Case 5: res[0-1] = 0X or X0 -> set both bits 0.
    if ( ( ( res[ 1 ] == WIR_L4::b0 ) && ( res[ 0 ] == WIR_L4::bX ) ) ||
         ( ( res[ 1 ] == WIR_L4::bX ) && ( res[ 0 ] == WIR_L4::b0 ) ) ) {
      res[ 0 ] = WIR_L4::b0;
      res[ 1 ] = WIR_L4::b0;
    }

    // Case 6: res[0-1] includes L, N or U -> set rUpper to all U.
    else {
      rUpper.setAllBits( WIR_L4::bU );
      insert( res, rUpper, n + 1 );
    }

    // Shift result right arithmetically (res is signed) by one bit in any case.
    res = res >> 1;
  }

  return ( res.extract( 1, 2 * n ) );
};
...
/*
  ...
  This operator bases on a restoring division (D. Kumar, P. Saha, A. Dandapat.
  Hardware implementation of methodologies of fixed point division algorithms.
  Intl. Journal on Smart Sensing and Intelligent Systems vol. 10(3), 2017.)
  extended to signed values in the partial order L4. It asserts if up/down
  values of different bit widths are combined.
*/
WIR_UpDownValue WIR_UpDownValue::operator / ( const WIR_UpDownValue &__o ) const
{
  DSTART(
  ...
  // Return the quotient from the least-significant bits produced by the
  // restoring division.
  return( restoringDivision( __o ).extract( 0, mBitWidth ) );
};
...
WIR_UpDownValue WIR_UpDownValue::operator % ( const WIR_UpDownValue &__o ) const
{
  ...
  // Return the remainder from the most-significant bits produced by the
  // restoring division.
  return( restoringDivision( __o ).extract( mBitWidth, mBitWidth ) );
};
...
std::ostream & operator << ( std::ostream &os, const WIR_UpDownValue &v )
{
  ...
  const WIR_ID_API *currentBitPacket = nullptr;
  WIR_LocationType currentBitPacketType = WIR_LocationType::reg;
  ...
  auto dumpBitPacket = [&]( stringstream &str ) {
    ...
    if ( currentBitPacketType == WIR_LocationType::reg ) {
      const auto &p =
        *(dynamic_cast<const WIR_RegisterParameter *>( currentBitPacket ));
      const auto &r = p.getRegister();
      str << r.getName();
      if ( r.isVirtual() ) {
        const auto &vreg = dynamic_cast<const WIR_VirtualRegister &>( r );
        if ( vreg.isPrecolored() )
          str << "/" << vreg.getPrecolor().getName();
      }
    } else {
      const auto &s = *(dynamic_cast<const WIR_Symbol *>( currentBitPacket ));
      str << "&amp;" << s.getName();
    }
...
/*
  restoringDivision performs a bitwise division of two up/down values.

  This method implements a restoring division (D. Kumar, P. Saha, A. Dandapat.
  Hardware implementation of methodologies of fixed point division algorithms.
  Intl. Journal on Smart Sensing and Intelligent Systems vol. 10(3), 2017.)
  extended to signed values in the partial order L4. It asserts if up/down
  values of different bit widths are combined.

  The most-significant half of the returned result contains the division's
  remainder, while the least-significant half contains the quotient.
*/
WIR_UpDownValue WIR_UpDownValue::restoringDivision( const WIR_UpDownValue &__o ) const
{
  DSTART(
    "WIR_UpDownValue WIR_UpDownValue::restoringDivision(const "
  ...
  // Check whether the two operands to be divided have different sign bits.
  auto differentSigns =
    bitXOR(
      { mBitValues[ n - 1 ], mLocation[ n - 1 ] },
      { __o.mBitValues[ n - 1 ], __o.mLocation[ n - 1 ] } );

  // A2 stores the dividend, i.e., *this.
  WIR_UpDownValue A2 { *this };
  A2.setSignedness( true );

  // Determine whether the dividend is negative. If so, turn it into a positive
  // value.
  if ( A2.mBitValues[ n - 1 ] == WIR_L4::b1 )
    A2 = -A2;

  // B stores the divisor, i.e., __o.
  WIR_UpDownValue B { __o };
  B.setSignedness( true );

  // Make sure the divisor is not zero.
  if ( B.getSignedValue() == 0 )
    throw invalid_argument( "Division by zero." );

  // Determine whether the divisor is negative. If so, turn it into a positive
  // value.
  if ( B.mBitValues[ n - 1 ] == WIR_L4::b1 )
    B = -B;

  // res holds the result of the restoring division. The most-significant n bits
  // of res represent the remainder, the least-significant n bits represent the
  // integer result of the division.
  WIR_UpDownValue res { WIR_L4::b0, 2 * n, true };

  // Initialize res:
  // - Least-significant n bits = A2
  // - Most-significant n bits = 0 (initialized above)
  insert( res, A2, 0 );

  for ( unsigned int i = 0; i < n; ++i ) {
    // Shift result left by one bit in any case.
    res = res << 1;

    // Get current most-significant n bits of res.
    WIR_UpDownValue A1 { WIR_L4::b0, n, true };
    A1 = res.extract( n, n );

    // Set up a temporary variable holding the unconditional result of the
    // subtraction of the divisor.
    WIR_UpDownValue T { WIR_L4::bU, n, true };
    T = A1 - B;

    // Produce current quotient bit in res[ 0 ].
    res[ 0 ] = ~T[ n - 1 ];
    if ( T.isLocationBit( n - 1 ) )
      res.mLocation[ 0 ] = T.mLocation[ n - 1 ];

    if ( res[ 0 ] == WIR_L4::b1 )
      // Make the result of the unconditional subtraction of the divisor
      // permanent by inserting it into the most-significant n bits of res.
      insert( res, T, n );
  }

  // Adjust sign of the division's final result.
  if ( differentSigns.first == WIR_L4::b1 ) {
    auto quotient = res.extract( 0, n );
    auto remainder = res.extract( n, n );

    insert( res, -quotient, 0 );
    insert( res, -remainder, n );
  }
