...
int main( void )
{
  ...
  ufAssert( uval( cbnez1, 0 ).extract( 0, 3 ).containsOnlyBit( WIR_L4::bX ) );
  ufAssert( uval( cbnez1, 0 ).extract( 3, 1 ).containsOnlyBit( WIR_L4::b1 ) );
  ufAssert( uval( cbnez1, 0 ).extract( 4, 28 ).containsOnlyBit( WIR_L4::bX ) );

  ufAssert( uval( cbnez2, 0 ).isBinaryInteger() );
  ufAssert( uval( cbnez2, 0 ).getSignedValue() == 0 );
