...
bool TC_Peep_PreIncr::matchPeephole( const WIR_Peephole::peephole &p )
{
  ...
  if ( !mSingleAReg ||
       ( ( offset == 0 ) && ( o0.getOpCode() != TC13::OpCode::LEA ) &&
  ...
  if ( !mSingleAReg ) {
    ...
    for ( WIR_RegisterParameter &rDef : reachingDefs.getReachingDefinitions() )
      if ( defs.count( rDef ) )
        ...
        auto leafs = rDef.getRegister().getLeafs();

        if ( any_of(
               leafs.begin(), leafs.end(),
               [&]( const reference_wrapper<WIR_BaseRegister> ref ) {
                 return( ref.get() == aReg ); } ) )
          return( false );
      }
  }
...
void TC_Peep_PreIncr::transformPeephole( const WIR_Peephole::peephole &p )
{
  ...
  if ( mSingleAReg ) {
    ...
    const auto &incr = mFoundIncr->getInstruction();
...
const WIR_BaseProcessor::OperationFormat &TC_Peep_PreIncr::getNewOperationFormat( const WIR_Operation &o ) const
{
  ...
  // starting point.
  const WIR_Instruction &i = o.getInstruction();
  set<WIR_id_t> visited;

  function<void( const WIR_BasicBlock & )> dfs =
