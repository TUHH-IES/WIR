...
WIR_Section::WIR_Section( WIR_Section &&__o ) :
  ...
  mAlignment { std::move( __o.mAlignment ) },
  mBlock { std::move( __o.mBlock ) },
  mStart { std::move( __o.mStart ) },
  mAt { std::move( __o.mAt ) },
  mLength { std::move( __o.mLength ) },
  mFill { std::move( __o.mFill ) },
  mRegion { __o.mRegion },
  mLoadRegion { __o.mLoadRegion }
{
cout << BOOST_CURRENT_FUNCTION << endl;
...
void WIR_Section::setAlignment( std::size_t a )
{
  ...
  const WIR_BaseProcessor &p = getProcessor();
  if ( p.isInserted() )
...
/*
  getRegion returns the WIR_MemoryRegion to which this section is attached.
*/
