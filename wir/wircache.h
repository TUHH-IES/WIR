...
/*!
  ...
  @author Heiko Falk <Heiko.Falk@tuhh.de>
*/
class WIR_Cache : public WIR_SystemComponent
{
  ...
  public:
    ...
    */
    ...
    //! A type to represent cache line (tag + index).
    using CacheLineID = WIR_MemoryAddress;
    ...
    */
    ...
    /*!
      @brief isWriteAllocate returns whether the cache allocates on writes
             (true) or not (false).
    ...
    */
    ...
    /*!
      ...
      @author Heiko Falk <Heiko.Falk@tuhh.de>
    */
    virtual bool isActiveInRange( const WIR_AddressRange & ) const override;

    /*!
      @brief getLineID extracts a line's ID (tag and index) from the given
             address.

      @param[in] a A const reference to a memory address.
      @return An addresses line ID.

      @author Heiko Falk <Heiko.Falk@tuhh.de>
    */
    CacheLineID getLineID( const WIR_MemoryAddress & ) const;
    ...
    */
    ...
    /*!
      @brief getAffectedLineIDs determines the range of cache line IDs that may
             be affected by the given memory access.

      @param[in] a A const reference to a memory access in a given address
                   range.
      @return An interval of potentially affected cache line IDs if the cache
              is enabled, an empty interval otherwise.

      @author Heiko Falk <Heiko.Falk@tuhh.de>
    */
    WIR_Interval<CacheLineID> getAffectedLineIDs( const WIR_AddressRange & ) const;
  ...
  private:
    ...
    */
    ...
    /*!
      ...
      @author Heiko Falk <Heiko.Falk@tuhh.de>
    */
    virtual WIR_Cache *clone( void ) const override;
    ...
    */
    ...
    /*!
      ...
      Typical values range from 1 (direct mapped) to 8 (set-associative).
    */
