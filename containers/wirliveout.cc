...
WIR_LiveOut::WIR_LiveOut( WIR_LiveOut &&__o ) :
  WIR_Container<WIR_LiveOut> { std::move( __o ) },
  mRegisterReferences { std::move( __o.mRegisterReferences ) }
{
...
cout << BOOST_CURRENT_FUNCTION << endl;
  ...
  WIR_Container<WIR_LiveOut>::operator = ( std::move( __o ) );
...
cout << BOOST_CURRENT_FUNCTION << endl;
  ...
  return(
    any_of(
      mRegisterReferences.begin(), mRegisterReferences.end(),
      [&]( reference_wrapper<WIR_BaseRegister> ref ) {
